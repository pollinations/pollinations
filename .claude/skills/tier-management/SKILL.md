...
description: Look up a Pollinations user's Pollen balance and current tier (read-only). Tiers are frozen — there are no automatic or manual tier upgrades. For finding users with errors, see model-debugging skill first.
---
...
This skill is **read-only**: it looks up a user's current tier and balance. It does not change tiers.

> The hourly tier-balance refill ("the drip") still runs for now and is out of scope here.
...
Prints the user's id, GitHub username, email, current tier, tier balance, and account creation date — read straight from production D1.
