...
async function searchIssues(query) {
    const encoded = encodeURIComponent(
        `repo:pollinations/pollinations ${query}`,
    );
...
async function main() {
    ...
    const URL_COL = headers.findIndex((h) => h.toLowerCase() === "web_url");
    ...
    if (toRecover.length === 0) {
        console.log(`${colors.green}All apps have Issue_URLs!${colors.reset}`);
        return 0;
    }
    ...
    for (let idx = 0; idx < toRecover.length; idx++) {
        ...
        if (!verbose) {
            process.stdout.write(`\rSearching: ${idx + 1}/${toRecover.length}`);
        }
            ...
            const candidates = urlResults.filter(
                (issue) =>
                    ...
                    (issue.title.includes("[Project Submission]") ||
                        ...
                        issue.title
                            .toLowerCase()
                            .includes(app.name.toLowerCase().slice(0, 10))),
            );
    ...
    console.log(`${colors.green}✓ Recovered: ${changes.length}${colors.reset}`);
    console.log(`${colors.yellow}✗ Not found: ${notFound}${colors.reset}`);
