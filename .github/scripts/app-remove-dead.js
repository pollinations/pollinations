	...
	return new Promise((resolve) => {
		...
		execFile(
			...
			(err, stdout) => {
				...
				resolve({ code: isNaN(code) ? "error" : code });
			},
		);
	});
}
...
async function main() {
	...
	const appsWithUrls = apps.filter(
		(a) => a.url && a.url.startsWith("http"),
	);
	...
	for (let round = 1; round <= ROUNDS; round++) {
		...
		for (const { app, code } of results) {
			if (DEAD_CODES.has(code)) {
				failCounts.set(app.lineIndex, failCounts.get(app.lineIndex) + 1);
			}
		}
