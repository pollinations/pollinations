...
function isFresh(app) {
  return !!app.approvedDate && new Date(app.approvedDate) >= new Date(Date.now() - THIRTY_DAYS_MS);
}
function isPollen(app) { return app.byop; }
function isBuzz(app) { return app.requests24h >= 100; }
...
function starsStr(app) {
  if (!app.stars) return "";
  if (app.stars >= 1000) return `${(app.stars / 1000).toFixed(1).replace(/\.0$/, "")}k`;
  return String(app.stars);
}

function makeTable(apps) {
  const header = "| App | Description | Author | ⭐ | |\n|-----|-------------|--------|---:|---|";
...
function generateGarden(apps) {
  const buzzApps = apps.filter(isBuzz).sort((a, b) => b.requests24h - a.requests24h);
...
`;
  ...
  for (const cat of CATEGORIES) {
    const catApps = apps.filter((a) => a.category === cat.id).sort(sortApps);
...
function updateRootReadme(apps) {
  ...
  const rows = last10.map((a) =>
    `| ${appLink(a)} | ${a.description || ""} | ${authorStr(a)} |`
  );
