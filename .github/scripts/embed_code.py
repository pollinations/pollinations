...
# Minified bundles pack thousands of characters onto a line. Hand-written code — even
# long generated type definitions — stays far below this, so average line length
# separates "big file worth indexing" from "build artifact" better than raw size does.
MAX_AVG_LINE_CHARS = 400

# Conventional machine-written banners. Deliberately narrow: a loose match on
# "autogenerated" wrongly skips hand-maintained files whose header merely mentions that
# they started life generated (e.g. shared/db/better-auth.ts), which is real schema code
# people search for.
...
def is_generated(text: str) -> bool:
    ...
    skipped on content rather than on size: a large hand-written file (APPS.md, a long
    doc) is worth indexing, a small minified one is not.
    """
    lines = text.split("\n", GENERATED_MARKER_SCAN_LINES)
    header = "\n".join(lines[:GENERATED_MARKER_SCAN_LINES]).lower()
    if any(marker in header for marker in GENERATED_FILE_MARKERS):
        return True

    line_count = text.count("\n") + 1
    return (len(text) / line_count) > MAX_AVG_LINE_CHARS
...
def collect_code_files(repo_root: Path) -> list[Path]:
    """Every tracked file worth reading. Generated/minified content is filtered later,
    in build_rows_for_file, since that decision needs the file's contents."""
    return [repo_root / rel_path for rel_path in git_tracked_files(repo_root) if is_embeddable_path(rel_path)]
...
def build_rows_for_file(repo_root: Path, rel_path: str, git_sha: str) -> list[dict]:
    ...
    if is_generated(content):
        print(f"  skipping {rel_path} — generated/minified", file=sys.stderr)
        return []
