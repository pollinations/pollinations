...
/**
 * Script to manually update GitHub star counts
 * Usage: node update-github-stars.js
 */
...
async function main() {
  try {
    console.log('🚀 Starting GitHub star count update...');
    ...
    // Try to load existing data to maintain any manual entries
    try {
      if (fs.existsSync(OUTPUT_PATH)) {
        currentStars = JSON.parse(fs.readFileSync(OUTPUT_PATH, 'utf8'));
        console.log(`Loaded existing star data for ${Object.keys(currentStars).length} repositories`);
      }
    } catch (error) {
      console.warn('Could not load existing star data, starting fresh');
    }
    
    // Copy existing data
    Object.assign(starData, currentStars);
    ...
    for (const { category, name, repo } of repos) {
      ...
      if (repoInfo) {
        ...
        if (starData[key] && Object.keys(starData[key]).length > 0) continue;
