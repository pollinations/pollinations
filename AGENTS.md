...
- Always deploy to **staging first**, verify, then prod. Use the shared helper for both workspaces: `npm run tinybird:check:staging --workspace pollinations-enter`, then `npm run tinybird:deploy:staging --workspace pollinations-enter`, then the matching `production` commands.
- Treat Tinybird admin/deploy tokens as deployer credentials, not Worker runtime secrets. Materialize team-managed credentials into gitignored Tinybird CLI config files: `enter.pollinations.ai/observability/.tinyb.staging` for staging and `enter.pollinations.ai/observability/.tinyb` for production. Do not put Tinybird deploy tokens in Wrangler runtime secrets.
- Validate first with `tinybird:check:*` (against both workspaces if either schema is in doubt)
