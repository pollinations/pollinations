...
Text-to-speech endpoints follow the OpenAI speech contract: Pollinations POSTs `{ "model", "input", "voice", "response_format" }` to the upstream `/v1/audio/speech` and expects binary audio back with an `audio/*` content type. The returned bytes and format are passed to the caller unchanged. Speech is billed per input character through the completion audio price; the registration test posts a short sample and bills its character count the same way.
...
- Speech models are priced per input character.
