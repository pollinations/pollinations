...
> **Available now:** DeepSeek Harness (`dsh`) and OpenCode (`opencode`). Pi, Prime Agent, and OpenClaw are coming soon.
...
| [OpenCode](https://opencode.ai) (`opencode`) | **Available now** — `polli harness opencode on` | Uses the existing [Pollinations OpenCode plugin](https://github.com/fkom13/opencode-pollinations-plugin) for models, media tools, usage, and quests. |
...
Uses the existing Pollinations OpenCode plugin. If OpenCode is missing, the harness prints the official install command (`curl -fsSL https://opencode.ai/install | bash` or `npm install -g opencode-ai`). Choose another default model with `--model <id>`. The plugin provides Pollinations models, media tools, usage, and quests.
