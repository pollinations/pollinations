...
> **Available now:** Bloom CLI, DeepSeek Harness, OpenCode, OpenClaw, Pi, Prime Agent, Codex, and Claude Code are integrated `polli harness` profiles.
...
Current OpenClaw requires Node `>=24.16.0 <25` or `>=26.1.0`; Pi requires Node `>=22.19.0`. Upgrade Node before updating either harness if needed. Bloom requires Python 3.12 or newer. Stop a running DSH server before launching its replacement, and back up saved sessions before a major harness upgrade. `polli harness claude-code` needs `node:sqlite`, unavailable on some Node 22 builds without `NODE_OPTIONS=--experimental-sqlite`; upgrade Node or set that flag if `on` reports it missing.
...
| [Codex](https://github.com/openai/codex) via [Codex Router](https://github.com/duolahypercho/codex-router) | **Available now** — `polli harness codex on` | Adds Pollinations as a Codex Router generic provider with its own credential, preserving native ChatGPT login and models. Defaults to `openai/gpt-5.4-nano`. |
| [Claude Code](https://claude.com/claude-code) via [Claude Code Router](https://github.com/musistudio/claude-code-router) | **Available now** — `polli harness claude-code on` | Adds Pollinations as a router provider and sets it as the Claude Code profile's model, preserving native Claude login. Defaults to `anthropic/claude-sonnet-5`. |
...
`on` requires both Codex (`npm install -g @openai/codex`) and Codex Router (`curl -fsSL https://raw.githubusercontent.com/duolahypercho/codex-router/main/install.sh | sh -s -- --target codex --guided --with-tray`) to be installed. It adds Pollinations as a Codex Router "generic provider" in `~/.codex/codex-router/generic-providers.json` (or `$CODEX_ROUTER_STATE_DIR`, inside `$CODEX_HOME`, default `~/.codex`), stores a dedicated key referenced by the router's own credential store instead of inline in config, and republishes the router's model catalog and active model. The first run also bootstraps the router's shared Codex wiring in `config.toml` if it is not already present; `off` never touches that shared wiring, so other providers and Codex's native ChatGPT login, models, and profiles stay intact. Choose another default with `--model <id>`; without one it defaults to `openai/gpt-5.4-nano`.
...
`on` requires both Claude Code (`npm install -g @anthropic-ai/claude-code`) and Claude Code Router (`npm install -g @musistudio/claude-code-router`) to be installed. Claude Code Router keeps its config in a SQLite database (`~/.claude-code-router/config.sqlite`); `on` adds Pollinations to the `Providers` list and points the Claude Code profile's model at it, preserving every other provider and setting in that document. This needs `node:sqlite` (Node 22.5+; some Node 22 builds still require `NODE_OPTIONS=--experimental-sqlite`). Choose another default with `--model <id>`; without one it defaults to `anthropic/claude-sonnet-5`.
