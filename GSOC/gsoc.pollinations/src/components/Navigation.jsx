    ...
    <div className="h-16.25  w-full rounded-[20px] bg-transparent mx-auto flex items-center justify-center mt-8 absolute top-10">
      <div className="navigation w-[70%] h-full bg-white-200 mx-auto rounded-[10px] shadow-xl flex flex-col">
            <div className=" flex items-center justify-between h-full px-6 overflow-hidden">
              ...
              <div className="flex items-center gap-8 flex-1 justify-center flex flex-row">
                {navItems.map(item => (
                  <Link key={item.path} to={item.path} className="text-gray-700 font-bold text-[#888] hover:text-black transition">
                    {item.name}
                  </Link>
                ))}

                <div className="socials flex flex-col gap-2 box-border">
