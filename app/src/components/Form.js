...
function FormView({
  input: originalInput, connected, metadata, onSubmit, onCancel,
}) {
  ...
  const [properties, uiSchema] = useMemo(() => {
    ...
    if (!filledForm) return [null, null];
    ...
    Object.entries(uiSchema).forEach(([field, prop]) => {
      if (prop['ui:widget'] === 'file') filledForm[field].default = undefined;
    });
...
const getPrefixOverride = (key) => Object.entries(prefixMappings).find(([prefix]) => key.startsWith(prefix));

const overrideDefaultValues = (propertiesWithSocial, ipfsInput) => mapObjIndexed((prop, formKey) => (formKey in ipfsInput ? { ...prop, default: ipfsInput[formKey] } : prop), propertiesWithSocial);
