import {
  Box, Button, GridList, GridListTile, Paper,
...
function VideoDisplay({ filename, content, style }) {
  return <video alt={filename} controls src={content} width="100%" height="auto" preload="metadata" style={style} />;
}
function ImageDisplay({ filename, content, style }) {
  return <img alt={filename} src={content} width="100%" height="auto" style={style} />;
}
...
function AudioViewer({ content, style, filename }) {
  ...
  return (
    <>
      <audio controls src={content} style={{ ...style, height: null }} />
      {filename}
    </>
  );
}
...
export default function ({ output, contentID }) {
