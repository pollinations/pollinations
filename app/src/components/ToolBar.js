...
function ToolBar({ node, showNode }) {
  return (
...
function OutputPreview({ contentID }) {
  const ipfs = useIPFS(contentID);

  const { first } = useMemo(() => mediaToDisplay(ipfs.output), [ipfs?.output]);
  debug('first', first);
  return (
    <Box>
      <Typography>Intermediate Result:</Typography>
      <MediaViewer {...first} content={first.url} style={{ maxWidth: '70%' }} />
    </Box>
  );
}
