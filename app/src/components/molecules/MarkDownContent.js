...
function MarkDownContent({ id, ...replacements }) {
  ...
  const overrideElements = tags.map((tag) => function ({ children }) {
    return <Typography variant={tag} children={children} />;
  });
...
const applyReplacements = (replacements, content) => Object.entries(replacements).reduce(replaceOne, content);
