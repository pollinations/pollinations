...
export function getPostData(ipfs, cid, shortenPost = true) {
  ...
  debug('Created post data', {
    name, text, videoURL, coverImage: coverImageURL, url,
  });

  return {
    post, title, videoURL, coverImage: coverImageURL, url,
  };
}

const formatText = (shortenPost, possibleText) => (shortenPost ? possibleText[1] : `\n\n${possibleText[1]}\n\n`);
