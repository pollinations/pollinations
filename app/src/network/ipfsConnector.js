...
export function writer(initialRootCID = null) {
  ...
  const initializedFolder = getClient().then((client) => initializeMFSFolder(client, initialRootCID));
...
const localIPFSAvailable = async () => false;
...
export const getIPNSURL = (id) => `https://public-ipfs-gateway.pollinations.ai/ipns/${id}`;

const stripSlashIPFS = (cidString) => {
  if (!cidString) throw new Error('CID is falsy');
...
const _normalizeIPFS = ({
  name, path, cid, type,
}) => ({
  name, path, cid: stringCID(cid), type,
});
...
async function optionallyResolveIPNS(client, cid) {
  ...
  if (cid.startsWith('/ipns')) cid = await ipfsResolve(client, cid);
  return cid;
}
...
const ipfsResolve = async (client, path) => stringCID(last(await toPromise(client.name.resolve(path, { nocache: true }))));
