...
export function publisher(nodeID, suffix = '/output', useIPNS = true) {
  ...
  const _publish = async (cid) => {
    ...
    if (useIPNS && !createdIPNSKey) {
      ...
      } else debug('IPNS key already exists. Reusing');
...
async function publish(client, nodeID, rootCID, suffix = '/output', ipnsKeyName = null) {
  ...
  try {
    if (ipnsKeyName !== null) experimentalIPNSPublish(client, rootCID, ipnsKeyName);

    if (nodeID !== 'ipns') await retryPublish(nodeID + suffix, rootCID);
  } catch (e) {
  ...
  (async () => {
    ...
    const doSub = async () => {
      ...
      try {
        ...
        if (e.message?.startsWith('Already subscribed')) return;
        await awaitSleep(300);
        await doSub();
      }
    };
