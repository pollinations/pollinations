...
export const getIPFSState = async (contentID, callback = (f) => f, skipCache = false) => {
  ...
  try {
    return await cachedIPFSState(ipfsReader, {
      cid: contentID, name: 'root', type: 'dir', path: '/', rootCID: contentID,
    }, callback, skipCache);
...
const cachedIPFSState = (ipfsReader, { cid, ...rest }, processFile, skipCache) => {
  ...
  } else debug('cache hit', cid);
  return cache[key];
};

// Do the actual work
const _getIPFSState = async (ipfsReader, {
  cid, type, name, path, rootCID,
}, processFile, skipCache) => {
