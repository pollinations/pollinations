...
export default React.memo(({ ipfs, node, dispatch }) => {
...
// Alert Message
function AlertMessage({ connected }) {
  if (connected) return <></>;
  return (
    <Alert severity="info" style={{ margin: '2em 0' }}>
      ...
      {' '}
      because
      <b>no Colab node is running</b>
      ! Click on
      {' '}
      <b>LAUNCH</b>
      {' '}
      (bottom right) or refer to INSTRUCTIONS for further instructions.
    </Alert>
  );
}
