...
const EASY = /\b(translate|translation|summar\w+|rewrite|rephrase|capital|plural|spell|convert|list|name|define|what is|who is|when did|yes or no)\b/i;
const HARD = /\b(design|architect\w*|prove|proof|theorem|derive|trade-?offs?|optimi[sz]e|refactor|research|synthesi[sz]e|phd|rigorous|step by step|think carefully|critique|benchmark|multi-?region|failover)\b/i;
...
function blendedCost(model: CatalogModel): number {
	...
	if (!Number.isFinite(prompt) || !Number.isFinite(completion)) return Number.NaN;
...
function healthMap(rows: StatusRow[] | undefined): Health {
	...
	for (const row of rows ?? []) {
		if (row.is_rollup !== 1 || row.event_type !== "generate.text" || !row.model) continue;
		const current = map.get(row.model);
		if (!current || (row.total_requests ?? 0) > (current.total_requests ?? 0)) {
...
function isDegraded(row: StatusRow | undefined): boolean {
	...
	if (total >= MIN_SAMPLES) return (row.status_2xx ?? 0) / total < MIN_SUCCESS_RATE;
...
function walk(value: unknown, out: { chars: number; media: number }): void {
	...
	else if (record.image_url ?? record.image ?? record.file ?? record.file_id) out.media++;
...
function inspect(input: unknown): { text: string; media: number; tokens: number } {
...
export function chooseModel(options: {
	...
	const pool = (options.catalog ?? []).filter((model) => {
		...
		if (model.community || String(model.id).startsWith("community/")) return false;
		if (options.media > 0 && !(model.input_modalities ?? []).includes("image")) return false;
		if (options.tools && model.tools !== true) return false;
		if (model.context_length && model.context_length < options.tokens) return false;
		return true;
	});

	if (pool.length === 0) {
		const [id] = STATIC_FALLBACK[options.tier];
		return { id, reason: `tier=${options.tier}; catalog unavailable, static fallback`, candidates: [id] };
	}
	...
	if (usable.length === 0) {
		const [id] = STATIC_FALLBACK[options.tier];
		return { id, reason: `tier=${options.tier}; every candidate degraded, static fallback`, candidates: [id] };
	}
	...
	return {
		...
		reason: [
			...
			relaxed ? "bar relaxed: no healthy model above it" : "cheapest healthy, then lowest p95",
...
async function classify(
	...
	try {
		...
		const label = /\b(FAST|BALANCED|DEEP)\b/.exec(outputText(await response.json()));
...
function withTrace(response: Response, trace: Record<string, string>): Response {
...
export default async function agent({
	...
	const tier: Tier = view.media > 0 && label.tier === "FAST" ? "BALANCED" : label.tier;
	...
	for (const id of decision.candidates.slice(0, MAX_ATTEMPTS)) {
		...
		if (response.ok || !/^(429|5\d\d)$/.test(String(response.status))) break;
	}
