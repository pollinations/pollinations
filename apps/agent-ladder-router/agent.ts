...
function isTextModel(m: CatalogModel, endpoint: string): boolean {
    ...
    if (m.supported_endpoints && !m.supported_endpoints.includes(endpoint))
        return false;
    return true;
}
...
function pickLadder(
    ...
    const text = models.filter(
        (m) => isTextModel(m, endpoint) && healthRank(m) < 3,
    );
    const pool =
        text.length > 0 ? text : models.filter((m) => isTextModel(m, endpoint));
    const needLong = bodyTextLength(body) > LONG_INPUT_CHARS;
    const needVision = wantsImage(body);

    let fit = pool;
    if (needVision) {
        const vision = pool.filter((m) =>
...
function reasonFor(pick: CatalogModel, body: AnyBody): string {
    ...
    if (bodyTextLength(body) > LONG_INPUT_CHARS)
        return "cheapest healthy large-context model";
    return "cheapest healthy text model";
}

function prependTraceJson(
    original: unknown,
    endpoint: string,
    trace: string,
): string {
            ...
            for (const item of output) {
                ...
                if (Array.isArray(content)) {
                    for (const part of content) {
                        if (
                            (part as AnyBody)?.type === "output_text" &&
                            typeof (part as AnyBody).text === "string"
                        ) {
...
export default async function agent({
    ...
    if (!catalogRes.ok)
        throw new Error(`Catalog fetch failed (${catalogRes.status})`);
