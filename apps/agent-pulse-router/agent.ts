...
async function pickModel(
    ...
    if (!catalogRes.ok) {
        return {
            model: FALLBACK,
            reason: `catalog ${catalogRes.status}; fallback`,
        };
    }
    ...
    if (pool.length === 0) {
        return {
            model: FALLBACK,
            reason: `${band}: no healthy pool; fallback`,
        };
    }

    let chosen = pool[0];
    if (!chosen) {
        return { model: FALLBACK, reason: `${band}: empty pool; fallback` };
    }
    if (band === "HARD") {
        // Skip the cheapest third — pick a mid/high tier healthy model by cost.
        const idx = Math.min(
            pool.length - 1,
            Math.max(1, Math.floor(pool.length * 0.55)),
        );
        chosen = pool[idx] ?? chosen;
    } else if (band === "NORMAL") {
        const idx = Math.min(
            pool.length - 1,
            Math.max(0, Math.floor(pool.length * 0.2)),
        );
        chosen = pool[idx] ?? chosen;
    }
