    ...
    return (
        <div className="w-full aspect-video bg-[#2c1e12] rounded-lg flex items-center justify-center border border-dashed border-[#5a4332]">
            <span className="text-[#6a5a48] text-xs text-center px-4">
                Save your game to<br />upload this image
            </span>
        </div>
    );
}

export function GalleryModal({ isOpen, onClose, character, storyHistory, inventory }: GalleryModalProps) {
    ...
    const hasAnything = hasAvatar || uploadedScenes.length > 0 || uploadedItems.length > 0;
        ...
        <>
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent className="bg-[#3a2817] border-4 border-[#d4a76a] text-[#f5e6d3] max-w-4xl max-h-[85vh]">
                    <DialogHeader>
                        <DialogTitle className="text-[#d4a76a]" style={{ fontFamily: "Cinzel, serif" }}>
                            Adventure Gallery
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <ScrollArea className="h-full max-h-[70vh] pr-4">
                        {!hasAnything ? (
                            <div className="text-center py-16 text-[#b8a389]">
                                <p className="text-lg mb-2">No media saved yet</p>
                                <p className="text-sm">Save your game with "Upload & Save" to persist images here.</p>
                            </div>
                        ) : (
                            <div className="space-y-8 pb-4">
                                ...
                                {hasAvatar && character && (
                                    <section>
                                        <h3 className="text-[#d4a76a] text-sm font-semibold uppercase tracking-wider mb-3"
                                        ...
                                        <div className="flex items-start gap-4">
                                            <img
                                                ...
                                                onClick={() => setLightboxUrl(character.avatar)}
                                            />
                                            <div>
                                                <p className="text-[#f5e6d3] font-semibold">{character.name}</p>
                                                <p className="text-[#b8a389] text-sm">{character.class}</p>
                                            </div>
                                        </div>
                                    </section>
                                )}
                                ...
                                {uploadedScenes.length > 0 && (
                                    <section>
                                        <h3 className="text-[#d4a76a] text-sm font-semibold uppercase tracking-wider mb-3"
                                            style={{ fontFamily: "Cinzel, serif" }}>
                                            Adventure Scenes ({uploadedScenes.length})
                                        </h3>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {uploadedScenes.map(entry => (
                                                <div key={entry.id} className="group">
                                                    <img
                                                        src={entry.image}
                                                        alt={entry.characterChoice || "Scene"}
                                                        ...
                                                        onClick={() => setLightboxUrl(entry.image)}
                                                    />
                                ...
                                {storyHistory.some(e => !isUploaded(e.image) && e.image) && (
                                    <section>
                                        <h3 className="text-[#6a5a48] text-xs font-semibold uppercase tracking-wider mb-2">
                                            Unsaved Scenes ({storyHistory.filter(e => !isUploaded(e.image) && e.image).length})
                                        </h3>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            {storyHistory.filter(e => !isUploaded(e.image) && e.image).map(entry => (
                                                <Placeholder key={entry.id} />
                                            ))}
                                        </div>
                                    </section>
                                )}
                                ...
                                {uploadedItems.length > 0 && (
                                    <section>
                                        <h3 className="text-[#d4a76a] text-sm font-semibold uppercase tracking-wider mb-3"
                                            style={{ fontFamily: "Cinzel, serif" }}>
                                            Relics & Items ({uploadedItems.length})
                                        </h3>
                                            ...
                                            {uploadedItems.map(item => (
                                                <div key={item.id} className="text-center">
                                                    <img
                                                        ...
                                                        onClick={() => setLightboxUrl(item.image)}
                                                    />
                                                    <p className="text-xs mt-1 truncate text-[#f5e6d3]">{item.name}</p>
                                                    <span
                                                        className="text-[10px] uppercase tracking-wider"
                                                        style={{ color: RARITY_COLORS[item.rarity] || "#b8a389" }}
                                                    >
                                                        {item.rarity}
                                                    </span>
                                                </div>
