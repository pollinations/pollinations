...
import type { ApiMessage, GenerateTextWithHistory } from "./types";
 ...
 */
export const createGenerateTextWithHistory = (
    baseUrl: string,
  ...
  -d '${JSON.stringify(requestData, null, 2)}'`;
        ...
        try {
            ...
            if (error.message?.includes("Request timed out")) {
                log(
                    "Request timed out after %dms, waiting 60s before next request",
                    API_TIMEOUT_MS,
                );
