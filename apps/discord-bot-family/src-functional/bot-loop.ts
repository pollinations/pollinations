...
async function generateResponseWithHistory(
    ...
    if (channel && "messages" in channel) {
        ...
        if (history) {
            ...
            // No history available — skip if no initial prompt
        ...
        // No channel — skip if no initial prompt
        if (!initialPrompt) return null;
        apiMessages = [{ role: "user", content: initialPrompt }];
...
export async function runBot(
    ...
    // Proactive posting: only in global/shared channels, not bot-specific channels
    if (config.globalChannelIds && config.globalChannelIds.length > 0) {
        setInterval(async () => {
            try {
                // Pick a random global channel (not bot-specific ones)
                ...
                // Skip if the last message was from this bot (prevents self-reply loops)
                const lastMsg = recent.first();
                if (lastMsg && lastMsg.author.id === client.user?.id) {
                    log("Skipping proactive post in %s — last message was from this bot", channelId);
                    return;
                }
