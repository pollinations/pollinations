...
async function main() {
    try {
        ...
        const generateText = createGenerateTextWithHistory(
            pollinationsConfig.baseUrl,
        );
        ...
        if (
            config.conversationChannelIds &&
            config.conversationChannelIds.length > 0
        ) {
            console.log(
                `💬 Conversation channels: ${config.conversationChannelIds.join(", ")}`,
            );
        } else {
            console.log(
                `💬 Conversation channels: DISABLED (bot will only respond to mentions and DMs)`,
            );
        }
