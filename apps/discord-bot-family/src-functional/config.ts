...
export const loadBotConfigs = (): BotConfig[] => {
    ...
    const globalConversationChannels =
        process.env.CONVERSATION_CHANNELS?.split(",") || [];
    ...
    for (let i = 1; ; i++) {
        ...
        if (!token) {
            log(
                `Warning: Bot ${i} has model '${botModel}' but no token defined. Skipping.`,
            );
        ...
        if (seenTokens.has(token)) {
            log(
                `Warning: Duplicate token found for Bot ${i}. Please ensure each bot has a unique token.`,
            );
        }
        ...
        const personality =
            process.env[`BOT_PERSONALITY_${i}`] || "A helpful AI assistant";
        ...
        const botSpecificChannels =
            process.env[`BOT_CONVERSATION_CHANNELS_${i}`];

        if (botSpecificChannels) {
            conversationChannelIds = botSpecificChannels
                .split(",")
                .map((id) => id.trim())
                .filter(Boolean);
            log(
                `Using bot-specific conversation channels for Bot ${i}: ${conversationChannelIds}`,
            );
        } else {
            log(
                `Using global conversation channels for Bot ${i}: ${conversationChannelIds}`,
            );
        }
    ...
    if (configs.length === 0) {
        log(
            "Warning: No bot configurations found. Check your environment variables.",
        );
    }
