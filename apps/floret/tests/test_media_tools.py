"""Unit tests for URL/data media hosting and generation tools, no network."""
...
@pytest.mark.parametrize(
    "source", ["frame.png", "/workspace/frame.png", "../outside.txt"]
)
async def test_upload_rejects_local_paths(source):
    with pytest.raises(ValueError, match="assets publish"):
        await media.upload_media(source)
...
async def test_toolset_dispatches_media_upload(monkeypatch):
    ...
    up = await toolset.dispatch(
        "upload_media", {"source": "https://example.test/frame.png"}
    )
...
async def test_uploaded_video_and_audio_become_artifacts(monkeypatch):
    ...
    async def fake_upload(source, filename=None):
        return f"https://media.pollinations.ai/{source.rsplit('/', 1)[-1]}"

    monkeypatch.setattr(toolset.media, "upload_media", fake_upload)

    vid = await toolset.dispatch(
        "upload_media", {"source": "https://example.test/final.mp4"}
    )
    assert [a["type"] for a in vid.artifacts] == ["video"]

    aud = await toolset.dispatch(
        "upload_media", {"source": "https://example.test/mix.mp3"}
    )
    assert [a["type"] for a in aud.artifacts] == ["audio"]

    # Frame images are intermediates — do not spam the reply with them.
    img = await toolset.dispatch(
        "upload_media", {"source": "https://example.test/last1.jpg"}
    )
