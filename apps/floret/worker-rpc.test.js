...
test("shell bridge uses supported fetch transport and destroys each invocation", async () => {
    ...
    const runtime = new Miniflare({
        ...
        modules: [
            ...
            },
            {
                ...
                contents: await readFile(new URL("shell-bridge.js", root), "utf8"),
            },
        ],
    ...
    try {
        for (const body of ["first", "second"]) {
            const response = await runtime.dispatchFetch("http://shell.internal/run", {
                method: "POST",
                headers: {
        ...
        const status = await runtime.dispatchFetch("http://shell.internal/status");
