...
export default function App() {
      ...
      try {
        ...
        if (tinybirdWAU) {
          for (const row of tinybirdWAU) {
            const existing = weekMap.get(row.week) || { week: row.week };
  ...
  const funnelData = currentWeek
    ? [
        { stage: "Signups", count: currentWeek.registrations || 0, rate: 100 },
        {
          stage: "Activated",
          count: currentWeek.activations || 0,
          rate: currentWeek.registrations
            ? (
                (currentWeek.activations / currentWeek.registrations) *
                100
              ).toFixed(0)
            : 0,
        },
        { stage: "Active (WAU)", count: currentWeek.wau || 0, rate: 100 },
        {
          ...
          rate: currentWeek.wau
            ? ((currentWeek.packPurchases / currentWeek.wau) * 100).toFixed(1)
            : 0,
        },
      ]
    : [];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <img src="/logo.svg" alt="pollinations.ai" className="h-8 w-8" />
              <h1 className="text-2xl font-semibold tracking-tight">
                pollinations.ai{" "}
                <span className="text-gray-500 font-normal">by Myceli.AI</span>
              </h1>
            </div>
            <p className="text-gray-500 text-sm">Weekly KPI Dashboard</p>
          </div>
          <button
            type="button"
        ...
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">
                ...
                <span className="text-gray-600">
                  · Last full week ({currentWeek?.week?.slice(5)})
                </span>
              </p>
            ...
            <div className="text-right">
              <span className="text-4xl font-semibold">{wapc}</span>
              <span
                className={`block text-sm mt-1 ${
                  wapcChange >= 0 ? "text-emerald-500" : "text-red-500"
                }`}
              >
                {wapcChange >= 0 ? "↑" : "↓"}{" "}
                {wapcChange != null ? Math.abs(wapcChange).toFixed(1) : 0}% WoW
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
          ...
          <StatCard
            ...
            change={calcChange(currentWeek?.tokens, previousWeek?.tokens)}
            icon={TrendingUp}
            format="compact"
          ...
          <StatCard
            ...
            change={calcChange(currentWeek?.revenue, previousWeek?.revenue)}
            icon={DollarSign}
            format="currency"
        ...
        <div className="mt-12 pt-8 border-t border-gray-800">
          <div className="flex items-center justify-between text-gray-500 text-xs">
            <div className="flex items-center gap-2">
              <img src="/myceli-logo.svg" alt="" className="h-4 w-4" />
              <span>Myceli.AI</span>
            </div>
