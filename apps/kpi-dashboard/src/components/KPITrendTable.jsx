...
export function KPITrendTable({ weeklyData, title }) {
  ...
  const getTrendIcon = (change) => {
    if (change > 5) return <TrendingUp className="w-3 h-3 text-green-400" />;
  ...
  return (
    <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-5 overflow-hidden">
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-gray-400 border-b border-gray-700">
              ...
              <th className="text-right py-2 px-2 min-w-16">WoW</th>
          ...
          <tbody>
            {kpis.map((kpi) => {
              ...
              const nowVal = currentWeek ? getValue(kpi, currentWeek) : null;
              // WoW: compare last two FULL weeks
              const lastVal = lastFullWeek ? getValue(kpi, lastFullWeek) : 0;
              const prevVal = prevFullWeek ? getValue(kpi, prevFullWeek) : 0;
              ...
              return (
                ...
                >
                  ...
                  {displayWeeks.map((week) => (
                    ...
                    >
                      {formatValue(getValue(kpi, week), kpi.format)}
                    </td>
                  ))}
                </tr>
              );
      ...
      <div className="mt-3 text-xs text-gray-500">
        <span className="text-blue-400">Now</span> = current partial week •{" "}
        <span className="text-green-400">WoW</span> = comparing last 2 full
        weeks • {displayWeeks.length} full weeks shown
      </div>
    </div>
  );
}
