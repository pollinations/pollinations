...
export function useGameEngine({
    ...
    // Presets lock once a piece is on the board OR the game has started
    // (the first seed is already generating from the chosen preset).
    const presetsLocked = hasStarted || pieces.length > 0;
