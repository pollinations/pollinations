...
import { z } from "zod";
import type { EmailConfig, ServiceConfig } from "../types/index.js";
...
const EnvSchema = z.object({
    ...
    EMAIL_FROM_ADDRESS: z.string().default("sender@example.com"),
...
function createEmailConfig(): EmailConfig {
    ...
    if (env.EMAIL_PROVIDER === "brevo") {
        if (!env.BREVO_KEY || !env.BREVO_MAIL) {
            throw new Error(
                "Brevo configuration is incomplete. Please provide BREVO_KEY and BREVO_MAIL",
            );
        }
    ...
    if (env.EMAIL_PROVIDER === "resend") {
        if (!env.RESEND_API_KEY) {
            throw new Error(
                "Resend configuration is incomplete. Please provide RESEND_API_KEY",
            );
        }
