...
import type { EmailConfig, EmailMessage, EmailServiceResponse } from './types/index.js';
...
export interface MicroServiceWrapper {
  ...
  sendWelcomeEmail(to: string | string[], userName: string, serviceName?: string): Promise<EmailServiceResponse>;
  sendPasswordResetEmail(to: string | string[], userName: string, resetLink: string, serviceName?: string, expiryTime?: string): Promise<EmailServiceResponse>;
  sendNotificationEmail(to: string | string[], title: string, message: string, serviceName?: string): Promise<EmailServiceResponse>;
  verifyConnection(): Promise<boolean>;
  getTemplates(): Record<string, any>;
}

export function createMicroServiceWrapper(emailConfig: EmailConfig): MicroServiceWrapper {
  ...
  return {
    ...
    async sendPasswordResetEmail(
      ...
      return microService.sendPasswordResetEmail(to, userName, resetLink, serviceName, expiryTime);
    },
    ...
    async sendNotificationEmail(
      ...
      return microService.sendNotificationEmail(to, title, message, serviceName);
    },
...
export type { EmailConfig, EmailMessage, EmailServiceResponse } from './types/index.js';
export { EmailMessageSchema, EmailConfigSchema } from './types/index.js';
