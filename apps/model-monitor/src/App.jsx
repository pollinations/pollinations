...
function getLastErrorColor(timestamp) {
  if (!timestamp || timestamp === "1970-01-01 00:00:00") return "text-gray-300";
...
function TrendIndicator({ trend }) {
  ...
  return (
    <div className="flex gap-1 text-[11px] justify-center">
      <span
        className={p95Color}
        title={`P95: ${p95Change > 0 ? "+" : ""}${p95Change.toFixed(0)}%`}
      >
        {p95Arrow}
      ...
      <span
        ...
        title={`5xx: ${err5xxChange > 0 ? "+" : ""}${err5xxChange.toFixed(1)}%`}
      >
        {err5xxArrow}
        ...
        <div className="text-[10px] text-gray-500 mt-1">
          ...
          {(stats.countTurbulent > 0 || stats.countOff > 0) && (
            <span className="ml-1">
              (
              {stats.countOff > 0 && (
                <span className="text-red-600">{stats.countOff} off</span>
              )}
              {stats.countOff > 0 && stats.countTurbulent > 0 && ", "}
...
function App() {
  ...
  const sortedModels = [...models].sort((a, b) => {
    ...
    switch (sort.key) {
      ...
      case "errors":
        return (
          dir * ((a.stats?.total_errors || 0) - (b.stats?.total_errors || 0))
        );
      case "lastError": {
      ...
      case "p50":
        return (
          dir *
          ((a.stats?.latency_p50_ms || 0) - (b.stats?.latency_p50_ms || 0))
        );
      case "p95":
        return (
          dir *
          ((a.stats?.latency_p95_ms || 0) - (b.stats?.latency_p95_ms || 0))
        );
      default:
        return 0;
    }
    ...
    <div className="min-h-screen p-4 md:p-6 bg-gray-50">
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
              <span>{isLiveMode ? "5-minute" : "60-minute"} window</span>
              <span className="flex items-center gap-1">
                ...
                />
                image: {sortedModels.filter((m) => m.type === "image").length}
              </span>
              <span className="flex items-center gap-1">
                <span
                  className={`inline-block w-2 h-2 rounded-full ${
                ...
                />
                text: {sortedModels.filter((m) => m.type === "text").length}
              </span>
            ...
            <thead className="bg-gray-50 text-[10px] text-gray-500 uppercase tracking-wide">
              <tr>
                ...
                <th className="px-3 py-2 font-medium text-center">
                  <span className="text-[9px]">P95 | 5xx</span>
                </th>
                <th className="px-3 py-2 font-medium text-center">P95 Chart</th>
              </tr>
            </thead>
              ...
              {sortedModels.length === 0 ? (
                <tr>
                  <td colSpan={11} className="p-8 text-center text-gray-400">
                    {lastUpdated ? "No models found" : "Loading models..."}
                  </td>
                </tr>
              ) : (
                sortedModels.map((model) => {
                  ...
                  const adjustedTotal = total - excluded401_403;
                  const share =
                    totalAllRequests > 0 ? (total / totalAllRequests) * 100 : 0;
                  const ok2xx = stats?.status_2xx || 0;
                  const lastErrorAt = stats?.last_error_at;
                  const lastErrorAgo = formatTimeAgo(lastErrorAt);
                  ...
                  const tailRatio = p50Sec && p95Sec ? p95Sec / p50Sec : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-gray-600">
                        {share > 0
                          ? (share < 1 ? share.toFixed(1) : Math.round(share)) +
                            "%"
                          : "—"}
                      </td>
                      <td
                      ...
                      >
                        {formatPercent(ok2xx, adjustedTotal, true)}
                      </td>
                      {/* Errors breakdown */}
                      ...
                      <td
                        className={`px-3 py-2 text-right tabular-nums ${
                          p50Sec ? getLatencyColor(p50Sec) : "text-gray-300"
                        }`}
                      >
                        {p50Sec ? `${p50Sec.toFixed(1)}s` : "—"}
                      </td>
                      ...
                      {/* P95 Sparkline */}
                      <td className="px-3 py-2 text-center">
                        <Sparkline data={model.sparkline} color="blue" />
                      </td>
                    </tr>
                  );
                })
        ...
        <div className="text-[10px] text-gray-500 bg-white border border-gray-200 rounded-lg p-3 space-y-2">
          <div className="font-medium text-gray-700">Health Status Rules</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div>
              ...
              <span>
                5xx ≥ 5% • 429 ≥ 15% • P95 &gt; 10s (text) / 1min (image)
              </span>
            </div>
          </div>
          <div className="text-gray-400 italic">
            Note: 401 (no auth) and 403 (no pollen) errors are excluded from
            uptime calculations as they are user errors, not model failures.
          </div>
        </div>
      </div>
    </div>
