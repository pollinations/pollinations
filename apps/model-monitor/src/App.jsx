...
// Error badge component - shows 5xx errors prominently, 4xx de-emphasized
function ErrorBadges({ stats }) {
    // 5xx server errors - these are actionable (our responsibility)
    const serverErrors = [
        {
            code: "500",
    ...
    // 4xx user errors - de-emphasized (not our responsibility)
    const userErrors = [
        { code: "400", count: stats?.errors_400 },
        { code: "401", count: stats?.errors_401 },
        { code: "403", count: stats?.errors_403 },
        { code: "429", count: stats?.errors_429 },
    ].filter((e) => e.count > 0);

    const totalUserErrors = userErrors.reduce((sum, e) => sum + e.count, 0);

    if (serverErrors.length === 0 && userErrors.length === 0) {
        return <span className="text-gray-300">—</span>;
    }

    return (
        <div className="flex flex-wrap gap-1 items-center">
            {/* 5xx errors - prominent */}
            {serverErrors.map(({ code, count, bg, text }) => (
                <span
                    key={code}
                    className="inline-flex items-stretch rounded overflow-hidden border border-red-300"
                    title={`${code}: ${count} server errors`}
                >
                    <span
...
// Excludes all 4xx (user errors) - only 5xx errors affect health
function computeHealthStatus(stats, modelType = "text") {
    ...
    // OFF: pct_5xx >= 20%, pct_504 >= 5%, or no successful requests
    if (pct5xx >= 20) return "off";
    if (pct504 >= 5) return "off";
    if (count2xx === 0 && adjustedTotal > 0) return "off";

    // TURBULENT: pct_5xx 5-20%, P95 > threshold
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {sortedModels.length === 0 ? (
                                ...
                                sortedModels.map((model) => {
                                    ...
                                    const total = stats?.total_requests || 0;
                                    // Exclude all 4xx user errors from success rate
                                    const userErrors =
                                        (stats?.errors_400 || 0) +
                                        (stats?.errors_401 || 0) +
                                        (stats?.errors_403 || 0) +
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td
                                                ...
                                                title={
                                                    userErrors > 0
                                                        ? `Excluding ${userErrors} user errors (4xx) from success rate`
                                                        : undefined
                                                }
                                            >
                                                {formatPercent(
