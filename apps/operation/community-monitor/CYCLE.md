...
2b. **Billing sanity check — informational only, never triggers deactivation.** `probe-results.json`'s `billingFlagsByModel` lists per-model anomalies from this cycle's cache-busted probes: zero or implausible uncached prompt usage, cached tokens on a unique single-message prompt, detail counts exceeding their parent counts, totals that do not add up, empty completions despite a 200, or a missing usage object. The OpenAI-compatible contract is simple: `prompt_tokens` includes cached prompt tokens, `completion_tokens` includes reasoning tokens, and `total_tokens = prompt_tokens + completion_tokens`; cached/reasoning details are subsets and are never added again. Cached tokens can represent provider-added context that Pollinations did not send, but they must still be included in `prompt_tokens`. If a provider cannot determine cache usage reliably, zero is safer than inventing it. These flags never feed duty 2's UNSTABLE/deactivation logic.
