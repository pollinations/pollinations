...
1. `/home/ubuntu/monitor/probe-results.json` — a fresh probe of every listed community model. **Nothing runs this for you** — as the very first action of every cycle, run it yourself: `cd /home/ubuntu/monitor && node probe.mjs`. It takes ~15-30s and overwrites this file. If it errors (e.g. a missing env var), that's worth a one-line note in your final summary, but don't let it block the rest of the cycle — fall back to Tinybird-only data (rule b below) same as if the file were merely stale.
   - Requests are cost-weighted, not uniform: models get 3–15 probe requests this cycle, capped at 15 per model, so cumulative spend stays modest (aiming under ~0.5 pollen/cycle, adaptive to last cycle's actual spend — see `state.json`'s `spend` key). Every request contains a unique marker to prevent providers caching or special-casing the public probe prompt. This is all handled inside probe.mjs.
...
2b. **Billing sanity check — informational only, never triggers deactivation.** `probe-results.json`'s `billingFlagsByModel` lists per-model anomalies from this cycle's cache-busted probes: zero or implausible uncached prompt usage, cached tokens on a unique single-message prompt, cached tokens exceeding total prompt tokens, empty completions despite a 200, or a missing usage object. `prompt_tokens` normally includes `prompt_tokens_details.cached_tokens`, so never call the full `prompt_tokens` value implausible without subtracting that cached subset first. Cached tokens can represent provider-added context that Pollinations did not send; report that distinction rather than calling them user-prompt tokens. These flags never feed duty 2's UNSTABLE/deactivation logic.
    ...
    - Flag names are stable categories; when posting, inspect that model's current raw `results` entries and quote the actual `prompt_tokens`/`cached_tokens` values rather than inventing or reusing an old number.
