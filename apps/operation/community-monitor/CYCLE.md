...
2. Tinybird health, last 4 hours:
   `curl -s "https://api.europe-west2.gcp.tinybird.co/v0/pipes/model_health.json?token=$TB_TOKEN&minutes=240"`. Fetch `https://gen.pollinations.ai/models` too and classify a model as community only when its catalog entry has `community: true`; canonical first-party IDs also contain `/`, so the name shape is not a valid classifier. Judge failures by `errors_5xx` only; 4xx never counts against a model.
...
2c. **Official (non-community) model monitoring — alert only, never deactivate.** Community models (duty 2) get an 80% threshold, a probe, a multi-cycle grace period, and automatic deactivation, because they're third-party endpoints outside our control. Official models are catalog entries without `community: true` (for example `openai/gpt-5.4`, `google/gemini-3-flash-preview`, and `anthropic/claude-sonnet-4.6`). They're first-party, so the bar is stricter and the response is lighter — you have no authority to touch them, only to raise the alarm fast.
    ...
    - This duty is independent of duty 2/2b's flagged/degradedStreak/billingFlagged state — the live catalog's `community` flag makes the community and official sets mutually exclusive, so there's no overlap to reconcile.
