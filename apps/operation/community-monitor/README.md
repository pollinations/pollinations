...
- `watchdog.sh` — legacy `screen` fallback, retained for manual recovery only.
- `nudge.sh` — legacy prompt-box recovery for the retired long-lived screen
  design. It is not installed in cron with the systemd deployment.
...
`.env.example`), install `community-monitor.service`, then run
