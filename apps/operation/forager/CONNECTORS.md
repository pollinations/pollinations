...
| **aws** | Umbrella Cost (Automat-it reseller meter): net-unblended cost + discount per usage month, both accounts (813596885972 Bedrock workloads + 202731947268 infra refactor) summed; Apr 2026 cash, May+ credit (payer credits eat invoices) | `POST api.umbrellacost.io/api/v1/authentication/token/generate` → `GET /api/v1/users` (accountKeys) → `GET /api/v2/invoices/cost-and-usage` (raw `authorization` header + `apikey: userkey:accountKey:`, tokens ~30 min) | `UMBRELLA_USERNAME`, `UMBRELLA_PASSWORD` | paid + credit | 2026-07-07 |
