...
| **google** | BQ billing export: gross_eur (prepaid) + abs(credits_eur) (credit), EUR×fx→USD | `gcloud auth activate-service-account --key-file={sa_json}` + `bq query … FROM billing_export.gcp_billing_export_resource_v1_{BA}` | `GCP_BILLING_SA_JSON` (SA JSON string in SOPS) | prepaid + credit | 2026-07-03 |
...
python3 -m ingest.record meter <provider> <YYYY-MM> <amount> --funding credit
python3 -m ingest.record meter <provider> <YYYY-MM> <amount> --funding prepaid
```
