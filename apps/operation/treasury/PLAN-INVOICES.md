...
- **The Tinybird project is SHARED, not treasury's:** all datasource/pipe definitions for the `operations` workspace live in `apps/operation/tinybird/` (a sibling of the operation apps). Treasury — and every future operation app — only consumes its pipes; datasource creation happens ONLY there.
...
- [ ] **Step 2: Parsers**
  - `automat_it.py` — the verified parser from `$SA/build/connectors/accrual.py:68-88`, regexes character-for-character (`BILLING PERIOD: \d{2}/(\d{2})/(\d{4})`, `Tax Invoice-\s*(\S+)`, `INVOICE TOTAL\s+EUR\s*([\d,]+\.?\d*)`, the Credits-Used layout regex, `remaining credits are\s*([\d,]+)\s*EUR`). Emits the cash total as the row (`kind="reseller"`, currency EUR); credit-used + credits-left land in the row's `note` (`"credits_eur=…; credits_left_eur=…"`) so nothing is lost before the meter/grants phases exist.
