...
> **Burn phase (B-tasks) — EXECUTING as of 2026-07-03.** The balance connectors, meter connectors, burn engine, and `provider_month` / `grants` tables are implemented in `apps/operation/forager/`. The full burn-phase plan (Tasks B1–B10, row shapes, architecture) is [`PLAN-BURN.md`](PLAN-BURN.md) in this directory. This PLAN.md governs the invoice phase and the treasury frontend; the burn phase feeds the same `operations` Tinybird workspace.
