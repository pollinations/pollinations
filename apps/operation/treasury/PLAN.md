...
> **Burn phase (B-tasks) — EXECUTING as of 2026-07-03.** The balance connectors, meter connectors, burn engine, and `provider_month` / `grants` tables described in Tasks B3–B10 are being implemented in `apps/operation/forager/`. The full burn-phase plan (tasks, row shapes, architecture) lives in [`apps/operation/forager/.superpowers/sdd/task-B9-brief.md`](../forager/.superpowers/sdd/task-B9-brief.md) (the PLAN-BURN document). This PLAN.md governs the invoice phase (Tasks 1–12) and the treasury frontend; the burn phase is a parallel track that feeds the same `operations` Tinybird workspace.
