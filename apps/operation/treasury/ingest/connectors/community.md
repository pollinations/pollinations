...
- Tinybird read token for the Treasury/operations workspace.

Live validation:

- Read-only `op_pollen` query tested on 2026-07-10.
- Treasury `op_pollen` returned community rows for June and July 2026.
- The old Forager connector path against `pollen_monthly` returned 403/resource-not-found in the current Treasury workspace, so use `op_pollen` here.
...
- The legacy Forager connector reads `pollen_monthly`; Treasury uses `op_pollen`.
