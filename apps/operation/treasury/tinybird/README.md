...
Treasury owns the OP Tinybird read surface.
...
- An `opening_balance` is the cash position at the start of its calendar month;
  Treasury uses it for both that month's Current Wise cash position and its
...
Treasury converts each fact to USD using the same monthly FX table as the other
financial insights.
