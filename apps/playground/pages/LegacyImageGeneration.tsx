...
export default function LegacyImageGeneration() {
  ...
  const onSubmit = async (data: ImageGenerationFormValues) => {
    ...
    try {
      ...
      const response = await fetch('/api/legacy-image-generation', {
        method: 'POST',
        headers: {
      ...
      leftPane={
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <PageHeader
              title="Legacy Image Generation"
              subtitle="Generate images using Pollinations AI models with legacy URLs."
            />
