	...
	.action(async (opts) => {
		...
		if (opts.token) {
			...
			try {
				...
				const msg = "Key stored but could not verify. It may still be valid.";
		...
		const isHuman = getOutputMode() === "human";
		const spinner = isHuman ? ora("Requesting device code...").start() : null;
		...
		try {
			const res = await fetch(`${ENTER_URL}/api/device/code`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ client_id: "polli-cli", scope: "generate" }),
			});
			if (!res.ok) {
		...
		if (opts.browser !== false) {
			try {
				...
				await open(url);
				printInfo("Browser opened. Sign in with GitHub and approve the code.");
			} catch {
		...
		saveCredentials({ apiKey: key, keyType: keyType as "pk" | "sk" });
		pollSpinner?.succeed("Authenticated!") ?? printSuccess("Authenticated!");
