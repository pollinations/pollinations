		...
		if (!isJson) {
			process.stderr.write(
				chalk.green(`\nChat session started (model: ${opts.model})\n`) +
					chalk.dim("Type /exit to quit, /clear to reset, /save <path> to save\n\n"),
			);
		}
		...
		const sendMessage = async (userMsg: string) => {
			...
			try {
				const res = await fetch(`${BASE_URL}/v1/chat/completions`, {
					...
					model: opts.model,
					messages,
					...(opts.temperature && { temperature: Number(opts.temperature) }),
				...
				if (isJson) {
					printResult({ role: "assistant", content, model: data.model, tokens: data.usage?.total_tokens });
				} else {
					process.stderr.write(`${chalk.yellow("ai")} > ${content}\n\n`);
				}
			} catch (err) {
