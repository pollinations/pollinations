    ...
    .action(async () => {
        ...
        try {
            const [profile, tiers] = await Promise.all([
                enter<Record<string, unknown>>("/api/account/profile", {
                    apiKey: key,
                }),
            ...
            const nextTier =
                currentIdx >= 0 && currentIdx < tierList.length - 1
                    ? tierList[currentIdx + 1]
                    : null;

            printResult({
                current_tier:
                    `${tierList[currentIdx]?.emoji ?? ""} ${currentTier}`.trim(),
                pollen_rate:
                    currentIdx >= 0
                        ? formatPollen(
                              tierList[currentIdx].pollen,
                              tierList[currentIdx].cadence,
                          )
                        : "unknown",
                refill: tierList[currentIdx]?.cadence ?? "unknown",
                next_reset: profile.nextResetAt ?? "unknown",
                next_tier: nextTier
                    ? `${nextTier.emoji} ${nextTier.tier}`
                    : "max tier",
            });
        } catch (err) {
