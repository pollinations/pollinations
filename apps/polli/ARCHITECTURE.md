...
flowchart TB
    subgraph ENTRY["**ENTRY POINTS**"]
        ...
        HTTP_API["HTTP API<br/>polli_api.py"]
    end

    subgraph DISCORD_LAYER["**DISCORD LAYER** - src/bot.py"]
        direction TB
        POLLIBOT["PolliBot<br/>(commands.Bot)"]
    ...
    class POLLIBOT,EVENTS,MSG_FLOW,ADMIN,TASKS discord
...
- **HTTP API** - OpenAI-compatible REST API (polli_api.py)

### Core Components

#### Discord Layer (src/bot.py)
- `PolliBot` extends `commands.Bot`
