...
async def on_message(message: discord.Message):
    ...
    if is_thread:
        ...
        if not session and is_reply_to_bot and not is_mentioned:
            await handle_inline_polli_mention(message)
            return
    ...
    logger.info("PATH: CREATING THREAD (line 1020) - This should ONLY happen for @polli!")
...
async def handle_inline_polli_mention(message: discord.Message):
    """
    Handle casual "polli" mention - inline, focused, human-like reply.

    Key differences from @polli:
    ...
    if not text and (image_urls or video_urls or file_urls):
        text = "[User mentioned polli with media/files - respond briefly and naturally]"

    # Fetch recent channel history for CONTEXT (last 20 messages - lighter than @polli)
    channel_history = []
    try:
    ...
    async with message.channel.typing():
        try:
            ...
            inline_system_prompt = {
                "role": "system",
                "content": (
                    "You are Polli responding inline. Be EXTREMELY SHORT. "
                    ...
                    "For anything complex, say '@polli for details'."
                ),
            }
            ...
            logger.error(f"Error processing inline polli mention: {e}")
        ...
        if not response_text:
            ...
            retry_result = await pollinations_client._call_api_with_tools(
                messages=[
                    {
                        "role": "system",
                        "content": "You are Polli. The user sent a message but you didn't respond. Generate a helpful response - ask clarifying questions if you're unsure what they want, or summarize what you found if you used tools.",
                    },
