...
# Try to use Rust-accelerated polli_core, fall back to Python
try:
    import polli_core

    _use_rust = True
    logger.info("polli_core (Rust) loaded — using accelerated chunking, hashing, file collection")
except ImportError:
    _use_rust = False
    logger.info("polli_core not available — using Python fallback")
...
def _chunk_code(content: str, file_path: str, max_lines: int = 100) -> list[dict]:
    if _use_rust:
        rust_chunks = polli_core.chunk_code(content, file_path, max_tokens=MAX_TOKENS_PER_INPUT, max_lines=max_lines)
        return [
            {
                "content": c.content,
...
def _file_hash(content: str) -> str:
    if _use_rust:
        return polli_core.hash_xxh3(content)
...
def _collect_code_files(repo_path: Path) -> list[Path]:
    if _use_rust:
        paths = polli_core.collect_files(
            str(repo_path),
        ...
        try:
            ...
            while batch_start < len(all_documents):
                ...
                while i < len(all_documents):
                    doc_tokens = (
                        polli_core.count_tokens(all_documents[i]) if _use_rust else len(_enc.encode(all_documents[i]))
                    )
...
async def initialize():
    ...
    try:
        ...
        if has_existing and current_head and current_head == last_head:
            logger.info(
                f"✅ Embeddings up to date (HEAD {current_head[:8]}, {collection.count()} chunks) — skipping re-embed"
            )
            _initialized.set()
            return
