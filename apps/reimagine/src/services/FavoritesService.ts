...
import type { LexicaImage, LexicaPrompt } from "../types/api";
import type { CivitaiPrompt, ExtendedLexicaPrompt } from "../types/civitai";
...
export class FavoritesService {
    ...
    static async addToFavorites(
        prompt: LexicaPrompt | CivitaiPrompt,
        selectedImage: LexicaImage,
    ): Promise<void> {
        try {
            ...
            const existingIndex = favorites.findIndex(
                (fav) =>
                    fav.prompt.id === prompt.id &&
                    fav.selectedImageId === selectedImage.id,
            );

            if (existingIndex === -1) {
                // Determine if this is a Civitai image
                const isCivitai =
                    "civitaiImageUrl" in prompt || "stats" in prompt;
                ...
                const extendedPrompt: ExtendedLexicaPrompt = isCivitai
                    ? {
                          ...
                          civitaiImageUrl: (prompt as CivitaiPrompt)
                              .civitaiImageUrl,
                ...
                const civitaiUrl = isCivitai
                    ? (prompt as CivitaiPrompt).civitaiImageUrl
                    : null;
                const civitaiStats = isCivitai
                    ? (prompt as CivitaiPrompt).stats
                    : null;
                const username = isCivitai
                    ? (prompt as CivitaiPrompt).username
                    : null;

                console.log("Storing Civitai data:", {
                    civitaiUrl,
                    civitaiStats,
                    username,
                });
                ...
                console.log(
                    "Final favorite item:",
                    JSON.stringify(newFavorite, null, 2),
                );
    ...
    static async removeFromFavorites(
        ...
        try {
            ...
            const updatedFavorites = favorites.filter(
                (fav) =>
                    !(
                        fav.prompt.id === promptId &&
                        fav.selectedImageId === imageId
                    ),
            );
     ...
     */
    static async getFavoritesBySource(
        source: "lexica" | "civitai",
