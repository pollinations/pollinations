...
Where [`polli`](../polli) is a Discord/GitHub assistant, `weaver` is a router: an OpenAI-compatible `/v1/chat/completions` endpoint (streaming supported) backed by a tool-calling loop that generates images and video, synthesises and transcribes speech, searches the web, and runs shell commands (ffmpeg included) — chaining them freely to satisfy a request across any modality.
...
docker run -p 8000:8000 --env-file .env weaver
```
