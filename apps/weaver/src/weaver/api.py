...
from weaver.agent import run_agent, run_agent_events
from weaver.config import _api_key_override, settings
...
async def _lifespan(_: FastAPI):
    ...
    from weaver.registry import warm_registry
...
app = FastAPI(title="weaver", lifespan=_lifespan)
...
async def _host_audio(art: dict[str, Any]) -> str | None:
    ...
    from weaver.tools import media
...
async def list_models() -> dict[str, Any]:
    return {
        "object": "list",
        "data": [{"id": "weaver", "object": "model", "owned_by": "pollinations"}],
    }
