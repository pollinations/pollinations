...
async def test_transcribe_roundtrip():
    ...
    assert "weaver" in text.lower() or "hello" in text.lower()
...
async def test_chained_video_via_frame_extraction(tmp_path, monkeypatch):
    ...
    from weaver.tools import media
    from weaver.tools.shell import bash
...
async def test_agent_events_stream_live():
    ...
    await warm_registry()
    from weaver.agent import run_agent_events
...
async def test_agent_end_to_end_multi_image():
    ...
    await warm_registry()
    from weaver.agent import run_agent
