...
// Bun variant Discord adapter — talks to the bun web server over HTTP.
// Same shape as the hono variant: surfaces compose, Discord is a client.
...
const BUN_BASE = process.env.BUN_BASE ?? "http://localhost:8787";

async function askBun(question: string, imageUrl?: string) {
    const res = await fetch(`${BUN_BASE}/chat`, {
        method: "POST",
        headers: {
    ...
    if (!res.ok) throw new Error(`bun ${res.status}`);
...
async function handle(msg: Message, client: Client) {
    ...
    const turn = await askBun(question, avatarUrl);
...
client.once(Events.ClientReady, (c) =>
    console.log(`[bun] CatGPT online as ${c.user.tag}`),
);
