...
async function askAgent(userId: string, question: string) {
  const res = await fetch(`${AGENT_BASE}/agents/catgpt/${encodeURIComponent(userId)}/chat`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ messages: [{ role: "user", content: question }] }),
  });
  if (!res.ok) throw new Error(`agent ${res.status}`);
  return (await res.json()) as { content: string; data: { comicUrl: string } };
}
...
client.once(Events.ClientReady, (c) => console.log(`[cf-agents] CatGPT online as ${c.user.tag}`));
