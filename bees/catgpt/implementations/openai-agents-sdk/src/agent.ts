...
const catReplyTool = tool({
  ...
  execute: async ({ question, image_url }, ctx) => {
    const apiKey = (ctx?.context as { apiKey?: string } | undefined)?.apiKey;
...
const comicTool = tool({
  ...
  execute: async ({ question, reply, image_url }, ctx) => {
    const apiKey = (ctx?.context as { apiKey?: string } | undefined)?.apiKey;
    return buildComicImageUrl(question, reply, image_url ?? null, { apiKey });
  },
});
...
export async function ask(question: string, imageUrl?: string, apiKey?: string) {
