-- Backfill existing BYOP keys from the old metadata.clientId field. The old
-- authorize flow wrote this into API key metadata, so we carry it forward for
-- existing keys while future writes use byop_client_key_id directly.
UPDATE `apikey`
