...
async function runGithubValidation(
    users: User[],
): Promise<AccountValidationResult[]> {
    if (users.length === 0) return [];
    return await validateAccountRecords(users);
}
...
async function validateGithubAccounts(
    ...
    const results = await runGithubValidation(usersWithGithub);
    ...
    const resolvedUsers = results.flatMap((result) =>
        ...
        GITHUB_USERNAME_RE.test(result.username)
            ? [
                  {
                      github_id: result.github_id as number,
    ...
    const resultsByGithubId = new Map(
        results.flatMap((result) =>
            Number.isInteger(result.github_id)
                ? [[result.github_id as number, result] as const]
                : [],
        ),
    );
