...
function makeUser(overrides: {
    ...
    const { ageDays = 365, qualityRepos = 3, commits = 50, stars = 5 } = overrides;
...
describe("scoreUser", () => {
    it("scores a valid user and returns approved=true when total >= 8", () => {
        const result = scoreUser(makeUser({ ageDays: 730, qualityRepos: 3, commits: 80, stars: 20 }), 123);
    ...
    it("returns approved=false when total < 8", () => {
        ...
        const result = scoreUser(makeUser({ ageDays: 1, qualityRepos: 0, commits: 0, stars: 0 }), 456);
    ...
    it("caps each scoring dimension at its maximum", () => {
        ...
        const result = scoreUser(makeUser({ ageDays: 9999, qualityRepos: 0, commits: 0, stars: 0 }), 789);
    ...
    it("ignores repos with zero diskUsage in quality count", () => {
        const data = {
            ...
            repositories: {
                totalCount: 5,
                nodes: [
                    { stargazerCount: 0, diskUsage: 0, createdAt: "2024-01-01T00:00:00Z" },
                    { stargazerCount: 0, diskUsage: 0, createdAt: "2024-01-01T00:00:00Z" },
                    { stargazerCount: 5, diskUsage: 200, createdAt: "2024-01-01T00:00:00Z" },
                ],
            },
