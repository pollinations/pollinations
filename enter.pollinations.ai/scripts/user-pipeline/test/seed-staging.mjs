...
console.log(
    "=== Phase 2: Exporting full rows from production (READ-ONLY) ===\n",
);
...
for (let i = 0; i < allUserIds.length; i += ID_BATCH) {
    ...
    if (i % 500 === 0) {
        console.log(
            `  Exporting batch ${Math.floor(i / ID_BATCH) + 1}/${Math.ceil(allUserIds.length / ID_BATCH)}...`,
        );
    }

    const users = queryProd(
...
const tierDist = queryStaging(
    "SELECT tier, COUNT(*) as c FROM user GROUP BY tier ORDER BY c DESC",
);
