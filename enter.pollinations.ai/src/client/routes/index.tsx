...
function RouteComponent() {
  ...
  const balances = {
    pack:
      customer?.activeMeters.find((m) => m.meterId === config.pollenPackMeterId)
        ?.balance || 0,
    tier:
      customer?.activeMeters.find((m) => m.meterId === config.pollenTierMeterId)
        ?.balance || 0,
  };
  ...
  const handleCreateApiKey = async (formState: CreateApiKey) => {
    ...
    if (
      ...
      const updateResponse = await fetch(`/api/api-keys/${apiKey.id}/update`, {
        method: "POST",
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-20">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-col sm:flex-row justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              <Button
                ...
                color="purple"
                weight="light"
                onClick={() => handleBuyPollen("v1:product:pack:5x2")}
              >
                + $5
              </Button>
              <Button
                ...
                color="purple"
                weight="light"
                onClick={() => handleBuyPollen("v1:product:pack:10x2")}
              >
                + $10
              </Button>
              <Button
                ...
                color="purple"
                weight="light"
                onClick={() => handleBuyPollen("v1:product:pack:20x2")}
              >
                + $20
              </Button>
              <Button
                ...
                color="purple"
                weight="light"
                onClick={() => handleBuyPollen("v1:product:pack:50x2")}
              >
                + $50
              </Button>
              <Button
          ...
          <PollenBalance
            balances={balances}
            dailyPollen={tierData?.active.subscriptionDetails?.dailyPollen}
