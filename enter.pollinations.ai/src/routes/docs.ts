    ...
    return (
        new Hono<Env>()
            ...
            .post("/search", async (c) => {
                ...
                const sections = LLM_DOC_TEXT.split(/\n(?=#{2,3} )/);
                ...
                if (scoredSections.length === 0) {
                    return c.json({
                        content: [
                            {
                                type: "text",
                                text: `No specific matches found for "${query}". You can view the full documentation at https://enter.pollinations.ai/api/docs/llm.txt`,
                            },
                        ],
                    });
                }
            ...
            .get("/open-api/generate-schema", async (c, next) => {
                ...
                const schema = (await response.json()) as Record<
                    string,
                    unknown
                >;
