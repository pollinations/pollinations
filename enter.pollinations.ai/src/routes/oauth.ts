...
export const oauthRoutes = new Hono<Env>()
    .post(
        ...
        async (c) => {
            ...
            if (body.purpose === "login" && !isDashboardClient(body.clientId)) {
                throw new HTTPException(403, {
                    message:
                        "Identity login is restricted to first-party dashboards",
                });
            }
    ...
    .post("/token", async (c) => {
        ...
        if (stored.purpose === "login") {
            if (!isDashboardClient(stored.clientId)) {
                return tokenError(
                    c,
                    "invalid_grant",
                    "Untrusted dashboard client",
                );
            }
            // Ask Better Auth on the server, including its adminUserIds policy.
            // The dashboard receives profile data, never role claims or API access.
            const permission = await createAuth(c.env)
            ...
            const activeUser = await getUserinfoForUser(
                c.env,
                stored.userId,
                true,
            ).catch(() => null);
