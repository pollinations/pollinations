...
export const proxyRoutes = new Hono<Env>()
    ...
    .post(
        ...
        async (c) => {
            ...
            // Collect passthrough params (including "image" for img2img in generations)
            const extra: Record<string, unknown> = {};
            for (const key of [...IMAGE_PASSTHROUGH_PARAMS, "image"] as const) {
                if (body[key] !== undefined) extra[key] = body[key];
            }

            const targetUrl = buildImageServiceUrl(
                c.env.IMAGE_SERVICE_URL,
                body.prompt,
                { model, width, height, quality, seed, extra },
            );

            const response = await fetchImageService(targetUrl, c, log);
            ...
            if (body.response_format === "url") {
                ...
                return c.json(
                    imageResponseUrl(imageUrl.toString(), body.prompt),
                );
            }
    ...
    .post(
        ...
        describeRoute({
            ...
            description: [
                ...
                "",
                "**Note:** File uploads are converted to data URIs and passed as URL params to the image backend. Very large files (>8KB) may hit HTTP header size limits — prefer passing image URLs instead.",
                "",
                "**Authentication:** Include your API key as `Authorization: Bearer YOUR_API_KEY`.",
            ].join("\n"),
            responses: {
...
// --- Shared helpers for OpenAI image endpoints ---
...
function parseImageSize(size: string | undefined): [number, number] {
    const [w, h] = (size || "1024x1024").split("x").map((s) => parseInt(s, 10));
...
/** Build the target URL for the image service with standard params. */
function buildImageServiceUrl(
    ...
    if (params.imageUrls?.length) {
        targetUrl.searchParams.set("image", params.imageUrls.join("|"));
    }
...
/** Fetch from the image service and throw an UpstreamError on failure. */
async function fetchImageService(
    targetUrl: URL,
    c: Context,
...
async function parseImageEditInput(c: Context): Promise<{
    ...
    for (const key of IMAGE_PASSTHROUGH_PARAMS) {
        if (body[key] !== undefined) extra[key] = body[key];
    }
