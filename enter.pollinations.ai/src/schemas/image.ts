...
export const GenerateImageRequestQueryParamsSchema = z.object({
    ...
    aspectRatio: z.string().optional().meta({
        description:
            "Video aspect ratio (`16:9`, `9:16`, or `1:1`). Only applies to video models. `1:1` currently supported by `grok-video` only. If not set, determined by width/height.",
    }),
    mode: z
        ...
        .meta({
            description:
                "Generation mode for `grok-video`. `fun` = playful/creative, `normal` = balanced (default), `spicy` = more intense. Only applies to `grok-video`.",
        }),
