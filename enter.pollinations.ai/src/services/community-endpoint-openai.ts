...
// Video endpoints are billed per second of output. The probe sends a minimal
// request and validates the response shape plus a usable duration.
export async function testCommunityVideoEndpoint({
    ...
    const body = await fetchJson(communityVideoGenerationsUrl(baseUrl), {
        ...
        body: JSON.stringify({
            model,
            prompt: "A short clip of green leaves moving in the wind.",
            duration: 3,
        }),
    });

    const videoData = await firstCommunityVideoData(body, baseUrl);
    if (!videoData) {
        throw new Error(
            "Endpoint did not return video data with a duration_seconds field",
        );
    }

    return {
        usage: { duration_seconds: videoData.durationSeconds },
        billableUsage: { completionVideoSeconds: videoData.durationSeconds },
    };
}
