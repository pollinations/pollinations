...
/**
 * Quest statistics — read entirely from the rewards ledger, no evaluator
 * re-scan needed. The scheduled evaluator already writes EVERY qualifying
 * reward into the rewards table (idempotently), so the table is the
 * materialized source of truth: every earned reward is a row.
