...
SCORING = [
    ...
    },  # 0.5pt/month, max 6 (12 months)
    {
        ...
        "multiplier": 0.1,
        "max": 3.0,
    },  # 0.1pt each, max 3 (90-day window)
    {
        "field": "repos",
        "multiplier": 0.5,
        "max": 1.0,
...
def _github_rest_request(url: str, retries: int = 3) -> tuple[dict | None, int]:
    ...
    for attempt in range(retries):
        try:
            ...
            if error.code == 401:
                raise RuntimeError(
                    "GitHub REST auth failed (HTTP 401) — check GITHUB_TOKEN"
                )
            ...
            if attempt < retries - 1 and error.code in (403, 429):
                ...
                if retry_after:
                    ...
                    wait = 60
                print(
                    f"   ⏳ REST rate limited (HTTP {error.code}), waiting {wait}s..."
                )
                time.sleep(wait)
                last_error = error
                continue
            ...
            if attempt < retries - 1:
                ...
                continue
    raise RuntimeError(
        f"GitHub REST request failed after {retries} retries: {last_error}"
    )
...
def lookup_accounts(github_ids: list[int]) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = {
            executor.submit(_lookup, (i, gid)): i for i, gid in enumerate(github_ids)
        }
...
def build_query(accounts: list[dict]) -> str:
    ...
    from_ts = (datetime.now(timezone.utc) - timedelta(days=90)).strftime(
        "%Y-%m-%dT00:00:00Z"
    )
...
def fetch_graphql_batch(
    ...
    resource_errors = [
        e for e in data.get("errors", []) if e.get("type") == "RESOURCE_LIMITS_EXCEEDED"
    ]
    if resource_errors:
        affected = {e["path"][0] for e in resource_errors if e.get("path")}
        print(
            f"   ⚠️  RESOURCE_LIMITS_EXCEEDED for {len(affected)} users in batch of {len(accounts)}"
        )
...
def validate_users(github_ids: list[int]) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = {
            executor.submit(fetch_graphql_batch, batch): i
