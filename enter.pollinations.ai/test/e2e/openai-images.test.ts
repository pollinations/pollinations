...
function getApiKey(): string {
    ...
    try {
        const tokens = readFileSync(resolve(__dirname, "../../.testingtokens"), "utf-8");
...
describe("OpenAI SDK — /v1/images/generations", () => {
    test("generates an image and returns b64_json", { timeout: 60_000 }, async () => {
    ...
    test("returns URL when response_format is url", { timeout: 60_000 }, async () => {
        const response = await client.images.generate({
            model: "flux",
    ...
    test("defaults to b64_json when response_format omitted", { timeout: 60_000 }, async () => {
        const response = await client.images.generate({
            model: "flux",
...
describe("OpenAI SDK — /v1/images/edits", () => {
    test("edits an image via JSON body with URL", { timeout: 120_000 }, async () => {
    ...
    test("edits an image via multipart file upload (OpenAI SDK)", { timeout: 120_000 }, async () => {
...
describe("OpenAI SDK — /v1/models", () => {
    test("lists models including image and text models", { timeout: 10_000 }, async () => {
        ...
        expect(fluxModel?.supported_endpoints).toContain("/v1/images/generations");

        // Should include a known text model
        const textModel = body.data.find((m) => m.id === "openai");
        expect(textModel).toBeDefined();
        expect(textModel?.supported_endpoints).toContain("/v1/chat/completions");
    });
});
