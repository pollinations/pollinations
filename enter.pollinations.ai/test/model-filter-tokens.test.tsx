...
describe("catalog filter dropdowns", () => {
    it.each([
        ...
        "changes %s without editing its other tokens",
        (initial, key, choice, expected, values) => {
            let query = initial;
            const onEdit = vi.fn();
            const close = vi.fn();
            ...
            expect(options.every(({ type }) => type === DropdownItem)).toBe(
                true,
            );
            const optionText = (children: ReactNode) =>
                Children.toArray(children)
                    ...
                    .join("");
            expect(
                options.map(({ props }) => optionText(props.children)),
            ).toEqual(values);
            expect(
