...
import { redactText, type BedrockResponse } from "@/utils/bedrock-guardrail.ts";
...
describe("redactText", () => {
	function makeResponse(
		piiEntities: BedrockResponse["assessments"][0]["sensitiveInformationPolicy"]["piiEntities"] = [],
		regexes: BedrockResponse["assessments"][0]["sensitiveInformationPolicy"]["regexes"] = [],
		outputText?: string,
	): BedrockResponse {
		return {
			action: piiEntities.length > 0 || regexes.length > 0 ? "GUARDRAIL_INTERVENED" : "NONE",
			assessments: [
				{
	...
	it("only redacts allowed types when filter is provided", () => {
		const response = makeResponse([
			{ action: "BLOCKED", match: "test@example.com", type: "EMAIL" },
			{ action: "BLOCKED", match: "AKIAIOSFODNN7EXAMPLE", type: "AWS_ACCESS_KEY" },
		]);
		const emailOnly = new Set(["EMAIL"]);
		expect(
			redactText("email: test@example.com key: AKIAIOSFODNN7EXAMPLE", response, emailOnly),
		).toBe("email: {EMAIL} key: AKIAIOSFODNN7EXAMPLE");
	});

	it("redacts regex matches (custom patterns)", () => {
		const response = makeResponse([], [
			{ action: "BLOCKED", match: "sk_abc123", name: "POLLINATIONS_SECRET_KEY", regex: "sk_[a-zA-Z0-9]+" },
		]);
	...
	it("uses Bedrock anonymized output when available", () => {
		const response = makeResponse(
			[{ action: "ANONYMIZED", match: "test@example.com", type: "EMAIL" }],
			[],
			"my email is {EMAIL}",
		);
		expect(redactText("my email is test@example.com", response)).toBe(
