...
test("Models with API costs should have non-zero operational costs", () => {
    ...
    const nanobanana = calculateCost("google/gemini-2.5-flash-image", usage);
...
test("nanobanana models calculate reasoning token costs", () => {
    ...
    const flashCost = calculateCost("google/gemini-3.1-flash-image", usage);
    ...
    const proCost = calculateCost("google/gemini-3-pro-image", usage);
    expect(proCost.completionReasoningTokens).toBeCloseTo(
        requiredCostRate("google/gemini-3-pro-image", "completionTextTokens") *
            (usage.completionReasoningTokens ?? 0),
        8,
    );
...
test("nanobanana reasoning token event prices use text output rates", () => {
    const flashPrice = getPriceDefinition("google/gemini-3.1-flash-image");
    ...
    const proPrice = getPriceDefinition("google/gemini-3-pro-image");
