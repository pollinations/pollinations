# 💰 Pricing Logic

> **Source**: Notion "Pollen - Pricing & Billing"

---

## Core Model

- **Product**: AI services (image, text, audio, video)
- **Unit**: Pollen credits (prepaid, per-use, USD-denominated)
- **Philosophy**: User-pays - end-users cover their own AI usage

## Pricing Principles (from Notion)

1. **Under-market pricing** - Below consumer rates via partnerships
2. **Prices only go down** - Scheduled reviews, reductions only
3. **Model buckets** - Groups smooth provider price swings
4. **No surprise charges** - Free allocations drive adoption

## Revenue Streams

| Stream | Current | Mechanism |
|--------|---------|-----------|
| **Dev Top-Ups** | ~$6K/mo | B2B credit purchases |
| **End-User Pollen** | Launching Mar 2026 | User-pays via apps |
| **Ads/Sponsorship** | Exploring | Model provider placements |

## Take Rates (from Financial Model)

- **Pollen GMV**: 4% (payment processing)
- **Ads Revenue**: 15% (network fee)

## Cost Structure

- **Current COGS**: ~€40K/mo (fixed, pre-monetization)
- **Target COGS**: 25% of revenue (post-monetization)
- **Target Gross Margin**: 75%

## Open Questions

- [ ] Verify exact Pollen-to-$ conversion rates
- [ ] Finalize video pricing (currently gated)
- [ ] A/B test price sensitivity

---

*Source: Notion "Pollen - Pricing & Billing" page*
