...
describe("community endpoint helpers", () => {
    ...
    it("builds community video models with per-second billing", () => {
        const modelId = "voodoohop/myvideo";
        const definition = communityModelDefinition({
            modelId,
            description: "Community video model",
            modality: "video",
            ...communityEndpointPrices({ completionVideoPrice: 0.1 }),
        });
        ...
        expect(definition).toMatchObject({
            ...
            supportedEndpoints: expect.arrayContaining([
                "/v1/videos/generations",
                "/image/{prompt}",
            ]),
            cost: { completionVideoSeconds: 0.1 },
        });
        expect(definition.cost).not.toHaveProperty("promptTextTokens");
        expect(definition.cost).not.toHaveProperty("completionImageTokens");
        expect(
            calculateUsageBilling({
                model: modelId,
    ...
    it("prices video in per-second units with its own ceiling", () => {
        ...
        expect(MAX_COMMUNITY_PRICE_PER_VIDEO_SECOND).toBeGreaterThan(
            MAX_COMMUNITY_PRICE_PER_SECOND,
        );
        expect(MAX_COMMUNITY_PRICE_PER_VIDEO_SECOND).toBeLessThanOrEqual(1);
    });

    it("derives the video generations URL", () => {
    ...
    it("restricts community video models to the video endpoints", () => {
    ...
    it("keeps the video price as the only billed bucket for its modality", () => {
        const definition = communityPriceDefinition(
            communityEndpointPrices({ completionVideoPrice: 0.05 }),
            "video",
        );
        expect(definition).toEqual({ completionVideoSeconds: 0.05 });
    });
    ...
    describe("community video endpoint billing", () => {
        ...
        it("bills per second using duration_seconds from the response", async () => {
            vi.stubGlobal(
                "fetch",
                vi.fn(async () =>
                    Response.json({
                        data: [
                            {
                                url: "https://api.example.com/assets/video.mp4",
            ...
            const result = await callCommunityVideoEndpoint(
                await videoEndpoint(),
                "a cat walking",
                { model: "video-gen-1", duration: 5 },
                secret,
            );

            expect(result.durationSeconds).toBe(5.0);
            expect(result.trackingData.usage).toEqual({
                completionVideoSeconds: 5.0,
            });
        });

        it("bills using inline base64 video", async () => {
            // minimal valid mp4 ftyp box encoded as base64
            const mp4Base64 = btoa(
                "\x00\x00\x00\x14ftypisom\x00\x00\x00\x00isom",
            );
            vi.stubGlobal(
                "fetch",
                vi.fn(async () =>
                    Response.json({
                        data: [{ b64_json: mp4Base64, duration_seconds: 3.0 }],
                    }),
                ),
            );
            ...
            expect(result.trackingData.usage).toEqual({
                completionVideoSeconds: 3.0,
            });
            expect(result.mimeType).toBe("video/mp4");
        });

        it("fails rather than billing zero when the endpoint omits duration_seconds", async () => {
            vi.stubGlobal(
                "fetch",
                vi.fn(async () =>
                    Response.json({
                        data: [{ b64_json: btoa("some-video") }],
                    }),
                ),
            );
        ...
        it("propagates upstream video endpoint failures", async () => {
            vi.stubGlobal(
                "fetch",
                vi.fn(async () =>
                    Response.json(
