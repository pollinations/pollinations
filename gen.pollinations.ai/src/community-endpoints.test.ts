...
fixtureTest(
    ...
    async () => {
        ...
        const undeclared = await register({});
        expect(undeclared.status).toBe(200);
        const undeclaredBody = (await undeclared.json()) as { id: string };
        const [undeclaredRow] = await db
            .select({ advertised: communityEndpointTable.advertised })
            .from(communityEndpointTable)
            .where(eq(communityEndpointTable.id, undeclaredBody.id));
        expect(undeclaredRow?.advertised).toBeNull();
