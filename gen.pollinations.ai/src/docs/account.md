...
Community text, image, video, and speech-to-text model management. Any authenticated account can list, create, update, delete, and call its private owner-only models. Text providers expose `/v1/chat/completions`; image providers expose `/v1/images/generations` and may also expose `/v1/images/edits`; video providers synchronously return completed media from `/v1/videos/generations`; transcription providers expose `/v1/audio/transcriptions`. Image responses use `b64_json`. Video responses use `data[].b64_json` or `data[].url` with `duration_seconds` for per-second billing. The endpoint test detects image-edit support and selects image pricing: valid OpenAI image token usage enables per-1M-token pricing, otherwise a fixed Pollen price is charged once per successful generated image.
