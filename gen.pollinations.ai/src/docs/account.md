...
- **Prompt agent**: instructions, a default `baseModel`, optional `allowedBaseModels`, and optional MCP servers. Callers can send `X-Pollinations-Agent-Model` with an approved model ID on text generation requests. The default model is always allowed; omit the header to keep it. The chosen model runs the saved prompt directly and its usage is billed normally. The API key must permit both the agent and the selected model. `/text/models` exposes choices as `allowed_base_models`. Existing reasoning controls remain independent: `reasoning_effort` for Chat Completions or `reasoning.effort` for Responses, subject to the selected model’s supported parameters.
