...
| OAuth access token | `at_` | Delegated access | None | Temporary user-authorized token with permissions, budget, expiry, and optional audience binding. |

> **Note:** Publishable Keys (`pk_`) for direct client-side requests have been replaced by the **BYOP (Bring Your Own Pollen)** auth flow. Frontend applications should use the OAuth authorization-code flow with PKCE to obtain a temporary user-authorized access token (`at_`). The legacy fragment redirect and device flow remain supported.
