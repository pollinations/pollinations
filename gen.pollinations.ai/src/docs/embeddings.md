...
`google/gemini-embedding-2` supports text, image, audio, and video inputs. `embed-v4.0` supports text and one image per input. The OpenAI and Qwen embedding models are text-only.
