...
The old `owner/model` IDs remain generation aliases in each model's `aliases` array. Key permissions store canonical IDs: aliases are normalized when a key is saved, and existing stored permissions are migrated with the rename.
