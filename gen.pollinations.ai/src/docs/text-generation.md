...
Managed prompt agents use Pollinations' configured Responses runtime and have no publisher-configured endpoint URL. They use their configured MCP tools and ignore caller-supplied function tool definitions. Their Responses output contains assistant messages and paired `function_call` / `function_call_output` items. Each pair has distinct item IDs and a shared `call_id`; tool names use `mcp__<server>__<tool>`. The result's `output` is a JSON string containing sanitized MCP content, with `isError: true` for tool failures. Both items have `status: "completed"`: the result records an executed tool, not a request for the client to execute it.

For stateless continuation, send previous output items with the next input. Complete call/result pairs are required and are treated as history without re-execution. Chat clients receive executed results as text and media links, not executable tool calls. The shared Chat adapter holds function calls until the terminal response distinguishes completed server tools from pending client tools; text continues streaming normally. This does not prescribe a tool-card UI: clients must support function results in Responses output and preserve both items for replay.
