...
Text models with `video` under `input_modalities` (for example `inclusionai/ling-3.0-flash-vl`) accept `video_url` parts the same way they accept `image_url`: a public `https://` URL or a `data:video/...;base64,...` data URI (Gemini models also accept YouTube and `gs://` URLs). Video tokens are metered from the provider's reported `video_tokens` detail when present and priced at the model's prompt rate; providers that fold video into the prompt total bill it as prompt text tokens, so the billing event stays exact either way.
