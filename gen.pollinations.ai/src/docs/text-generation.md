...
**Gemini** — the prefix must be at least ~2,048 tokens (~4,096 on Gemini 3 models). Requests with tools are not cached. `gemini-3.6-flash`, `gemini-fast`, and `gemini-flash-lite-3.1` cache by default because built-in tools are opt-in; `gemini-3-flash`, `gemini-large`, and the search variants require `"tools": []` or a JSON `response_format` to disable their default tools. Cache creates bill at the standard input rate plus a storage fee for the 1-hour TTL ($1 per 1M cached tokens on Flash models, $4.50 on Pro); hits bill at ~10% of input. The storage fee means caching pays off only when the prefix is reused often — roughly a dozen reuses per hour on the cheapest models.
