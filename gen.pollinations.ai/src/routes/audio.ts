...
export async function generateElevenLabsSpeechWithTimestamps(opts: {
    ...
    const outputFormat = mapOutputFormat(responseFormat);
    const endpoint = `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}/with-timestamps?output_format=${outputFormat}`;
...
export const audioRoutes = new Hono<Env>()
    ...
    .post(
        ...
        resolveModel("generate.audio", { defaultModel: "elevenlabs" }),
        track("generate.audio"),
        async (c) => {
