...
function generateLLMDoc(): string {
    return [
        ...
        "",
        "Generation endpoints accept an optional `safe` parameter for request-input safety checks. If `safe` is omitted, `false`, or `0`, no safety checker runs.",
        "",
        "Supported values:",
        "",
        "- `privacy`: redact detected PII such as email addresses.",
        "- `secrets`: redact detected secrets.",
        "- `sexual`, `violence`, `shield`: block matching unsafe requests.",
        "- `true`: alias for `privacy,secrets`.",
        "- `nsfw`: alias for `sexual,violence`.",
        "",
        "Use it as a query parameter on simple endpoints or a body field on OpenAI-compatible endpoints:",
        "",
        "```bash",
        ...
        "```",
        "",
        'Safety failures return `400` with `error.type: "safety_error"`. Safety service outages fail closed with `503`. Responses may include `X-Safety-Applied`, `X-Safety-Redacted`, or `X-Safety-Status` headers.',
        "",
        "## Generation",
        "",
    ...
    return {
        ...
        info: {
            ...
            description: [
                ...
                "| Value | Behavior |",
                "|-------|----------|",
                "| `privacy` | Redacts detected PII such as email addresses |",
                "| `secrets` | Redacts detected secrets |",
                "| `sexual` | Blocks matching sexual content requests |",
                "| `violence` | Blocks matching violent content requests |",
                "| `shield` | Blocks additional safety policy matches |",
                "| `true` | Alias for `privacy,secrets` |",
                "| `nsfw` | Alias for `sexual,violence` |",
                "",
                "Use it as a query parameter on simple endpoints or a body field on OpenAI-compatible endpoints:",
