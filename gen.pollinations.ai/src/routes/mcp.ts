...
/**
 * Every registered MCP server bills downstream: the caller's credential is
 * forwarded once and the server's own Pollinations calls are what get
 * billed, so this proxy moves no money itself.
 */
...
export const mcpRoutes = new Hono<Env>()
    ...
    .all("/mcp/:serverId", async (c) => {
        ...
        return binding.fetch(requestForMcp(c.req.raw));
    });
