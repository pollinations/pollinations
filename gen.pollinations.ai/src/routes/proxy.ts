...
async function getVisibleModelEntriesForEventType(
    ...
    return (await getGenerationModelRegistry(c.env))
        .visibleEntries(getModelAccess(c))
        .filter((entry) => entry.eventType === eventType);
}

function getModelAccess(c: Context<Env>): GenerationModelAccess {
    return {
...
export const proxyRoutes = new Hono<Env>()
    ...
    .get(
        ...
        describeRoute({
            ...
            description:
                'Returns available models (text, community text, image, realtime, audio, embeddings) in the OpenAI-compatible format (`{object: "list", data: [...]}`). Use this endpoint if you\'re using an OpenAI SDK. For richer metadata including pricing and capabilities, use `/models`, `/text/models`, `/image/models`, `/audio/models`, or `/embeddings/models` instead. When authenticated: private community models owned by the caller or their app developer are included, models are filtered by API key permissions, and `paid_only` models are hidden if the account has no paid balance.',
            responses: {
                200: {
                    description: "Success",
                    content: {
    ...
    .get(
        ...
        describeRoute({
            ...
            summary: "List Models",
            description:
                "Returns all available text, community text, image, video, 3D, realtime, audio, and embedding models with pricing, capabilities, and metadata. When authenticated: private community models owned by the caller or their app developer are included, models are filtered by API key permissions, and `paid_only` models are hidden if the account has no paid balance.",
            responses: {
                200: {
                    description: "Success",
                    content: {
    ...
    .get(
        ...
        describeRoute({
            ...
            description:
                "Returns all available text generation and community text models with pricing, capabilities, and metadata including context window size, supported modalities, and tool support. When authenticated: private community models owned by the caller or their app developer are included, models are filtered by API key permissions, and `paid_only` models are hidden if the account has no paid balance.",
            responses: {
                200: {
                    description: "Success",
                    content: {
