...
const modelCatalogHeaderParameters = () => [
    {
        name: MODEL_SOURCE_HEADER,
        in: "header" as const,
        required: false,
        description:
            "Optional model-source filter. A query parameter takes precedence.",
        schema: {
    ...
    {
        name: MODEL_RELIABILITY_HEADER,
        in: "header" as const,
        required: false,
        description:
            "Optional reliability mode. `all` adds health metadata; `reliable` also filters the list. A query parameter takes precedence.",
        schema: { type: "string" as const, enum: ["all", "reliable"] },
    },
];
...
// Factory for model-list endpoints: validates discovery filters, applies access
// rules first, and only loads health when reliability metadata is requested.
...
const modelsListHandler = (
    ...
    [
        ...
        async (c: Context<Env>) => {
            ...
            const filters = resolveModelCatalogFilters(query, {
                source: c.req.header(MODEL_SOURCE_HEADER),
                reliability: c.req.header(MODEL_RELIABILITY_HEADER),
            });
            const catalogEntries = await filterModelCatalogEntries(
            ...
            return c.json(
                catalogEntries.map(({ entry, health }) =>
                    health === undefined
                        ? entry.info
                        : { ...entry.info, health },
                ),
...
export const proxyRoutes = new Hono<Env>()
    ...
    .get(
        ...
        async (c) => {
            ...
            const modelEntries = await filterModelCatalogEntries(
                filterEntriesByPermissions(
