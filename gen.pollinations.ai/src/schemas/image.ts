...
export const GenerateImageRequestQueryParamsSchema = z.object({
    ...
    width: z.coerce.number().int().nonnegative().optional().default(1024).meta({
        description:
            "Width in pixels. For images, exact pixels; `flux-2-pro`, `flux-2-flex`, `microsoft/mai-image-2.6-flash`, and `microsoft/mai-image-2.6` require multiples of 16 (MAI also needs at least 768 px per side and at most 2,359,296 total pixels). `black-forest-labs/flux.1.1-pro` requires 256–1440 px per side in multiples of 32 and at most 1.6 megapixels. For video models, used for aspect ratio; use `resolution` to select a resolution tier.",
    }),
    height: z.coerce
        .number()
        .int()
        ...
        .meta({
            description:
                "Height in pixels. For images, exact pixels; `flux-2-pro`, `flux-2-flex`, `microsoft/mai-image-2.6-flash`, and `microsoft/mai-image-2.6` require multiples of 16 (MAI also needs at least 768 px per side and at most 2,359,296 total pixels). `black-forest-labs/flux.1.1-pro` requires 256–1440 px per side in multiples of 32 and at most 1.6 megapixels. For video models, used for aspect ratio; use `resolution` to select a resolution tier.",
        }),
    seed: z.coerce
        .number()
        .int()
        ...
        .meta({
            description:
                "Seed for reproducible results. Use -1 for random. Supported by: black-forest-labs/flux.1-schnell, black-forest-labs/flux.1.1-pro, tongyi-mai/z-image-turbo, bytedance/seedream-4.0, black-forest-labs/flux.2-klein-4b, bytedance/seedance-2.0. Other models ignore this parameter.",
        }),
    safe: SafeSchema,
    quality: z
...
export const GenerateVideoRequestQueryParamsSchema =
    GenerateImageRequestQueryParamsSchema.extend({
