...
const GenerateImageRequestQueryParamsBaseSchema = z.object({
    ...
    input_references: z
        ...
        .refine(
            ...
            { message: "input_references accepts HTTP(S) image URLs." },
        )
        .meta({
            description:
                "Reference image URL(s) for visual guidance without fixing the first or last frame. Supported by models with `reference_images` in `video_capabilities`. Separate multiple URLs with `|` or `,`. Cannot be combined with `image` frame controls.",
        }),
