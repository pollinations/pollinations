...
const GenerateImageRequestQueryParamsBaseSchema = z.object({
    ...
    model: z
        ...
        .meta({
            description:
                "Model to use. **Image:** flux, zimage, gptimage, kontext, seedream5, seedream5-pro, nanobanana, nanobanana-pro, klein. **Video:** veo, seedance-pro, seedance-2.0, wan, wan-pro, p-video, nova-reel. See /image/models for full list.",
        }),
    width: z.coerce.number().int().nonnegative().optional().default(1024).meta({
        description:
            "Width in pixels. For images, exact pixels. For video models, used for aspect ratio.",
    }),
    height: z.coerce
        .number()
        .int()
        ...
        .meta({
            description:
                "Height in pixels. For images, exact pixels. For video models, used for aspect ratio.",
        }),
    seed: z.coerce
        .number()
        .int()
    ...
    duration: z.coerce.number().int().min(1).max(120).optional().meta({
        description:
            "Video duration in seconds. Only applies to video models. `veo`: 4, 6, or 8s. `seedance-pro`: 2-10s. `seedance-2.0`: 4-15s. `wan`: 5, 10, or 15s. `p-video`: 1-20s. `nova-reel`: 6-120s (multiples of 6).",
    }),
    resolution: z.enum(["480p", "720p", "1080p"]).optional().meta({
        description:
            "Video resolution. Supported values vary by model and are listed in `resolutions` on `/image/models` and `/models`. Omit it to use the model's first listed resolution.",
    }),
    draft: z.coerce.boolean().optional().default(false).meta({
        description:
            "Use the cheaper draft tier. Currently supported by `p-video`.",
    }),
