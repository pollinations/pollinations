...
// A structurally valid MP3: an ID3v2 tag header (12 bytes, version byte 0x03).
const TEST_MP3_BYTES = [
...
describe("community endpoint helpers", () => {
    ...
    it("builds community speech models billed per character", () => {
    ...
    it("prices speech characters in per-1M units, not per second", () => {
    ...
    describe("community speech endpoint billing", () => {
        ...
        it("forwards the text and bills the upstream characters", async () => {
            const fetchMock = vi.fn(async (input, init) => {
                const request = new Request(input, init);
                expect(request.url).toBe(
                    "https://api.example.com/v1/audio/speech",
        ...
        it("labels non-audio upstream content types as audio/mpeg", async () => {
            vi.stubGlobal(
                "fetch",
                vi.fn(
                    async () =>
...
fixtureTest(
    "registers an OpenAI-compatible speech endpoint and bills it through speech APIs",
    async ({ apiKey }) => {
        const ownerGithubUsername = `owner-${crypto.randomUUID().slice(0, 8)}`;
        ...
        const fetchMock = vi.fn(async (input, init) => {
            ...
            if (request.url === speechUrl) {
                ...
                expect(body.model).toBe("grok-tts-1");
                expect(body.voice).toBe("alloy");
                expect(body.response_format).toBe("mp3");
                // The registration probe sends its own short text; only the
                // real generation call carries the caller's input.
                const isProbe = body.input === "A simple green sprout.";
                if (!isProbe) {
        ...
        expect(testResponse.status).toBe(200);
        await expect(testResponse.json()).resolves.toMatchObject({
            message: "Endpoint responded with speech audio",
            usage: { characters: 22 },
        ...
        expect(listedAudioModel).toMatchObject({
            ...
            pricing: { completionAudioTokens: "0.00001" },
        });
        expect(listedAudioModel?.supported_endpoints).toEqual(
            expect.arrayContaining(["/v1/audio/speech"]),
        );
...
fixtureTest("creates, edits, routes, and deletes managed agents", async () => {
        ...
        new Request(
            `https://enter.test/api/account/my-models/${endpointAgentId}/update`,
            {
                ...
                body: JSON.stringify({
                    ...
                    responsesUrl:
                        "https://updated-agent.example.com/v1/responses",
    ...
    expect(endpointAgentResponse).toMatchObject({
        ...
        type: "endpoint_agent",
        baseUrl: "https://updated-agent.example.com/v1",
        responsesUrl: "https://updated-agent.example.com/v1/responses",
        upstreamModel: "updated-endpoint-agent",
        perUserRpm: 7,
        requiredSafetyFeatures: ["violence"],
    });
