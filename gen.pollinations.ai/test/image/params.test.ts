...
describe("ImageParamsSchema", () => {
    ...
    describe("input_references", () => {
        it("rejects input_references for models without reference_images capability", () => {
            const result = ImageParamsSchema.safeParse({
                model: "flux",
                input_references: "https://example.com/ref.png",
            });
            ...
            if (!result.success) {
                expect(result.error.issues[0]).toMatchObject({
                    ...
                    message: "input_references is not supported by flux.",
                });
            }
        });
        ...
        it("accepts pipe-separated input_references for seedance-2.0", () => {
            ...
            if (result.success) {
                expect(result.data.input_references).toEqual([
            ...
            if (!result.success) {
                expect(result.error.issues[0]).toMatchObject({
                    ...
                    message: expect.stringContaining(
                        "cannot combine input_references",
                    ),
                });
            }
        });
