...
test("Tripo P2 bills provider usage once across joined requests and cached retrieval", async () => {
    reportedFalUnits = 220;
    const bindings = withInlineGenerationCoordinator({
        ...env,
        FAL_KEY: "fal_test_key",
    });
    ...
    const init = {
        ...
        body: JSON.stringify({ model: "tripo3d/p2", seed: 42 }),
    };
    ...
    const rejected = await fetch3d(
        `/3d/${crypto.randomUUID()}?model=tripo3d%2Fp2`,
