...
// Model to use - nanobanana-pro for higher quality
const POLLINATIONS_MODEL = "nanobanana-pro";
const ORIGINAL_CATGPT_IMAGE =
    "https://raw.githubusercontent.com/pollinations/catgpt/refs/heads/main/images/original-catgpt.png";
...
/**
 * Set a custom API key for Pollinations
...
function generateImageURL(prompt, uploadedImageUrl = null) {
    ...
    if (uploadedImageUrl) {
        ...
        imageParam = encodeURIComponent(
            `${ORIGINAL_CATGPT_IMAGE},${uploadedImageUrl}`,
        );
    } else {
        imageParam = encodeURIComponent(ORIGINAL_CATGPT_IMAGE);
    }

    // Build URL with model and optional API key
    let url = `${POLLINATIONS_API}/${encodeURIComponent(prompt)}?model=${POLLINATIONS_MODEL}&image=${imageParam}&nologo=true&enhance=true`;

    // Add API key if provided (enables higher rate limits and priority)
    if (POLLINATIONS_API_KEY) {
        url += `&token=${encodeURIComponent(POLLINATIONS_API_KEY)}`;
    }
    ...
    try {
        ...
        try {
            ...
            if (dataUri.length > 500000) {
                // ~500KB as base64 is larger than binary
                showNotification(
