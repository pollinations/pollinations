...
const EXAMPLES = [
    ...
    {
        prompt: "What is my horoscope? I am gemini. And don't say napping",
        imageUrl: null,
    },
...
function getSavedPrompts() {
    try {
        const savedData =
            JSON.parse(localStorage.getItem("catgpt-v-3-generated")) || [];
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    apiKeyInput.addEventListener("input", (e) => {
        // Update the API key in ai.js when user types
        setApiKey(e.target.value.trim());
        // Save to localStorage for persistence
        if (e.target.value.trim()) {
            localStorage.setItem("catgpt-api-key", e.target.value.trim());
        } else {
            localStorage.removeItem("catgpt-api-key");
        }
    });
    ...
    const savedApiKey = localStorage.getItem("catgpt-api-key");
    if (savedApiKey) {
...
async function generateMeme() {
    ...
    if (imageFile) {
        ...
        try {
            ...
            showNotification(
                "Failed to upload image. Using default CatGPT style.",
                "warning",
            );
            uploadedImageUrl = null;
            hideThumbnail();
...
function handleImageError(errorType = "general") {
    ...
    if (errorType === "timeout") {
        specificMessage =
            "⏰ This cat took too long to respond... probably distracted by a laser pointer! " +
            randomMessage;
    } else {
...
function stopCatAnimation() {
    ...
    document
        .querySelectorAll('[style*="catSlide"]')
...
function loadExamples() {
    ...
    allPrompts.forEach((promptData, index) => {
        const card = createExampleCard(
            promptData,
            index,
            index < savedPrompts.length,
        );
...
function createExampleCard(promptData, index, isUserGenerated = false) {
    ...
    const customImageUrl =
        typeof promptData === "object" ? promptData.imageUrl : null;
