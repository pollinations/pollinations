...
const SettingsPanel = ({ isOpen, settings, onChange, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className="settings-panel" role="dialog" aria-modal="true" aria-label="Session settings">
        ...
        <div className="settings-panel__content">
          <div className="settings-field">
            ...
            <textarea
              ...
              rows={6}
            />
            <p className="settings-field__hint">This runs once at the start of every conversation.</p>
          </div>
          ...
          <div className="settings-panel__grid">
            <div className="settings-field">
              ...
              <input
                ...
                onChange={handleValueChange('maxTokens', (value) => clampNumber(value, 256, 8000))}
              />
              <p className="settings-field__hint">Sets the upper bound for each completion.</p>
            </div>

            <div className="settings-field">
              <div className="settings-field__label-row">
                <label htmlFor="temperature">Temperature</label>
                <span className="settings-field__value">{safeSettings.temperature.toFixed(2)}</span>
              </div>
              <input
                id="temperature"
                type="range"
                ...
                onChange={handleValueChange('temperature', (value) => Number(value))}
              />
              <p className="settings-field__hint">Lower for precise answers, higher for creative ones.</p>
            </div>

            <div className="settings-field">
              <div className="settings-field__label-row">
                <label htmlFor="topP">Top P</label>
                <span className="settings-field__value">{safeSettings.topP.toFixed(2)}</span>
              </div>
              <input
                id="topP"
                type="range"
                ...
                onChange={handleValueChange('topP', (value) => Number(value))}
              />
              <p className="settings-field__hint">Limits sampling to the most probable tokens.</p>
            </div>
          </div>
          ...
          <div className="settings-panel__actions">
            <button
              ...
              onClick={() => {
                onChange?.('systemPrompt', 'You are a helpful AI assistant who speaks concisely and helpfully.');
