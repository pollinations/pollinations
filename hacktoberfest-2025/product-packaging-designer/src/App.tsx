...
import { Upload, Sparkles, Download, Loader2, Box, Droplet, ShoppingBag, Package, Palette, Moon, Sun, Eye } from 'lucide-react';
...
const validateFile = (file: File | null, maxSize: number): { isValid: boolean; error?: string } => {
  if (!file) {
  ...
  if (file.size > maxSize) {
    ...
    return { isValid: false, error: `File size exceeds ${sizeMB}MB limit. Please choose a smaller image.` };
  }
...
function App() {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const errorMessage = handleError(error, 'An error occurred while processing the image. Please try again.');
      alert(errorMessage);
      setFile(null);
    }
  };
  ...
  const generatePackaging = async () => {
    ...
    if (file && file.size > MAX_FILE_SIZE) {
      alert(`Image too large! Please use an image under ${MAX_FILE_SIZE / (1024 * 1024)}MB.`);
      return;
    }
    ...
    try {
      ...
      if (file) {
        try {
          ...
          console.warn('Cloudinary upload failed, using local image:', cloudinaryError);
      ...
      const selectedPackaging = packagingTypes.find(p => p.id === packagingType);

      const prompt = `
Create a high-quality, realistic ${selectedPackaging?.prompt || 'product packaging'} design 
...
${brandName.trim() ? ` Brand name: "${brandName}".` : ''}
      ...
      const imageUrl = `${POLLINATIONS_API}/${encodedPrompt}?model=nanobanana&image=${encodeURIComponent(uploadedUrl)}&referrer=pollinations.github.io&quality=high`;
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        img.onerror = (error) => {
          ...
          reject(new Error('Failed to generate image. The AI service may be temporarily unavailable. Please try again later '));
        };
      ...
      const errorMessage = handleError(error, 'An unexpected error occurred. Please try again.');
      alert(errorMessage);
    } finally {
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      alert(`${errorMessage} Please try right-clicking and saving the image manually.`);
    }
  };
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 ${
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className={`absolute -bottom-40 left-1/2 w-60 h-60 rounded-full blur-3xl opacity-15 ${
      ...
      <div className="relative container mx-auto px-4 py-8 max-w-7xl">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-2xl ${
                isDark ? 'bg-gradient-to-br from-blue-500 to-teal-500' : 'bg-gradient-to-br from-blue-400 to-teal-400'
              } shadow-lg`}>
            ...
            <p className={`text-lg ml-16 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className={`rounded-3xl shadow-2xl p-8 backdrop-blur-sm border transition-all duration-300 ${
            isDark
            ...
            <div className="space-y-8">
              <div>
                ...
                <label className={`group block relative overflow-hidden w-full border-3 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 ${
                  uploadedImage
                    ? isDark ? 'border-teal-500/50 bg-teal-500/5' : 'border-teal-400/50 bg-teal-50/50'
                    : isDark ? 'border-gray-600 hover:border-blue-500 bg-gray-700/30' : 'border-gray-300 hover:border-blue-400 bg-gray-50/50'
                }`}>
                  <input
                    type="file"
                ...
                <div className="grid grid-cols-2 gap-3">
                  {packagingTypes.map((type) => {
                    ...
                    return (
                      ...
                      >
                        <Icon className={`w-7 h-7 mx-auto mb-2 transition-transform duration-300 ${
                          isSelected
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {styleOptions.map((style) => {
                    const isSelected = selectedStyle === style.id && !customStyle;
                    return (
                      <button
                        key={style.id}
                        onClick={() => {
          ...
          <div className={`rounded-3xl shadow-2xl p-8 backdrop-blur-sm border transition-all duration-300 ${
            ...
            <h2 className={`text-2xl font-bold mb-6 flex items-center gap-3 ${
              ...
              <Sparkles className={isDark ? 'text-teal-400' : 'text-blue-500'} />
              Generated Mockup
            </h2>

            <div className={`relative rounded-2xl p-8 min-h-[500px] flex items-center justify-center transition-all duration-300 ${
              ...
              {isGenerating ? (
                ...
                <div className={`w-full space-y-4 ${imageLoaded ? 'animate-fadeIn' : ''}`}>
                ...
                <div className="text-center space-y-4">
                  <div className={`w-24 h-24 mx-auto rounded-3xl flex items-center justify-center mb-4 ${
                    isDark ? 'bg-gradient-to-br from-teal-500 to-blue-600' : 'bg-gradient-to-br from-blue-400 to-teal-400'
                  } shadow-2xl`}>
        ...
        <div className={`mt-8 rounded-3xl shadow-2xl p-8 backdrop-blur-sm border transition-all duration-300 ${
          isDark
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { num: 1, title: 'Upload', desc: 'Add your product photo', gradient: 'from-blue-500 to-blue-600', icon: Upload },
              { num: 2, title: 'Customize', desc: 'Choose style & type', gradient: 'from-teal-500 to-teal-600', icon: Palette },
              { num: 3, title: 'Generate', desc: 'AI creates mockup', gradient: 'from-purple-500 to-pink-600', icon: Sparkles },
              { num: 4, title: 'Download', desc: 'Save your design', gradient: 'from-green-500 to-emerald-600', icon: Download },
            ].map((step) => {
              const StepIcon = step.icon;
              return (
                <div key={step.num} className="text-center space-y-3 group">
                  <div className={`w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br ${step.gradient} flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform duration-300`}>
            ...
            {[
              {
                ...
                style: 'luxury',
                type: 'box',
                image: 'https://res.cloudinary.com/dwzvfzqs7/image/upload/v1759939221/yv0fx8tfo40gzg9xj9o8.jpg',
                gradient: 'from-slate-400 to-slate-600'
              },
              {
                title: 'Luxury Camera',
                style: 'luxury',
                type: 'bottle',
                image: 'https://res.cloudinary.com/dwzvfzqs7/image/upload/v1759939178/p6bjg0grgyzsh5bmh9sj.jpg',
                gradient: 'from-yellow-400 to-amber-600'
              },
              {
                title: 'Bag pakaging',
                style: 'eco-friendly',
                type: 'bag',
                image: 'https://res.cloudinary.com/dwzvfzqs7/image/upload/v1759939305/vod3vhtinuzfenmoll0m.jpg',
                gradient: 'from-green-400 to-emerald-600'
              }
            ].map((example, index) => (
              <div
              ...
              >
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3">
                    <div className={`px-3 py-1 rounded-full text-xs font-bold text-white backdrop-blur-md bg-gradient-to-r ${example.gradient} shadow-lg`}>
                      {example.style}
                    </div>
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <h4 className="text-white font-bold text-lg mb-1">{example.title}</h4>
                    <div className="flex items-center gap-2">
      ...
      {/* Footer */}
      <footer className={`mt-16 py-8 text-center border-t transition-all duration-300 ${
        isDark 
