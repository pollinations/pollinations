...
function readSampledKeys(interval = SAMPLE_INTERVAL) {
  if (!existsSync(KEYS_FILE)) {
    ...
    console.log('💡 Make sure to run this script from the directory containing the keys file');
    process.exit(1);
  }
  
  const content = readFileSync(KEYS_FILE, 'utf8');
  const allKeys = content.split('\n')
    .map(line => line.trim())
  ...
  console.log(`📋 Found ${allKeys.length} total keys, sampling every ${interval}th key (${sampledKeys.length} selected)`);
  return sampledKeys;
}
...
async function downloadObject(key, index) {
  ...
  try {
    console.log(`📥 [${index + 1}] Downloading: ${sanitizedKey.substring(0, 60)}...`);
    ...
    console.error(`❌ [${index + 1}] Error downloading ${sanitizedKey}: ${error.message}`);
