    ...
    while ((match = pattern.exec(originalKey)) !== null) {
      ...
      if (url.startsWith('http')) {
        ...
        try {
          ...
          const splitUrls = decodedUrl.split(',').map(u => u.trim()).filter(u => u);
          urls.push(...splitUrls);
        } catch (error) {
          // If decoding fails, try with the raw match
          const fallbackUrl = 'https://' + url;
          const splitUrls = fallbackUrl.split(',').map(u => u.trim()).filter(u => u);
          urls.push(...splitUrls);
        }
      }
    }
...
async function main() {
  ...
  console.log(`📷 Total unique URLs extracted: ${results.reduce((sum, r) => sum + r.imageUrls.length, 0)}`);
  ...
  import('fs').then(fs => {
    fs.writeFileSync('./extracted_image_urls.json', JSON.stringify(outputData, null, 2));
