...
async function organizeGeneration(item, index) {
  ...
  console.log(`   📥 Input images (${item.imageUrls.length}) will be downloaded separately`);
  ...
  console.log(`   📝 Prompt: "${decodedPrompt.substring(0, 80)}${decodedPrompt.length > 80 ? '...' : ''}"`);
...
async function main() {
  ...
  console.log(`📊 Processing ${results.length} generations with input images\n`);
  ...
  for (let i = 0; i < results.length; i++) {
    try {
      ...
      console.error(`❌ Error processing generation ${i + 1}: ${error.message}`);
    }
  }
  ...
  console.log(`🖼️  Total input images downloaded: ${results.reduce((sum, r) => sum + r.imageUrls.length, 0)}`);
