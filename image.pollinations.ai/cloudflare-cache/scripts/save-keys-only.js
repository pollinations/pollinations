...
function loadCursorState() {
  try {
    if (fs.existsSync(CURSOR_STATE_FILE)) {
      const state = JSON.parse(fs.readFileSync(CURSOR_STATE_FILE, 'utf8'));
      console.log(`📂 Resuming from cursor: ${state.cursor ? state.cursor.substring(0, 20) + '...' : 'start'}`);
...
function saveCursorState(state) {
  try {
    ...
    fs.writeFileSync(CURSOR_STATE_FILE, JSON.stringify(stateToSave, null, 2));
  } catch (error) {
...
async function findResumePoint(lastKey) {
  ...
  while (batchCount < 50) { // Limit search to avoid infinite loop
    batchCount++;
    ...
    if (keyIndex !== -1) {
      console.log(`✅ Found resume point! Key was at position ${keyIndex} in batch ${batchCount}`);
      ...
      if (keyIndex < data.keys.length - 1) {
        ...
        return { cursor: data.cursor, skipInBatch: keyIndex + 1, batchNumber: batchCount };
      } else {
        // Last key was the final key in this batch, so we need the next cursor
        return { cursor: data.cursor, skipInBatch: 0, batchNumber: batchCount + 1 };
      }
    }
...
async function main() {
  ...
  if (STATUS_MODE) {
    ...
    if (fs.existsSync(OUTPUT_FILE)) {
      ...
      console.log(`📁 File contains: ${lines.length.toLocaleString()} keys`);
    }
    return;
  }
  ...
  if (!cursorState.cursor && fs.existsSync(OUTPUT_FILE)) {
    ...
    if (resumeInfo.shouldResume) {
      console.log(`📁 Found existing file with ${resumeInfo.totalExisting.toLocaleString()} keys`);
      console.log(`🔍 Last key: ${resumeInfo.lastKey.substring(0, 100)}...`);
  ...
  console.log(`📊 Starting from: ${cursorState.totalKeys} keys, batch ${cursorState.batchCount + 1}`);
  ...
  try {
    ...
    if (cursorState.totalKeys > 0) {
      console.log(`🔄 Resuming from batch ${batchCount + 1}, already have ${totalFetched.toLocaleString()} keys`);
    }
    ...
    while (totalFetched < MAX_OBJECTS) {
      ...
      if (batchCount % 10 === 0) {
        console.log(`📋 Batch ${batchCount}: Fetching ${BATCH_SIZE} objects... (${totalFetched.toLocaleString()} total so far)`);
      }
      ...
      if (USE_KEYS_ONLY) {
        ...
        if (listResponse.ok) {
          ...
          console.log(`⚠️  Keys-only not available, falling back to regular list action...`);
          ...
          if (!listData || !listData.objects || listData.objects.length === 0) {
          ...
          throw new Error(`Keys-only request failed: ${listResponse.status} ${listResponse.statusText}`);
        }
      } else {
      ...
      if (keysBuffer.length >= BUFFER_SIZE) {
        ...
        console.log(`💾 Wrote ${keysBuffer.length.toLocaleString()} keys to file (${totalFetched.toLocaleString()} total)`);
        keysBuffer = [];
      ...
      if (batchCount % 50 === 0) {
        const elapsed = (Date.now() - startTime) / 1000;
        const rate = totalFetched / elapsed;
        console.log(`📊 Progress: ${batchCount} batches, ${totalFetched.toLocaleString()} objects | Rate: ${rate.toFixed(1)}/s`);
      }
    ...
    if (keysBuffer.length > 0) {
      fs.appendFileSync(OUTPUT_FILE, keysBuffer.join('\n') + '\n');
      console.log(`💾 Wrote final ${keysBuffer.length.toLocaleString()} keys to file`);
    }
    ...
    if (!cursor) {
      try {
        if (fs.existsSync(CURSOR_STATE_FILE)) {
          fs.unlinkSync(CURSOR_STATE_FILE);
          console.log('🧹 Cleaned up cursor state file (download completed)');
        }
      } catch (e) {
    ...
    if (typeof totalFetched !== 'undefined' && typeof batchCount !== 'undefined' && typeof cursor !== 'undefined') {
      const errorState = {
        cursor: cursor,
        totalKeys: totalFetched,
        batchCount: batchCount,
      ...
      console.log('💾 Progress saved to cursor state - you can resume by running the script again');
    }
