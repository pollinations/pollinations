...
import { createSemanticCache, findSimilarImage, cacheImageEmbedding } from './semantic-cache.js';
...
export async function checkSemanticCache(hybridCache, prompt, params) {
  ...
  try {
    ...
    const similarImage = await findSimilarImage(hybridCache.semanticCache, prompt, params);
    
    if (similarImage) {
      console.log(`[HYBRID] Found semantic match: ${similarImage.cacheKey} (similarity: ${similarImage.similarity.toFixed(3)})`);
      return {
 ...
 */
export function storeImageEmbeddingAsync(hybridCache, cacheKey, prompt, params, ctx) {
  ...
  try {
    ...
    ctx.waitUntil(
      cacheImageEmbedding(hybridCache.semanticCache, cacheKey, prompt, params)
        .catch(error => {
    ...
    console.error('[HYBRID] Failed to schedule async embedding storage:', error);
    // Don't throw - this shouldn't break the request
  }
}
