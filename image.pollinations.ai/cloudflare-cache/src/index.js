...
export default {
  async fetch(request, env, ctx) {
      ...
      if (cachedImage) {
        ...
        if (cachedImage.httpMetadata) {
          ...
          if (cachedImage.httpMetadata.contentEncoding) {
            cachedHeaders.set(
              "content-encoding",
              cachedImage.httpMetadata.contentEncoding,
            );
          }
          if (cachedImage.httpMetadata.contentDisposition) {
            cachedHeaders.set(
              "content-disposition",
              cachedImage.httpMetadata.contentDisposition,
            );
          }
          if (cachedImage.httpMetadata.contentLanguage) {
            cachedHeaders.set(
              "content-language",
              cachedImage.httpMetadata.contentLanguage,
            );
          }
        } else {
    ...
    if (
      ...
      ctx.waitUntil(
        cacheResponse(cacheKey, response.clone(), env, url.toString(), request),
      );
