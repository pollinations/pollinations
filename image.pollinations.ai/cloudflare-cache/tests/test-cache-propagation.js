...
const ADJECTIVES = [
    'mysterious', 'ancient', 'glowing', 'shimmering', 'ethereal', 'majestic', 
    'vibrant', 'serene', 'dramatic', 'elegant', 'rustic', 'futuristic',
    'crystalline', 'volcanic', 'frozen', 'luminous', 'weathered', 'pristine'
];

const NOUNS = [
    'castle', 'forest', 'mountain', 'river', 'temple', 'garden', 'cathedral',
    'lighthouse', 'bridge', 'waterfall', 'cavern', 'tower', 'meadow', 'canyon',
    'palace', 'monastery', 'village', 'archipelago', 'glacier', 'oasis'
];

const VERBS = [
    'emerges from', 'stands beneath', 'reflects in', 'overlooks', 'guards',
    'rests beside', 'towers above', 'flows through', 'nestles within',
    'crowns', 'embraces', 'watches over', 'rises from', 'borders'
];
...
async function testCachePropagation() {
    ...
    console.log('| Attempt | Dots | Test Prompt | Time | Cache | Similarity | Semantic Search | Status |');
    console.log('|---------|------|-------------|------|-------|------------|-----------------|--------|');
    ...
    while (true) {
        ...
        if (!result) {
            console.log(`| ${attempt.toString().padEnd(7)} | ${dots.padEnd(4)} | ${testPrompt.substring(0, 20)}... | ${formatTime(Date.now() - startTime)} | ERROR | ERROR | ERROR | ❌ FAIL |`);
            attempt++;
            continue;
        }
        ...
        const promptPreview = testPrompt.length > 25 ? testPrompt.substring(0, 25) + '...' : testPrompt;
        
        console.log(`| ${attempt.toString().padEnd(7)} | ${dots.padEnd(4)} | ${promptPreview.padEnd(11)} | ${timeElapsed} | ${(result.cache || 'N/A').padEnd(5)} | ${(result.similarity || 'N/A').padEnd(10)} | ${(result.semanticSearch || 'N/A').padEnd(15)} | ${status} |`);
        ...
        if (attempt > 30) {
            ...
            console.log('💡 This could indicate an issue with vector indexing or the cache system.');
