...
async function testLiveVectorize() {
  ...
  console.log('Make sure to run "wrangler dev --env test" in another terminal first!\n');
  ...
  try {
    ...
    const response1 = await fetch(`${baseUrl}/prompt/a+cat?width=512&height=512`);
    ...
    const response2 = await fetch(`${baseUrl}/prompt/a+kitten?width=512&height=512`);
    const cacheType2 = response2.headers.get('x-cache') || 'unknown';
    const cacheTypeDetailed2 = response2.headers.get('x-cache-type') || 'unknown';
    ...
    const response3 = await fetch(`${baseUrl}/prompt/a+cat?width=256&height=256`);
    ...
    const response4 = await fetch(`${baseUrl}/prompt/a+cat?width=512&height=512`);
    ...
    console.log(`   Test 2 (similar prompt): ${cacheType2} ${similarity2 ? `(${similarity2})` : ''}`);
    console.log(`   Test 3 (diff resolution): ${cacheType3}`);
    console.log(`   Test 4 (exact match): ${cacheType4}`);
    
    console.log('\n🎉 Live Vectorize testing completed!');
    console.log('Check the wrangler dev console for detailed semantic cache logs.');
    
  } catch (error) {
    console.error('❌ Test failed:', error.message);
    console.log('\n🔧 Troubleshooting:');
    console.log('   1. Make sure "wrangler dev --env test" is running');
    console.log('   2. Check if Vectorize index "pollinations-image-cache-test" exists');
