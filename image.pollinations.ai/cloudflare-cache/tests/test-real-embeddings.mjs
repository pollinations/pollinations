...
async function testEmbeddingSimilarities(ai) {
    ...
    try {
        ...
        console.log(`✅ Base embedding generated (${baseEmbedding.length} dimensions)\n`);

        // Test all variations
        console.log('🔄 Testing variations...\n');
        console.log('| Original Prompt | Normalized Prompt | Similarity | Above 0.8? | Notes |');
        console.log('|-----------------|-------------------|------------|-------------|-------|');
        ...
        for (const variation of TEST_PROMPTS) {
            try {
                ...
                console.log(`| ${variation.slice(0, 15).padEnd(15)} | ${normalized.slice(0, 17).padEnd(17)} | ${similarity.toFixed(4)} | ${aboveThreshold ? '✅ YES' : '❌ NO'} | ${notes} |`);
                
                // Small delay to be nice to the API
                await new Promise(resolve => setTimeout(resolve, 50));
            } catch (error) {
                console.log(`| ${variation.slice(0, 15).padEnd(15)} | ERROR | ERROR | ❌ ERROR | ${error.message} |`);
            }
        }
        ...
        console.log('- Normalization removes punctuation, so dots should normalize to same text');
        ...
        console.log('- 0.8 threshold appears appropriate for avoiding false positives');
...
export { testEmbeddingSimilarities, generateEmbedding, cosineSimilarity, normalizePrompt };
...
export default {
    async fetch(request, env) {
        ...
        return new Response('Use /test-embeddings to run the embedding similarity test', {
            headers: { 'Content-Type': 'text/plain' }
        });
    }
};
