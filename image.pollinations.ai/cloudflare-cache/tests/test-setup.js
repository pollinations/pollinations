...
function runCommand(command, silent = false) {
  try {
    const output = execSync(command, { encoding: 'utf8', stdio: silent ? 'ignore' : 'pipe' });
...
if (!commandExists('wrangler')) {
  ...
  if (wranglerVersion.success) {
    console.log(`   ✅ Wrangler installed: ${wranglerVersion.output.trim()}`);
    
    // Check if version is >= 3.71.0 (required for Vectorize V2)
    const versionMatch = wranglerVersion.output.match(/(\d+)\.(\d+)\.(\d+)/);
    if (versionMatch) {
      const [, major, minor, patch] = versionMatch.map(Number);
      if (major > 3 || (major === 3 && minor >= 71)) {
        console.log('   ✅ Version supports Vectorize V2');
      } else {
        console.log('   ⚠️  Version may not support Vectorize V2 (requires ≥3.71.0)');
      }
    }
  } else {
...
if (!fs.existsSync(wranglerConfigPath)) {
  ...
  const checks = [
    ...
    { name: 'Vectorize index name', pattern: /index_name\s*=\s*["']pollinations-image-cache["']/ },
...
if (fs.existsSync(indexPath)) {
  ...
  const integrationChecks = [
    ...
    { name: 'Semantic cache check', pattern: /checkSemanticCacheAndRespond/ },
...
console.log('\n📊 Setup Summary:');
if (hasErrors) {
  console.log('❌ Setup incomplete - please fix the issues above before testing');
  console.log('\n🚦 Next steps:');
