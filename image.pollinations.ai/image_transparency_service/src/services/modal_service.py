...
# Create a Modal app
app = modal.App("image-transparency-service")

# Create an image from the Docker registry with GPU support
image = (
    modal.Image.from_registry("pytorch/pytorch:latest")
    .pip_install("torch", "transformers", "Pillow", "httpx")  # Install dependencies
)

@app.function(
    image=image,
    gpu="A10G",  # Using GPU for optimal inference performance
...
def web_app():
    ...
    os.environ["SERVICE_TYPE"] = "image_transparency"  # Set service type for heartbeat
