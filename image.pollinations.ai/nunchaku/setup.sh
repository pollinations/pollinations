...
validate_env() {
    local missing=0
    # PLN_GPU_TOKEN is required for authentication
    for var in HF_TOKEN WORKER_NUM PUBLIC_IP GPU0_PUBLIC_PORT GPU1_PUBLIC_PORT PLN_GPU_TOKEN; do
        if [ -z "${!var}" ]; then
