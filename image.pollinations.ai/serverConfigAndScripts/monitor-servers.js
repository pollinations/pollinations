...
// EC2 endpoint (requires SSH tunnel: ssh -L 16384:localhost:16384 enter-services)
const REGISTER_URL =
    process.env.REGISTER_URL || "http://localhost:16384/register";
...
function bar(value, max, width = 20, full = "█", empty = "░") {
    const filled = Math.min(
        width,
        Math.round((value / Math.max(max, 1)) * width),
    );
...
function render(servers) {
    ...
    console.log(
        `${colors.bright}${colors.cyan}⚡ FLUX${colors.reset} ${colors.dim}${timestamp}${colors.reset}`,
    );
    ...
    // Header
    console.log(
        `${colors.dim}Port  │ Queue            │ Reqs  │ RPS  │ Errs${colors.reset}`,
    );
    console.log(
    ...
    const totalRps = servers
        .reduce((sum, s) => sum + parseFloat(s.requestsPerSecond), 0)
        .toFixed(2);
...
async function monitor() {
    try {
        ...
        console.error(
            `${colors.red}Error fetching data:${colors.reset}`,
            err.message,
        );
    }
}
