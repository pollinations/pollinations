	...
	Object.entries(SERVERS).forEach(([type, servers]) => {
		if (servers.length > 0) {
			const serverQueueInfo = servers.map((server) => ({
				...
				errorRate:
					((server.errors / server.totalRequests) * 100 || 0).toFixed(2) + "%",
				requestsPerSecond: (
...
export const getNextServerUrl = async (type = "flux") => {
	...
	const minQueueSize = Math.min(
		...activeServers.map((server) => server.queue.size + server.queue.pending),
	);
...
export const handleRegisterEndpoint = (req, res) => {
	if (req.method === "POST") {
		...
		const availableServers = Object.entries(SERVERS)
			.map(([type, servers]) => servers.map((server) => ({ ...server, type })))
			.flat();
		// res.writeHead(200, {
