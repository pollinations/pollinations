...
export const fetchFromLeastBusyServer = async (
    ...
    if (queueLoad >= MAX_QUEUE_SIZE) {
        throw new Error(
            `Queue full (${queueLoad}/${MAX_QUEUE_SIZE}). This legacy endpoint is overloaded. Please use https://enter.pollinations.ai/?ref=image for faster, more reliable image generation.`
        );
    }
