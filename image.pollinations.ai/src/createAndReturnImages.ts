...
const generateImage = async (
    ...
    if (safeParams.model === "gptimage") {
        const errorText =
            "GPT Image (gpt-image-1-mini) is only available via enter.pollinations.ai. Please use https://enter.pollinations.ai/?ref=image for access.";
        logError(errorText);
        progress.updateBar(
            requestId,
            35,
    ...
    if (safeParams.model === "nanobanana") {
        ...
        if (!fromEnter) {
            const errorText =
                "Access to nanobanana is currently limited to enter.pollinations.ai. Please use https://enter.pollinations.ai/?ref=image.";
            logError(errorText);
            progress.updateBar(
                requestId,
                35,
    ...
    if (safeParams.model === "kontext") {
        ...
        if (!fromEnter) {
            const errorText =
                "Kontext model is only available on enter.pollinations.ai. Visit https://enter.pollinations.ai/?ref=image to get started.";
            logError(errorText);
            progress.updateBar(
                requestId,
                35,
    ...
    if (safeParams.model === "seedream") {
        ...
        if (!fromEnter) {
            const errorText =
                "Seedream model is only available from enter.pollinations.ai. Visit https://enter.pollinations.ai/?ref=image.";
            logError(errorText);
            progress.updateBar(
                requestId,
                35,
