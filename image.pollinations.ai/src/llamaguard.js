...
export async function checkContent(content) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(
      () => reject(new Error("LlamaGuard check timed out after 5 seconds")),
      timeoutMs,
    );
  });

  try {
    const result = await Promise.race([timeoutPromise, _checkContent(content)]);
    return result;
  } catch (error) {
    throw error;
  }
...
async function _checkContent(content) {
  ...
  try {
    ...
    if (content.length > MAX_CONTENT_LENGTH) {
      log(
        `Content length exceeds ${MAX_CONTENT_LENGTH} characters, slicing to first ${MAX_CONTENT_LENGTH}`,
      );
