...
describe("Image Processing Integration Tests", () => {
    ...
    beforeAll(async () => {
        ...
        testImageBuffer = await fs.readFile(
            path.join(process.cwd(), "test", "fixtures", "test-image.jpg"),
        );
    });
    ...
    describe("Image Pipeline Integration", () => {
        it("should process an image through the entire pipeline", async () => {
            ...
            const finalBuffer = await addPollinationsLogoWithImagemagick(
                blurredBuffer,
                logoPath,
                {
                    width: 512,
    ...
    describe("Error Handling", () => {
        it("should handle invalid image data gracefully", async () => {
            ...
            await expect(
                resizeImage(invalidBuffer, 512, 512),
            ).rejects.toThrow();
        });
    });
