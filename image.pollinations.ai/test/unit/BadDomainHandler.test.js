...
describe("BadDomainHandler", () => {
    ...
    beforeEach(() => {
        process.env = {
            ...originalEnv,
            BAD_DOMAINS: "bad-site.com,another-bad-site.com",
        };
        vi.clearAllMocks();
    });
    ...
    describe("isBadDomain", () => {
        it("handles various domain scenarios correctly", () => {
            ...
            expect(badDomainHandler.isBadDomain("http://bad-site.com")).toBe(
                true,
            );
            expect(badDomainHandler.isBadDomain("subdomain.bad-site.com")).toBe(
                true,
            );
            expect(badDomainHandler.isBadDomain("BAD-SITE.COM")).toBe(true);
    ...
    describe("transformToOpposite", () => {
        it("transforms a prompt using the API", async () => {
            ...
            const result = await badDomainHandler.transformToOpposite(
                "young woman without clothes",
            );
        ...
        it("handles API errors gracefully", async () => {
            ...
            const result =
                await badDomainHandler.transformToOpposite(originalPrompt);
    ...
    describe("processPrompt", () => {
        it("handles non-bad domains correctly", async () => {
            const result = await badDomainHandler.processPrompt("test prompt", {
                referer: "good-site.com",
            });
        ...
        it("transforms prompts for bad domains when random value is low", async () => {
            ...
            const result = await badDomainHandler.processPrompt(
                "original prompt",
                { referer: "bad-site.com" },
            );
        ...
        it("caches results with the same parameters", async () => {
            ...
            const result1 = await badDomainHandler.processPrompt(
                "test prompt",
                { referer: "good-site.com" },
            );
            ...
            const result2 = await badDomainHandler.processPrompt(
                "test prompt",
                { referer: "good-site.com" },
            );
