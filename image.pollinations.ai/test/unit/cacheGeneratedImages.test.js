...
import {
  ...
  cacheImage,
  memoize,
} from "../../src/cacheGeneratedImages.js";
...
describe("Image Caching", () => {
  ...
  describe("Basic Cache Operations", () => {
    it("should cache and retrieve an image", async () => {
      ...
      await cacheImage(metadata.prompt, metadata, () =>
  ...
  describe("Cache Size Management", () => {
    it("should evict oldest entries when cache is full", async () => {
      ...
      expect(
        isImageCached(oldestEntry.metadata.prompt, oldestEntry.metadata),
      ).toBe(false);

      // Verify newest entries are still cached
      const newestEntry = entries[entries.length - 1];
      expect(
        isImageCached(newestEntry.metadata.prompt, newestEntry.metadata),
      ).toBe(true);
    });
    ...
    it("should update LRU order when accessing cached items", async () => {
      ...
      await cacheImage(firstMeta.prompt, firstMeta, () =>
        Promise.resolve(firstData),
      );

      const secondData = Buffer.from("second-image");
      const secondMeta = { prompt: "second", model: "test", seed: 2 };
      await cacheImage(secondMeta.prompt, secondMeta, () =>
        Promise.resolve(secondData),
      );
      ...
      await cacheImage(thirdMeta.prompt, thirdMeta, () =>
        Promise.resolve(thirdData),
      );
  ...
  describe("Error Handling", () => {
    it("should handle errors in buffer promise creator", async () => {
      ...
      await expect(
        cacheImage(metadata.prompt, metadata, () =>
          Promise.reject(new Error("Failed to create buffer")),
        ),
