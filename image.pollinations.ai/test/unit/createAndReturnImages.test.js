...
} from "../../src/createAndReturnImages.js";
...
describe("Image Creation and Processing", () => {
  beforeEach(() => {
    ...
    writeExifMetadata.mockResolvedValue(
      Buffer.from("test-image-with-metadata"),
    );
  ...
  describe("calculateScaledDimensions", () => {
    it("should maintain dimensions if already at target size", () => {
      const { scaledWidth, scaledHeight, scalingFactor } =
        calculateScaledDimensions(1024, 1024);
    ...
    it("should scale up smaller images", () => {
      const { scaledWidth, scaledHeight, scalingFactor } =
        calculateScaledDimensions(512, 512);
      expect(scaledWidth).toBe(1024);
      expect(scaledHeight).toBe(1024);
  ...
  describe("convertToJpeg", () => {
    ...
    it("should handle conversion errors", async () => {
      ...
      await expect(convertToJpeg(buffer)).rejects.toThrow("Conversion failed");
    });
  });
  ...
  describe("API Calls", () => {
    ...
    describe("callMeoow", () => {
      ...
      it("should handle API errors", async () => {
        ...
        await expect(callMeoow(prompt, safeParams)).rejects.toThrow(
    ...
    describe("callMeoow2", () => {
      ...
      it("should handle API errors", async () => {
        ...
        await expect(callMeoow2(prompt, safeParams)).rejects.toThrow(
          "Server responded with 500",
        );
      });
    });
