...
REGISTER_URL = os.environ.get(
    "REGISTER_URL", "http://ec2-54-147-14-220.compute-1.amazonaws.com:16384/register"
)
...
def heartbeat_loop():
    ...
    url = "%s?port=%s&ip=%s&type=%s&token=%s" % (
        REGISTER_URL,
        port,
        PUBLIC_IP,
        SERVICE_TYPE,
...
def status(prompt_id: str = Query(...)):
    hist = json.loads(
        urllib.request.urlopen("%s/history/%s" % (COMFY_BASE, prompt_id)).read()
    )
            ...
            if items:
                ...
                if fn.endswith((".mp4", ".webm")):
                    ...
                    with urllib.request.urlopen(
                        "%s/view?%s" % (COMFY_BASE, params)
                    ) as r:
                        data = r.read()
                    ctype = "video/webm" if fn.endswith(".webm") else "video/mp4"
                    log.info(
                        "Serving %s: %.1f MB", prompt_id[:8], len(data) / 1024 / 1024
                    )
