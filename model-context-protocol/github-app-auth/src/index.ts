...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const logAndReturnResponse = (response: Response, context: string = "") => {
      ...
      console.log(
        `[${new Date().toISOString()}] ${method} ${path} - Response sent (${response.status}) in ${duration}ms ${context}`,
      );
      return response;
    };
    ...
    try {
      ...
      if (path.match(/^\/api\/user\/(.+)\/domains$/) && method === "GET") {
        ...
        try {
          ...
          const domains = user.domain_allowlist
            ? JSON.parse(user.domain_allowlist)
            : [];
      ...
      if (path.match(/^\/api\/user\/(.+)\/domains$/) && method === "PUT") {
        ...
        try {
          ...
          if (!Array.isArray(domains)) {
            return logAndReturnResponse(
              new Response(
                JSON.stringify({ error: "Invalid domains format" }),
                {
                  status: 400,
                  headers: { "Content-Type": "application/json" },
                },
          ...
          return logAndReturnResponse(
            new Response(
              JSON.stringify({ error: "Failed to update domains" }),
              {
                status: 500,
                headers: { "Content-Type": "application/json" },
              },
      ...
      return logAndReturnResponse(
        errorResponse,
        `Unhandled error: ${serverError.message}`,
      );
    }
  },
};
