...
async function main() {
  ...
  try {
    ...
    await askQuestion(
      'Press Enter after you see "Authentication Complete" in the browser...',
    );
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const statusResponse = await fetch(
        `${AUTH_API_BASE_URL}/status/${sessionId}`,
      );
    ...
    if (manualSessionId) {
      console.log(
        `Checking status for manually entered session ID: ${manualSessionId}`,
      );

      const manualStatusResponse = await fetch(
        `${AUTH_API_BASE_URL}/status/${manualSessionId}`,
      );
