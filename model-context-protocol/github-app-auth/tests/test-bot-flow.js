...
async function startAuth() {
  ...
  try {
    ...
    if (openBrowser.toLowerCase() === "y") {
      await open(authUrl);
      console.log(
        "Browser opened. Please complete the authentication in your browser.",
      );
    }
...
async function getDomains() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/user/${githubUserId}/domains`,
      {
        headers: {
...
async function main() {
  ...
  console.log(
    "This bot helps you manage your allowlisted domains after authenticating with GitHub.",
  );
  console.log(
    "You will be guided through the authentication process and then can manage your domains.\n",
  );

  const startNow = await askQuestion(
    "Would you like to start the authentication process? (y/n): ",
  );
