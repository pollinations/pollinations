...
// Use an immediately invoked async function expression (IIFE) to handle top-level await
(async () => {
  try {
    // Import and setup polyfill for AbortController if needed
    const { setupAbortControllerPolyfill } = await import('./src/utils/polyfills.js');
