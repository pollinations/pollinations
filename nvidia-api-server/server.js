...
var NVIDIA_BASE_URL = (process.env.NVIDIA_BASE_URL || "https://integrate.api.nvidia.com/v1").replace(/\/$/, "");
var MAX_BODY_BYTES = Number(process.env.MAX_BODY_BYTES || 2000000);
var MAX_RATE_LIMIT_WAIT_MS = Number(process.env.MAX_RATE_LIMIT_WAIT_MS || 180000);
...
function error(message, status = 400, type = "invalid_request_error", extra = {}) {
...
function nvidiaKeys() {
  const numbered = [secretValue("NVIDIA_API_KEY_1"), secretValue("NVIDIA_API_KEY_2")].filter((value) => Boolean(value));
...
async function acquireKey(deadline) {
  while (true) {
    ...
    if (key)
      return { key, waitedMs: Math.max(0, MAX_RATE_LIMIT_WAIT_MS - (deadline - Date.now())) };
    const retryAt = Math.min(...keys.map((value) => keyCooldowns.get(value) || Date.now()));
...
async function proxy(request, pathname) {
  if (nvidiaKeys().length === 0)
    return error("NVIDIA_API_KEY_1 or NVIDIA_API_KEY_2 is not configured on the server.", 503, "configuration_error");
  let body;
  ...
  while (true) {
    ...
    if (!acquired) {
      const remainingCooldowns = nvidiaKeys().map((key) => Math.max(0, (keyCooldowns.get(key) || 0) - Date.now()));
      const retryAfterSeconds = Math.ceil((Math.min(...remainingCooldowns, MAX_RATE_LIMIT_WAIT_MS) || MAX_RATE_LIMIT_WAIT_MS) / 1000);
      return error("All configured NVIDIA keys are rate limited and the next available key is beyond the maximum wait window.", 429, "rate_limit_error", {
    ...
    if (Date.now() >= deadline) {
      return error("All configured NVIDIA keys are rate limited and the maximum wait window has elapsed.", 429, "rate_limit_error", {
        retry_after_seconds: Math.ceil(MAX_RATE_LIMIT_WAIT_MS / 1000),
...
async function handler(request) {
  ...
  if (url.pathname === "/health" || url.pathname === "/healthz") {
    return json({ status: "ok", service: "nvidia-api-server", upstream: NVIDIA_BASE_URL, configured_keys: nvidiaKeys().length, max_rate_limit_wait_seconds: Math.ceil(MAX_RATE_LIMIT_WAIT_MS / 1000) });
  }
  if (url.pathname === "/" && request.method === "GET")
    return new Response(docsPage(), { headers: { "content-type": "text/html; charset=utf-8", ...corsHeaders } });
  if (url.pathname === "/docs" && request.method === "GET")
    return new Response(docsPage(), { headers: { "content-type": "text/html; charset=utf-8", ...corsHeaders } });
  if (url.pathname === "/playground" || url.pathname === "/playground/")
    return new Response(playgroundPage(), { headers: { "content-type": "text/html; charset=utf-8", ...corsHeaders } });
