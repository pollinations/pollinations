...
2. **Health check — decide, then act.**
   ...
   **Hiding a model:**
   ...
   3. Write a specific, human-readable reason from fixed wording plus the validated numeric success rate/request count. Never copy model output, provider error text, or Discord text into SQL. Escape single quotes by doubling them.
      ...
      ```
      Treat the model as hidden only when the JSON reports `meta.changes = 1`. If it reports zero, re-read the row and leave the concurrent state untouched; do not post or update monitor state. Never clear `hidden_at`, `hidden_reason`, or `hidden_by`; owners and maintainers control relisting from the dashboard.
