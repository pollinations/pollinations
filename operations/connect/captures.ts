...
export function createCaptureService(options: {
    ...
    type Resources = {
        ...
        browser?: Browser;
        transport?: ReturnType<typeof serve>;
    ...
    async function getResources() {
        ...
        await once(session.transport, "listening");
    ...
    async function capture(job: Job) {
        const { recipe } = job;
        console.info(
            `Connect preview: ${job.selection.flow}/${job.selection.section}/${recipe.id}`,
        );
        ...
        try {
            ...
            page.on("framenavigated", (frame) => {
                if (frame === page?.mainFrame()) {
                    ...
                    if (url.origin === origin)
                        visitedRoutes.add(`${url.pathname}${url.search}`);
                }
            });
            ...
            await page.goto(`${origin}/pollen-connect-screen.html?${query}`, {
            ...
            if (entryRoute && !visitedRoutes.has(entryRoute))
            ...
            if (requestedFaults.length) {
                ...
                for (;;) {
                    const response = await runtime.fetch(
                        new Request(`${origin}/__connect/review/requests`),
                    );
                    if (!response.ok)
            ...
            if (
                await page
                    .locator(
                        '#root[role="alert"], html[data-connect-bootstrap-error]',
                    )
                    .count()
            )
                throw new Error("Connect bootstrap failed");
            if (
                    ...
                    ? new Response(html, {
                          headers: {
                              ...
                              "Content-Security-Policy":
                                  "default-src 'none'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://avatars.githubusercontent.com; font-src 'self' data:; script-src 'none'; connect-src 'none'; form-action 'none'; base-uri 'self'; frame-ancestors 'self'",
                          },
                      })
                    : new Response(null, { status: 404 });
            }
