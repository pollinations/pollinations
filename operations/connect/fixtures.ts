...
export const USER_ID = "connect-local-user";
export const OWNER_ID = "connect-local-developer";
export const CLIENT_KEY_ID = "connect-local-app";
export const CLIENT_ID = "pk_connect_local_example_not_a_real_credential";
...
export async function seedFixtures(
    ...
    await db.batch([
        db.prepare("DELETE FROM connect_device"),
        db
        ...
        db
            ...
            .bind(
                OWNER_ID,
                "Connect Example Developer",
                "developer@connect.test",
                100000002,
                "developer",
                now,
                now,
        ...
        db
            ...
            .bind(
                ...
                JSON.stringify({
                    keyType: "publishable",
                    createdVia: "connect-local-fixture",
        ...
        db
            .prepare(`INSERT INTO oauth_client
                ...
                VALUES ('connect-local-admin-client', ?, ?, 0, 1, ?, ?, ?,
            ...
            .bind(
                ...
                "Admin example",
                JSON.stringify(["http://localhost:4180/auth/callback"]),
        ...
        db
            .prepare(`INSERT OR REPLACE INTO connect_conditions
...
export async function setConditions(
    ...
    const stored = await db
        .prepare(
            "SELECT account, pollen, allowance, role FROM connect_conditions WHERE id = 1",
        )
        .first<Conditions>();
    ...
    const statements = [
        db
            .prepare(
                "UPDATE connect_conditions SET account = ?, pollen = ?, allowance = ?, role = ? WHERE id = 1",
            )
            .bind(
                conditions.account,
                conditions.pollen,
...
export async function readState(db: D1Database) {
    ...
    const device = await db
        .prepare(`SELECT c.user_code AS userCode,
            ...
            FROM connect_device c LEFT JOIN device_code d
...
export type ConnectState = Awaited<ReturnType<typeof readState>>;
