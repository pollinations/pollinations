...
const keyIds = ["connect-review-key", "connect-review-app"] as const;
const listingIds = ["connect-review-model", "connect-review-agent"] as const;
...
export async function prepareDashboardReview(
    ...
    const conditions = await db
        .prepare("SELECT allowance FROM connect_conditions WHERE id = 1")
        ...
        ...listingIds.map((id, index) =>
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, '[]', ?, 'private', ?, ?)`)
                .bind(
                    ...
                    index
                        ? PROMPT_AGENT_BASE_URL_PLACEHOLDER
                        : "https://connect-review.invalid/v1",
