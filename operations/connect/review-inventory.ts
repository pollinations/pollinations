...
// Frozen requests are useful for visual review. Journey should progress through
// those states naturally; real waiting conditions (e.g. payment pending) remain.
export function isJourneySituation(recipe: ReviewCase): boolean {
    return !(
        (recipe.action?.type === "sign-in" &&
            recipe.action.outcome === "pending") ||
        recipe.requests?.some((request) => request.outcome === "pending") ||
        recipe.steps?.some((step) =>
            step.requests?.some((request) => request.outcome === "pending"),
        )
    );
}

export function journeySituation(cases: ReviewCase[], selected: ReviewCase) {
    if (isJourneySituation(selected)) return selected;
    return (
        cases.find(
            (recipe) =>
                recipe.pageId === selected.pageId && isJourneySituation(recipe),
        ) ?? cases.find(isJourneySituation)
    );
}
...
export function situationLabel(
    ...
    if (resource) {
        ...
        if (label === "Hide") return `${noun} listed`;
        if (label === "Relist") return `${noun} hidden`;
    }
    const context = {
