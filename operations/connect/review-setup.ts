...
export const reviewSetupOptions = {
    ...
    listing: ["hidden", "queued", "endpoint-agent", "public"],
...
// Only metadata in Connect's isolated database. No credentials or UI markup.
export async function prepareReviewData(db: D1Database, setup: ReviewSetup) {
    ...
    await db.batch([
        ...
        db
            .prepare(
                "UPDATE user SET stripe_customer_id = NULL, auto_top_up_enabled = 0 WHERE id = ? AND stripe_customer_id = 'cus_connect_review'",
            )
            .bind(USER_ID),
        db
            .prepare(
                "DELETE FROM stripe_auto_top_up_attempt WHERE id = 'connect-review-attempt' AND user_id = ?",
            )
            .bind(USER_ID),
        db
            .prepare(
                "DELETE FROM stripe_checkout_credits WHERE session_id = 'cs_connect_review' AND user_id = ?",
            )
            .bind(USER_ID),
        db
            .prepare(
                "DELETE FROM account WHERE id = 'connect-review-discord' AND user_id = ?",
            )
            .bind(USER_ID),
    ]);
    ...
    if (setup.discord)
        await db
            .prepare(
                "INSERT OR REPLACE INTO account (id, provider_id, account_id, user_id, access_token, created_at, updated_at) VALUES ('connect-review-discord', 'discord', '100000000000000001', ?, 'invalid-local-review-placeholder', ?, ?)",
            )
    ...
    if (setup.billing)
        await db
            .prepare(
                "UPDATE user SET stripe_customer_id = 'cus_connect_review', auto_top_up_enabled = ?, auto_top_up_amount_usd = 5 WHERE id = ?",
            )
            .bind(
    ...
    if (setup.billing === "payment-action")
        await db
            .prepare(
                "INSERT OR REPLACE INTO stripe_auto_top_up_attempt (id, user_id, stripe_invoice_id, amount_usd, status, created_at) VALUES ('connect-review-attempt', ?, 'in_connect_review', 5, 'pending', ?)",
            )
    ...
    if (setup.listing === "queued")
        await db
            .prepare(
                "UPDATE community_endpoint SET pending_visibility = 'public', pending_payload = payload, pending_at = ? WHERE id = 'connect-review-model'",
            )
            .bind(now + 10800)
            .run();
    if (setup.listing === "endpoint-agent")
        await db
            .prepare(
                "UPDATE community_endpoint SET type = 'endpoint_agent', base_url = 'https://connect-review.invalid/v1/chat/completions', payload = '{\"api\":\"chat_completions\",\"perUserRpm\":null}' WHERE id = 'connect-review-agent'",
            )
            .run();
    ...
    if (setup.payment === "failed")
        await db
            .prepare(
                "INSERT OR REPLACE INTO stripe_auto_top_up_attempt (id, user_id, amount_usd, status, failure_reason, created_at) VALUES ('connect-review-attempt', ?, 5, 'failed', 'Your card was declined.', ?)",
            )
            .bind(USER_ID, Date.now())
            .run();
    if (setup.rewards) {
