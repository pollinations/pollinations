...
beforeAll(async () => {
    ...
    expect((await request("/__connect/reset", {})).status).toBe(200);
...
async function beginSignIn(decision = "continue") {
    ...
    expect(new URL(url).pathname).toBe("/__connect/identity");
...
test("local provider failure is consumed once and the real callback succeeds on retry", async () => {
    ...
    // A new Connect view must restore this real session, not a stale fixture token.
