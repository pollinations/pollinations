...
if (!startArgument || !endArgument || !outputArgument) {
    throw new Error(
        "Usage: node collect-anthropic-cost-report.mjs <YYYY-MM-DD> <YYYY-MM-DD> <output.json> [group-by-csv]",
    );
}
...
for (let cursor = start; cursor < end; ) {
    ...
    if (!response.ok) {
        throw new Error(
            `Anthropic cost report failed for ${cursor.toISOString()}..${windowEnd.toISOString()}: HTTP ${response.status}`,
        );
    }
    windows.push({
