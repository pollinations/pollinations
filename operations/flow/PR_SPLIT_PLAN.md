...
**This section supersedes the historical plan below.** This development tool is now named **Pollinations Flow** and lives in `operations/flow`; **Pollinations Connect** remains the product login feature. The approved Git branch/workspace keep their existing names. The goal is to ship a usable, hosted review tool that faithfully runs current `main`, while building an evidence-backed backlog of improvements to the product. Flow must be useful before those improvements are implemented. The SDK, dashboard and Device reconciliation has runtime evidence. Clean-checkout startup, built-page serving without Vite and Linux container execution are verified. Configurable public origins and visible source revisions are implemented; local runtime/build checks pass. Linux browser/isolation checks pass with runtime-configured origins and capture provenance. Hosted reviewer access/routing, cross-host session cleanup and remote deployment remain open.

### Public-origin and provenance checkpoint — 26 September

- **Hosted access policy:** any authenticated Pollinations account can use Flow, including the first hosted preview. This supersedes the earlier admin-only preview decision and is the ongoing policy. Each visitor gets isolated review data; signing in to Flow does not grant access to production admin apps or their data.
- Real admin apps retain their existing server-side admin authorization. Flow's Admin example runs the real auth/UI code against its isolated fixture issuer and data, so any Flow visitor can inspect both allowed and denied simulated situations. Fixture roles never become production privileges, and real reviewer tokens must not enter the simulated runtime.
- Shared dashboard auth currently requires `role === "admin"` in both login and session validation (`packages/auth/src/server.ts`). Reuse its OAuth/session machinery with an explicit authenticated-account policy for Flow; preserve the admin requirement for existing consumers. Verify ordinary users can enter Flow, cannot enter real admin apps, and cannot read another reviewer's environment. Do not duplicate auth or globally remove the admin check.
...
- Local verification: 249 default tests passed, 136 opt-in checks skipped; typecheck/build passed (existing large-chunk warning). Account settings renders in Screens after Vite dependency optimization finished. [Linux run 36259531016](https://github.com/pollinations/pollinations/actions/runs/36259531016) passes at `677d59b123`: the same image works with localhost and 127.0.0.1 origins, captures retain source metadata, and two containers isolate wallet/fault/reset/restart/replacement. Follow-up `a4496278b2` additionally checks the built Admin page and config explicitly; its CI is separate.
- **Next gate:** reviewer auth and per-reviewer container routing under the all-accounts policy above. Shared auth currently owns `/auth/*` and fixed cookie names, which collide with the real Admin example on its origin. Do not hide that with cookie/header rewriting or copy auth logic into Flow. Resolve the namespace/boundary explicitly before remote exposure. Protect both browser origins and all fixture controls, and verify one reviewer cannot address another reviewer's runtime.
- **Cross-host session gate:** the current fixture-condition response emits Admin logout cookies on the Enter response. That works on localhost with different ports, but cannot clear cookies belonging to a different hostname. Execute the real logout on the Admin origin and prove role changes, Start over and reset after a signed-in Admin journey. The packaged smoke checks do not issue an Admin session, so their pass is not evidence for this case.
...
- Clean-copy installation, build, startup, browser navigation, fonts and an error capture pass in both development and built-page mode. The Linux image now passes the browser checks and isolates data, faults, resets, restart and replacement across two containers. Configurable origins and visible source revisions are covered by the later checkpoint above. Hosted access/routing and cross-host session cleanup remain open. No remote deployment is included yet.

Local startup from the repository root: `npm ci`, then `npm run dev --workspace=pollinations-flow`. The predev hook installs Chromium; startup generates the shared UI CSS and fonts. Open `http://localhost:4180/flow`; local Worker transport uses 4181 and Admin uses 4182. To run another checkout alongside it, set `FLOW_PORT=4280` for dev and reset; the three ports become 4280–4282. Browser origins default to those local ports and can be configured separately with `FLOW_ENTER_ORIGIN` and `FLOW_ADMIN_ORIGIN`. `npm run reset --workspace=pollinations-flow` explicitly resets this tool's local fixture account/data; it is not needed merely to open an existing running review. To serve the generated pages without Vite, run `npm run build --workspace=pollinations-flow`, then `npm run start --workspace=pollinations-flow`, with the desired `FLOW_PORT` when starting it; browser origins are injected at runtime. This still runs locally from the checkout and uses the same real Workers and capture service. Local databases, generated assets and dependencies are ignored by Git.
...
**Next:** implement the smallest authenticated reviewer-to-container routing using existing repository conventions, with distinct reviewer auth routes/cookies, and finish the cross-host session cleanup check. Public origins, private asset transport and source/capture provenance are now implemented. Then deploy a separate Cloudflare preview through GitHub Actions and verify its real URL, callback/cookies, capture/recovery and reviewer isolation. The PR remains draft until those hosting gates are met.
