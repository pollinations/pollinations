...
- SDK and dashboard reconciliation is complete for the declared inventory. Four Device approval rows remain source-only; separately scoped approval is pending. This is not proof of every possible product state.
...
- Use the SDK's built-in `sessionStorage`; the custom adapter flagged by CodeQL #274 is removed. Start over clears only this app's SDK namespace. The remote CodeQL result still needs confirmation after pushing.
