...
| Product baseline | `1b7e62106623946fae1f9f454946cef73aa3f481` — fast-forwarded to fetched main on 26 September; capture evidence remains tied to this or the earlier recorded baseline |
| Latest remote main observed | `279edfad8fd9fdbeeedf77047dd72a77a968fce7` — four newer commits fetched before the draft checkpoint; not integrated or verified yet |
...
- Reconcile the inherited tests that still import unmerged product helpers, including `test/runtime.test.ts` and `api-error.ts`. A passing focused suite is not a passing full suite; do not restore the old product implementation to satisfy these tests.
- Integrate the four newer main commits, rerun affected checks, and preserve the old evidence baseline. The new commits concern model retirement announcements, Azure GPT-6, infrastructure documentation and Gemini environment configuration; Flow must continue to use isolated fixtures instead of importing real bindings.
- Prove a clean-checkout startup and the packaged/hosted runtime, including isolated concurrent review sessions, configurable origins and visible source revisions. No deployment configuration or remote deployment is included yet.

Local startup from the repository root: `npm install`, then `npm run dev --workspace=pollinations-flow`. The predev hook installs Chromium. Open `http://localhost:4180/flow`; local Worker transport uses 4181 and Admin uses 4182. `npm run reset --workspace=pollinations-flow` explicitly resets this tool's local fixture account/data; it is not needed merely to open an existing running review. Local databases, generated assets and dependencies are ignored by Git.
...
The active inventory has **311 scoped entries**, **272 distinct situation IDs**, and **16 flow sections**. The ledger retains **317 rows**: 311 active situations plus six deferred historical checkout-status cases absent from main. Eight code-agent, six wallet/billing, four Account settings, three Quests cases and the missing Admin cancellation state were added from current main. Repetition across entry paths is intentional; the identity is `(flow, section, situation)`. This inventory is not proof that every screen on today's main is covered.

The complete [scenario ledger](./scenario-audit.csv) retains every entry. Its evidence IDs refer to the findings below. Four App states were observed in the browser journey; **252 situations are captured**: 25 App and 54 Device at `d3e84e005d`, plus 22 dashboard Keys/Apps, 55 Models/Agents, 42 Wallet/Billing, 38 Account/Quests and 16 Admin at `1b7e621066`, all on 26 September. **55 active rows remain source-audit-only**; six checkout-status rows are explicitly deferred. The 42 wallet captures cover 20 dashboard situations at desktop/light and 22 standalone Top-up situations at mobile/dark. Captured output records what main renders, not proof that a product gap is fixed. App, Keys and Models captures use mobile/dark; Device covers manual entry in mobile/dark and linked entry in desktop/light; Apps, Agents and Quests use desktop/light; Account settings uses mobile/dark. Credential-revealing success cases remain separately gated. Admin signed-out captures use mobile/dark; its three signed-in cases use desktop/light and passed with scoped approval for disposable OAuth tokens/sessions.

| Disposition | Entries | Meaning |
| --- | ---: | --- |
| `adapt-to-main` | 239 | Keep the situation, adapt the harness to the current owner code, and verify its actual output. This is not a pass result. |
...
| C02 | **Keys/Apps/Models/Agents reconciled:** shared routes and section anchors, current dialogs, controls, empty/loading states, pending/error actions and recovery. Observation follows visible dialogs and actual list returns. Eight code-agent situations cover creation, editing and sync without deploying code. Remaining dashboard sections still need reconciliation. | `flow-dashboard.ts`, `flow-dashboard-driver.ts`, `runtime-frame.tsx`, `review-dashboard.ts`; main `/keys` and `/my-models` owners. Seventy-seven captures and 23 recovery checks at `1b7e621066`; key reveal and successful code deployment are not verified. |
...
| G06: key lifecycle after rejected handoff | Source evidence; Device key-issuing checks still await scoped approval | Known rejection versus ambiguous response; Enter approval/handoff transaction and key ownership |
| G01/G02/G04: endpoint errors and recovery | App/Device captures show lost distinctions and available actions; approval-envelope behavior remains source-only | Check real endpoint envelopes and injected transport faults separately; Enter routes/components |
...
| G07: key editor | Source evidence, pending reconciliation | Actual standalone loads and recovery; Enter owner |
...
Prove this with one packaged runtime before building more hosting infrastructure:
...
- Run one ordinary journey and one error situation through the packaged runtime, including direct navigation, sign-in callback, cookies, captures and restart. Audit environment-dependent behavior: `ENVIRONMENT=test`, local provider fixtures and the current forced development build mode are not production-environment parity and must not be hidden.
- Give independent review sessions separate runtime state. The current single server shares its database and fault settings; publishing that unchanged lets one tester reset another. Start by testing one container/runtime per review session, reusing the existing runtime instead of adding product-level database partitioning. Verify two simultaneous sessions, captures and reset isolation, and disposable state after restart. Measure resource requirements before selecting instance limits.
...
Do not rewrite Enter/Gen to fit a static host or add a second scenario backend. If the Container proof fails, identify the concrete runtime limit before choosing another Node host. Hosting is now part of the delivery goal; no remote deployment or infrastructure changes have been made yet.
