...
export const dashboardScreens: CanvasScreen[] = [
    ...
    },
    {
        ...
        owner: "Pollinations",
        screen: "login-failed",
        variants: Object.values(loginSituations).map((error) => ({
            label: error.title,
            screen: error.id,
            params: {
    ...
    },
    {
        ...
        variants: [
            ...
            { label: "Loading catalog", params: { quest_catalog: "loading" } },
            {
                label: "Catalog unavailable",
                params: { quest_catalog: "error" },
            },
            {
                label: "Rewards session expired",
                params: { quest_rewards: "session-expired" },
            },
            {
    ...
    ...resourceScreens("agents", "agent", "Your agents", "agent"),
    {
        ...
        variants: [
            { label: "Account settings" },
            {
                ...
                params: { settings_case: "profile-loading" },
            },
            {
                label: "Profile unavailable",
                params: { settings_case: "profile-error" },
            },
            {
                label: "Signing out",
                params: { action: "sign-out", result: "waiting" },
            },
            {
                label: "Sign-out failed",
                params: { action: "sign-out", result: "error" },
            },
            {
...
const collections = {
    keys: { prefix: "key", path: "/keys", anchor: "api-keys" },
    apps: { prefix: "app", path: "/keys", anchor: "app-keys" },
    models: { prefix: "model", path: "/my-models", anchor: "models" },
    agents: { prefix: "agent", path: "/my-models", anchor: "agents" },
} as const;
            ...
            variants: [
                ...
                ...(section === "agents"
                    ? [
                          {
                              label: "Code agent",
                              params: { agent_kind: "code" },
                          },
                          {
                              label: "Creating code agent",
                              params: {
                                  agent_kind: "code",
                                  action: "create",
                                  result: "waiting",
...
export function dashboardRouteForPreview(screen: string): string | undefined {
    ...
    for (const [section, { path, anchor }] of Object.entries(collections))
        if (
            screen === `dash-${section}` ||
...
export function dashboardScreenForLocation(
    path: string,
    dialogTitle = "",
    hash = "",
) {
    ...
    const sections = Object.entries(collections).filter(
        ([, entry]) => path === entry.path,
    );
    if (sections.length) {
        // Main groups keys/apps and models/agents on shared pages. A real
        // dialog identifies its resource; otherwise use the section anchor.
        const resource = dialogTitle
            ? path === "/keys"
                ? /app key/i.test(dialogTitle)
                    ? "apps"
                    : "keys"
                : /agent/i.test(dialogTitle)
                  ? "agents"
                  : "models"
            : undefined;
        const [section, { prefix }] =
            sections.find(([id, entry]) =>
                resource ? id === resource : hash === `#${entry.anchor}`,
            ) ?? sections[0];
        const action = /^(Create|Add|Register)|key created$/i.test(dialogTitle)
            ? "create"
            : /^Edit/.test(dialogTitle)
              ? "edit"
              : /^Delete/.test(dialogTitle)
                ? "delete"
                : /^(Unlist|Relist)/.test(dialogTitle)
                  ? "visibility"
                  : null;
                ...
                },
                {
                    ...
                    label:
                        action === "create" && ["keys", "apps"].includes(id)
                            ...
                            : ["models", "agents"].includes(id) &&
                                ["delete", "visibility"].includes(action)
                              ? "Confirm / Cancel"
                              : "Complete / Cancel",
                },
            ]),
