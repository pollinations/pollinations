...
import type { JourneySection } from "./flow-journey-state";
...
const states: DeviceState[] = [
    ...
    ...[
        loginSituations.default,
        loginSituations.banned,
        loginSituations.staging,
    ].map((error, index) => ({
        ...state(
            error.id,
            "sign-in-errors",
            error.id,
            error.id === "login-failed" ? "Returning from GitHub" : error.title,
            index + 1,
            2,
    ...
    ...deviceRequestResults
        ...
        .map((result, index) =>
            state(
                ...
                result.id === "used" ? "consent" : "device-errors",
                "device-consent",
                result.label,
                6 + index,
                3,
    ...
    ...deviceSubmitResults.slice(1).map((result, index) =>
        state(
            ...
            result.id === "deny" ? "device-result" : "device-errors",
            "device-consent",
            result.label,
            9 + index,
            2,
...
const deviceNodes: FlowNode[] = [
    ...
    },
    {
        id: "device-waiting",
        kind: "outcome",
        label: "Device still waiting",
        note: "Decline failed; browser reports success",
        x: 4170,
        y: 1580,
    },
    {
...
const deviceEdges: FlowEdge[] = [
    ...
    edge("error", "device-signing-in", "Sign in with GitHub"),
    ...
    ...deviceCodeResults
        ...
        .flatMap((result) => [
            ...
            edge(`device-code-${result.id}`, "device-verifying", "Continue"),
            edge(`device-code-${result.id}`, "device-code", "Edit code"),
        ]),
    ...
    ...deviceRequestResults
        .slice(1)
        .filter((result) => result.id !== "used")
        .map((result) =>
            edge(`device-request-${result.id}`, "device-denying", "Decline"),
        ),
    edge("device-request-used", "device-approving", "Allow access"),
    edge("device-request-used", "device-denying", "Decline", true),
    edge("consent", "device-approving", "Allow access"),
    edge("consent", "device-denying", "Decline", true),
    {
    ...
    edge(
        "device-denying",
        "device-submit-deny",
        "Decline failed; success still shown",
        true,
    ),
    ...["key", "approve", "session"].map((reason) =>
        edge(`device-submit-${reason}`, "device-denying", "Decline"),
    ),
    edge("device-submit-deny", "device-waiting", "Device still waiting", true),
    edge("device-waiting", "device-stopped", "Code expires", true),
...
const deviceErrorGroups = [
    ...
    },
    {
        ...
        ids: ["device-declined", "device-submit-deny"],
    },
];
