...
export function ScreenViewer({
    ...
    return (
        <section className="flow-screen-viewer" aria-label={title}>
            <div className="flow-screen-caption">
                <strong>{title}</strong>
...
export function ScreenOwnership({ entry }: { entry: CanvasScreen }) {
    return (
        <small className="flow-screen-owner">
...
export function ScreenWindow({
    ...
    const content = (
        <div
            className={`flow-screen-window ${terminal ? "flow-terminal-window" : "flow-browser-window flow-screen-viewport"}`}
        >
            {terminal && (
                <div className="flow-window-bar">
                    <span aria-hidden="true">&gt;_</span>
                    <strong>App terminal</strong>
                    <span className="flow-window-owner">· Developer</span>
                </div>
            )}
            <div className="flow-window-content">{children}</div>
        </div>
    );
    return terminal ? (
        <div className="flow-terminal-device flow-screen-viewport">
            {content}
        </div>
    ) : (
        content
...
export function DeviceTerminal({
    ...
    return (
        <div className="flow-terminal-output">
            <pre>
                {!done
            ...
            {!done && (
                <>
                    {onOpen ? (
                        ...
                        <span className="flow-terminal-link">{url}</span>
                    )}
                    {(device || !onOpen) && (
                        <pre>
                            {device
...
export function ScreenContent({
    ...
    if (!focused)
        return (
            <ScreenWindow entry={entry}>
                <div className="flow-route-reference">
