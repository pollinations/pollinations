...
async function bundleWorker(service: string) {
    // Read only execution settings. Bindings and data remain isolated below;
    // loading a product config must never connect Flow to its remote resources.
    const config = readWorkerConfig({
        config: path.join(repository, service, "wrangler.toml"),
    });
...
async function migrate(db: D1Database) {
    await db
        .prepare(`CREATE TABLE IF NOT EXISTS flow_migrations
    ...
    for (const migration of migrations) {
        const applied = await db
            .prepare("SELECT name FROM flow_migrations WHERE name = ?")
            .bind(migration.name)
            .first();
        if (applied) continue;
        await db.batch([
            ...
            db
                .prepare("INSERT INTO flow_migrations (name) VALUES (?)")
                .bind(migration.name),
        ]);
    }
    await db
        .prepare(`CREATE TABLE IF NOT EXISTS flow_conditions
        (id INTEGER PRIMARY KEY CHECK (id = 1), account TEXT NOT NULL,
         pollen TEXT NOT NULL, allowance TEXT NOT NULL,
         role TEXT NOT NULL DEFAULT 'member')`)
        .run();
    const columns = await db
        .prepare("PRAGMA table_info(flow_conditions)")
        .all<{ name: string }>();
    if (!columns.results.some((column) => column.name === "role")) {
        await db
            .prepare(
                "ALTER TABLE flow_conditions ADD COLUMN role TEXT NOT NULL DEFAULT 'member'",
            )
            .run();
    }
    await db
        .prepare(`CREATE TABLE IF NOT EXISTS flow_device
...
export async function startRuntime(
    ...
    const common = {
        bindings,
        d1Databases: { DB: "flow-local-isolated-db" },
        kvNamespaces: { KV: "flow-local-isolated-kv" },
        ...
        workers: [
            {
                ...common,
                name: "flow-enter",
                compatibilityDate: scripts.enter.compatibilityDate,
                compatibilityFlags: scripts.enter.compatibilityFlags,
                serviceBindings: { COMPOSIO_MCP: reviewServices.integrations },
                modules: [
                    {
                        type: "ESModule",
                        path: "enter.mjs",
                        contents: scripts.enter.code,
                    },
                ],
            },
            {
                ...common,
                name: "flow-gen",
                compatibilityDate: scripts.gen.compatibilityDate,
                compatibilityFlags: scripts.gen.compatibilityFlags,
                modules: [
                    {
                        type: "ESModule",
                        path: "gen.mjs",
                        contents: scripts.gen.code,
                    },
                ],
                serviceBindings: { ENTER: "flow-enter" },
    ...
    try {
        console.log("Flow: starting isolated Workers");
        let db = (await mf.getD1Database("DB", "flow-enter")) as D1Database;
        console.log("Flow: applying local database migrations");
        await migrate(db);
        await syncLocalIdentity(db);
        let enter = await mf.getWorker("flow-enter");
        let gen = await mf.getWorker("flow-gen");
        ...
        async function fetch(request: Request): Promise<Response> {
            const url = new URL(request.url);
            if (url.pathname.startsWith("/__flow/")) {
                ...
                try {
                    ...
                    let establishSession =
                        url.pathname === "/__flow/state" &&
                        request.method === "GET";
                    let clearAdminSession = false;
                    const initialized = await db
                        .prepare("SELECT id FROM flow_conditions WHERE id = 1")
                        .first();
                    if (!initialized && url.pathname !== "/__flow/reset") {
                        return Response.json(
                            {
                                error: "Run the local fixture reset to initialize Flow",
                            },
                            { status: 409 },
                        );
                    }
                    if (url.pathname === "/__flow/identity")
                        return localProvider.handoff(request);
                    if (
                        url.pathname === "/__flow/review/requests" &&
                        request.method === "GET"
                    )
                    ...
                    if (url.pathname === "/__flow/outcome") {
                    ...
                    if (
                        url.pathname === "/__flow/reset" &&
                        request.method === "POST"
                    ) {
                        const kv = await mf.getKVNamespace("KV", "flow-enter");
                        ...
                        url.pathname === "/__flow/conditions" &&
                        request.method === "POST"
                    ) {
                        const patch = {
                        ...
                        url.pathname === "/__flow/review/prepare" &&
                        request.method === "POST"
                    ) {
                        ...
                        if (setup.device) {
                            const started = await fetch(
                                new Request(
                                    `${ENTER_ORIGIN}/__flow/device/start`,
                                    { method: "POST" },
                                ),
                            );
                            if (!started.ok)
                            ...
                            if (setup.device === "missing-app")
                                await db
                                    .prepare(
                                        "UPDATE device_code SET client_id = 'pk_flow_missing_app' WHERE device_code = (SELECT device_code FROM flow_device WHERE id = 1)",
                                    )
                                    .run();
                            if (setup.device === "expired")
                                await db
                                    .prepare(
                                        "UPDATE device_code SET expires_at = ? WHERE device_code = (SELECT device_code FROM flow_device WHERE id = 1)",
                                    )
                            ...
                            if (setup.device === "used")
                                await db
                                    .prepare(
                                        "UPDATE device_code SET status = 'denied' WHERE device_code = (SELECT device_code FROM flow_device WHERE id = 1)",
                                    )
                                    .run();
                        }
                    } else if (
                        url.pathname === "/__flow/device/start" &&
                        request.method === "POST"
                    ) {
                        ...
                        await db
                            .prepare(`INSERT OR REPLACE INTO flow_device
                        ...
                        url.pathname === "/__flow/device/poll" &&
                        request.method === "POST"
                    ) {
                        const device = await db
                            .prepare(
                                "SELECT device_code, status FROM flow_device WHERE id = 1",
                            )
                        ...
                        if (device.status !== "completed") {
                            ...
                            if (response.ok && token.access_token) {
                                ...
                                await db
                                    .prepare(
                                        "UPDATE flow_device SET status = 'completed' WHERE id = 1 AND device_code = ?",
                                    )
            ...
            reload(scripts: WorkerScripts) {
                ready = ready
                    ...
                    .then(async () => {
                        ...
                        await migrate(db);
                        enter = await mf.getWorker("flow-enter");
                        gen = await mf.getWorker("flow-gen");
                    });
                return ready;
            },
            dispose: async () => {
