import { dashboardRouteForPreview } from "./flow-dashboard";
...
export function screenRoute(
    ...
    if (query.get("account_case") === "checkout-return") {
        walletSearch.set("stripe_success", "true");
        walletSearch.set("session_id", "cs_flow_review");
    }
    ...
    if (screen === "dashboard-sign-in" || screen === "dashboard-connected") {
        ...
        if (error && Object.hasOwn(adminSignInSituations, error))
    ...
    if (screen.startsWith("account-key")) {
        const params = new URLSearchParams({
            ...
            redirect: `${origin}/flow-example.html`,
        });
    ...
    if (screen.startsWith("device")) {
        ...
        if (screen.startsWith("device-consent"))
            return `/authorize?${new URLSearchParams({
                user_code: code,
                ...(state.device?.clientId && {
                    app_key: state.device.clientId,
                }),
            })}`;
    ...
    if (screen.startsWith("oauth") || screen.startsWith("direct")) {
        const params = new URLSearchParams({
            ...
            redirect_uri: `${origin}/flow-example.html`,
            response_type: "code",
        ...
        if (invalid === "app") params.set("client_id", "pk_flow_missing_app");
