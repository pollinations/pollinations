...
import { canvasScreenUrl } from "../flow-canvas-data";
    ...
    it.each([
        ...
        for (const edge of flow.map.edges) {
            ...
            expect([
                ...
                "Enter another code",
                "Cancel",
            ]).not.toContain(edge.label);
        }
        const error = flow.map.nodeForState("device-request-invalid");
        for (const id of [
            ...
            expect(flow.map.nodeForState(id)).toBe(error);
        expect(flow.map.edges.filter((edge) => edge.from === error)).toEqual([
            expect.objectContaining({ to: "consent", label: "Decline" }),
        ]);
        expect(flow.map.nodeForState("device-request-used")).toBe("consent");
        expect(flow.map.nodeForState("device-submit-deny")).toBe(
            "device-declined",
        );
        expect(flow.edges).toContainEqual(
            expect.objectContaining({
                from: "device-submit-deny",
                to: "device-waiting",
            }),
        );
    });
