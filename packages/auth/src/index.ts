...
export function createPollinationsAuth(config: PollinationsAuthConfig) {
    ...
    async function finishLogin(request: Request) {
        ...
        if (
            ...
            !token.access_token.startsWith("oauth_")
        ) {
            return authError(
                "Dashboard login denied or expired. Please sign in again.",
                tokenResponse.status === 400 ? 403 : 502,
                clearFlow,
            );
        }
    ...
    async function handle(request: Request): Promise<Response | null> {
        ...
        if (url.pathname === LOGOUT_PATH) {
            // Do not redirect to the protected root: it would immediately sign
            // the user back in while their separate Enter session still exists.
            return new Response(
