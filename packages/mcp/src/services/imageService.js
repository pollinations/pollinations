...
async function fetchMedia(url, timeoutMs, expectedMediaType) {
    ...
    if (!contentType.startsWith(`${expectedMediaType}/`)) {
        await response.body?.cancel();
        throw new Error(
            `Expected ${expectedMediaType} response, received ${contentType}`,
        );
    }
...
const imageParamsSchema = z
    .object({
        ...
        model: z
            ...
            .optional()
            .describe(
                "Image model or alias. Use listModels for the live list.",
            ),
        width: z
            .number()
            .int()
            .optional()
            .describe(
                "Image width in pixels; support and defaults vary by model",
            ),
        height: z
            .number()
            .int()
            .optional()
            .describe(
                "Image height in pixels; support and defaults vary by model",
            ),
        seed: z
            .number()
            .int()
