...
```
The config file contains `systemPrompt`, `baseModel`, and optional `mcpServers` or `optillm`; create also requires `--name` and `--title` for the callable model listing. OptiLLM accepts the bounded `re2`, `cot_reflection`, `bon`, `mcts`, and `rstar` approaches and cannot be combined with MCP tools. Updates replace the complete agent configuration.
