...
```
Defaults: `zimage`, 1024x1024. Pick a different model with `--model flux` (see `polli models --type image`). For edits / img2img, pass one or more `--image <url>` flags — **must be public http(s) URLs**, local paths are rejected client-side. **Only models that list `"image"` in `input_modalities` actually consume the flag** — `flux` and `zimage` are text-only and will silently ignore `--image`. Find i2i-capable models with `polli models --type image --json | jq -r '.[] | select(.input_modalities | contains(["image"])) | .name'` (common choices: `nanobanana`, `kontext`, `p-image-edit`). To use a local file, upload it first with `polli upload` (see next recipe). **Img2img/edit models can treat `--width`/`--height` as a hint anchored to the source image's aspect rather than an authoritative output size** — always check the actual output dimensions (`file`, `ffprobe`) before trusting the flags, and if an exact aspect matters, pre-crop the source to that aspect or enforce it in a downstream crop/scale step rather than relying on the generation call alone.
...
- **Backgrounding a `polli gen` call can fail silently.** A backgrounded generation can finish with no output file and an empty job log — a transient failure, not necessarily a content-safety rejection (which returns an explicit 400). Prefer running `polli gen` calls in the foreground; if one must run backgrounded, verify the expected `--output` file exists once the job returns, and retry once in the foreground before concluding it failed.
- **Content-safety rejections aren't only about violent language.** A `400 content_safety_violation` can trip several provider categories, not just violence — e.g. body-focused action phrasing can land in a sexual-content category on some image-edit models. If an edit/i2i prompt is rejected, don't assume only violent wording needs softening; rephrase the benign intent in neutral terms (e.g. sport/activity framing) and retry with a new seed.
