...
| Machine-readable output | append \`--json\` or \`--yaml\` or \`--csv\` |
...
- **\`--json\` / \`--yaml\` / \`--csv\`:** machine-parseable output to stdout; all human messages go to stderr. **Always prefer structured output when parsing** — it's the only shape with a stable contract.
...
2. **Prefer structured output (\`--json\`, \`--yaml\`, \`--csv\`)** whenever you'll parse the output. Never grep human-formatted tables.
...
writeFileSync(join(rootDir, "SKILL.md"), skillContent, "utf-8");
