// Public read-only Tinybird token, scoped to the model_health pipe.
...
export async function fetchModelStats(
    ...
    const url = `https://api.europe-west2.gcp.tinybird.co/v0/pipes/model_health.json?token=${TOKEN}&minutes=${minutes}`;
