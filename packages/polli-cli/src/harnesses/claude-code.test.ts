...
// Status checks key liveness through the Pollinations API; keep unit tests
// offline and deterministic.
vi.mock("./keys.js", async (importOriginal) => {
    const mod = await importOriginal<typeof import("./keys.js")>();
    return {
        ...mod,
        keyIsValid: async (key: string) => key.startsWith("sk_valid"),
    };
});
...
const settings = { apiKey: "sk_valid_dedicated", model: models[0].id, models };
...
describe("claude-code adapter", () => {
    ...
    it("asks for a running router instead of starting one", async () => {
        ...
        expect(status.details?.next).toContain("ccr start");
    });
    ...
    it("off restores the pre-on config byte-for-byte when untouched", async () => {
    ...
    it("off removes only Pollinations-owned entries after outside edits", async () => {
        await configureClaudeCode(ctx, settings);
    ...
    it("reports unconfigured when the provider key is missing", async () => {
        await configureClaudeCode(ctx, settings);
        config.Providers = config.Providers.filter(
            (p) => p.name !== "pollinations",
        );
        expect((await claudeCode.status(ctx)).configured).toBe(false);
    });
});
