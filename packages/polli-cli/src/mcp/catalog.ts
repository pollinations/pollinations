...
/**
 * Fetch the live MCP catalog. Nothing is hardcoded in the CLI: the gateway
 * decides which servers exist and which URL each one is served at, so new
 * servers appear without a CLI release.
 */
...
/** Resolve requested server ids against the catalog; empty means all. */
