...
export interface McpClientAdapter {
     ...
     * orphan. Null when the client stores the key elsewhere (VS Code secret
     * storage) or none was written yet.
     */
...
const bearerHeader = (key: string) => ({ Authorization: `Bearer ${key}` });
...
interface JsonTarget {
    ...
    /** Extra config merged alongside entries (e.g. VS Code inputs). */
...
const jsonClient = (adapter: {
    ...
    return {
            ...
            const { installed, file } = update(ctx, (config, table) => {
                for (const server of servers) {
                    ...
                    if (existing !== undefined && !isOwnedEntry(existing)) {
                        // Never clobber a non-Pollinations server that happens
                        // to share the id: the user keeps their own config.
        ...
        existingKey: (ctx) =>
            recoverKeyFromTable(readJsonObject(target.file(ctx))[target.table]),
    };
};
...
const configHome = (ctx: McpContext) =>
...
const vscodeUserDir = (ctx: McpContext): string => {
    ...
    return join(configHome(ctx), "Code", "User");
};
...
const jsonClients: McpClientAdapter[] = [
    ...
    jsonClient({
        ...
        target: {
            file: (ctx) => join(configHome(ctx), "opencode", "opencode.json"),
            table: "mcp",
            entry: (server, key) => ({
                type: "remote",
                url: server.url,
    ...
    jsonClient({
        ...
        target: {
            file: (ctx) => join(configHome(ctx), "zed", "settings.json"),
...
const cliClient = (adapter: {
    ...
    const ensureCli = (ctx: McpContext) => {
        if (!commandExists(target.command, ctx.env)) {
            throw new Error(
...
const cliClients: McpClientAdapter[] = [
    ...
    cliClient({
        ...
        target: {
            ...
            installedIds: jsonTableIds(
                (ctx) => join(configHome(ctx), "amp", "settings.json"),
                "mcpServers",
                "amp",
            ),
            recoverKey: (ctx) => {
                const config = readJsonObject(
                    join(configHome(ctx), "amp", "settings.json"),
                );
                const amp =
...
// Claude Code, Codex CLI, VS Code / Copilot, Cursor, OpenCode, Gemini CLI,
// Copilot CLI, Windsurf, Cline, Amp, Kiro, Zed, Warp.
const PRIORITY = [
    "claude-code",
    "codex",
    "vscode",
