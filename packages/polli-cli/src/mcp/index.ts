...
import { ownedNames, serverIdOf } from "./entries.js";
import type {
    McpClient,
    McpInstallOptions,
    McpResult,
...
export const installMcp = async (
    ...
    const hints = client.postInstall?.(ctx, apiKey) ?? [];
    if (client.secretMode === "input") {
        hints.push(`Pollinations key for the prompt input: ${apiKey}`);
    }
    return statusMcp(ctx, clientId);
};
...
export const offMcp = async (
    ...
    const hints = removed.length > 0
        ? (client.postStrip?.(ctx, removed) ?? [])
        : [];
