...
const serversMap = (
    ...
    config && isMap(config[envelopeKey]) ? (config[envelopeKey] as Record<string, unknown>) : {};
 ...
 * entries (owned = URL under BASE_URL), preserve the rest of the config.
 */
export const jsonClient = (spec: JsonClientSpec): McpClient => {
    const path = (ctx: HarnessContext) => spec.configPath(ctx);
    const withServers = (
        ctx: HarnessContext,
        mutate: (servers: Record<string, Record<string, unknown>>) =>
        ...
        writeTextAtomic(path(ctx), `${JSON.stringify(config, null, 2)}\n`, 0o600);
        return next;
    };
    return {
        id: spec.id,
        ...
        installedServers: (ctx) => {
            ...
            return Object.entries(servers)
                .filter(([name, entry]) => isOwnedEntry(name, entry, spec.urlField))
            ...
            spec.secretMode === "input"
                ...
                : (ctx) => {
                      ...
                      for (const entry of Object.values(servers)) {
                          if (!isMap(entry) || !isOwnedEntry("", entry, spec.urlField))
                              continue;
