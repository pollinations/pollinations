...
async function callModel(model, prompt, apiKey) {
    ...
    return {
        ...
        prompt,
        latencyMs,
        reply: data.choices?.[0] ? data.choices[0].message.content : null,
...
async function main() {
    ...
    for (const model of models) {
        ...
        const avgTokens =
            rows.reduce(
                (sum, r) => sum + (r.usage ? r.usage.total_tokens || 0 : 0),
                0,
            ) / rows.length;
        console.log(
            `${model}: ${avgLatency.toFixed(0)}ms avg, ${avgTokens.toFixed(0)} tokens avg`,
        );
    }
