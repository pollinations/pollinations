...
function main() {
	...
	for (const line of lines) {
		...
		const assistantContent = JSON.stringify(
			record.action ? { say: record.say, action: record.action } : { say: record.say },
		);
	...
	out.end(() => {
		console.log(`Wrote ${written} training examples to ${args.out} (skipped ${skipped} malformed lines).`);
	});
}
