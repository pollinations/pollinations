...
async function fetchWithTimeout(
    ...
    const abortHandler = externalSignal ? () => controller.abort() : undefined;
...
export class Pollinations {
    ...
    constructor(config: PollinationsConfig = {}) {
        ...
        this.imageTimeout =
            config.imageTimeout ??
            (config.timeout ? config.timeout : DEFAULT_IMAGE_TIMEOUT);
        this.videoTimeout =
            config.videoTimeout ??
            (config.timeout ? config.timeout : DEFAULT_VIDEO_TIMEOUT);
    }
    ...
    private buildImageUrl(
        ...
        const params: Record<string, unknown> = {
            model: options.model || "tongyi-mai/z-image-turbo",
            width: options.width,
    ...
    private buildVideoUrl(
        ...
        const model = options.model || "google/veo-3.1-fast";
     ...
     * @example
     * ```ts
     * const url = await pollinations.videoUrl('A cat playing piano', { model: 'google/veo-3.1-fast', duration: 4 });
     * ```
     */
    async videoUrl(
        prompt: string,
     ...
     *
     * // Music generation
     * const { buffer } = await pollinations.audio('upbeat jazz', { model: 'elevenlabs/music-v2', duration: 30 });
     * ```
     */
    async audio(
        text: string,
    ...
    async audioSpeech(
        ...
        const body = {
            ...
            model: options.model || "elevenlabs/eleven-v3",
        };
     ...
     * const result = await pollinations.transcribe(audioBuffer, { model: 'openai/whisper-large-v3' });
    ...
    async transcribe(
        ...
        formData.append("model", options.model || "openai/whisper-large-v3");
