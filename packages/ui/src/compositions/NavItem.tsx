...
type BaseNavItemProps = {
    ...
    active?: boolean;
    /** Square left edge for navigation rows that touch the side of a rail. */
    flushLeft?: boolean;
...
const base =
    "polli-control polli:flex polli:items-center polli:gap-2 polli:px-3 polli:py-2 polli:text-left polli:text-sm polli:font-medium polli:whitespace-nowrap polli:transition-colors";
...
export function NavItem<T extends React.ElementType = "button">({
    ...
    return (
        <Component
            ...
            className={cn(
                base,
                flushLeft ? "polli:rounded-r-full" : "polli:rounded-full",
