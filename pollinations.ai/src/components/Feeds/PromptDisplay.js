...
export function PromptDisplay({
  ...
  const currentPrompt =
    sharedPrompt !== undefined ? sharedPrompt : localPrompt || getPromptFromItem()
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      if (
        !isMobile &&
        (e.target === resizeHandleRef.current || resizeHandleRef.current.contains(e.target))
      ) {
        isResizing = true
        startY = e.clientY
  ...
  return (
    <Box width="100%">
      {isEditMode && <Typography sx={LabelStyle}>{renderTooltipLabel()}</Typography>}
