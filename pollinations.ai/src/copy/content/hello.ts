// HelloPage content configuration
// Pure text - no AI transformations by default

export const HELLO_PAGE = {
    // Hero Section
    heroTitle: {
        text: "Open-source AI for people who make things.",
        transform: true,
    },

    heroIntro: {
        text: "A community-driven platform where developers, artists, and tinkerers build together. No gatekeeping, no corporate nonsense — just good tools and good people.",
        translate: true,
    },

    heroTagline: {
        text: "APIs built in the open. Loved by developers. Powered by community.",
        translate: true,
    },

    // What pollinations.ai Is
    whatIsTitle: {
        text: "What pollinations.ai Is",
        translate: true,
    },

    whatIsDescription: {
        text: "pollinations.ai is an open-source AI platform built by and for the community. We offer a unified API for images, text, audio, and video. Everything runs in the open: our code, our roadmap, our conversations. Hundreds of developers are already building tools, games, bots, and weird experiments with us. You're welcome to join.",
        translate: true,
    },

    whatIsTagline: {
        text: "No black boxes. No vendor lock-in. Just a friendly API and a Discord full of people who actually help each other.",
        translate: true,
    },

    // Pollen Section
    pollenTitle: {
        text: "Pollen",
        translate: true,
    },

    pollenDescription: {
        text: "Running AI models costs money. Pollen is how we keep the servers humming without ads or selling your data. One simple credit across all models — predictable, transparent, no surprises.",
        translate: true,
    },

    // Get Pollen Section
    getPollenTitle: {
        text: "How to Get Pollen",
        translate: true,
    },

    getPollenIntro: {
        text: "Two main paths:",
        translate: true,
    },

    buyCardTitle: {
        text: "Buy Pollen",
        translate: true,
    },

    buyCardDescription: {
        text: "Add Pollen to your wallet and build. Straightforward packs, no subscriptions, no locked-in tiers.",
        translate: true,
    },

    buyCardPromo: {
        text: "🎁 Buy 1, get 1 free during beta!",
        translate: true,
    },

    earnCardTitle: {
        text: "Contribute & Get Pollen",
        translate: true,
    },

    earnCardDescription: {
        text: "Ship an app, help in Discord, fix bugs, improve docs — every contribution earns you Pollen. We notice and we share.",
        translate: true,
    },

    // Earn Pollen: Sponsorship Tiers
    tiersSubtitle: {
        text: "Grow",
        translate: true,
    },

    tiersDescription: {
        text: "Start small, contribute, watch your Pollen grow:",
        translate: true,
    },

    tierSporeTitle: {
        text: "Spore — Just arrived",
        translate: true,
    },

    tierSporeDescription: {
        text: "Welcome! Here's some Pollen to play with.",
        translate: true,
    },

    tierSeedTitle: {
        text: "Seed — Part of the community",
        translate: true,
    },

    tierSeedDescription: {
        text: "You're on GitHub or Discord, you've said hi. More Pollen for you.",
        translate: true,
    },

    tierFlowerTitle: {
        text: "Flower — You shipped something",
        translate: true,
    },

    tierFlowerDescription: {
        text: "You built an app with pollinations.ai. Nice! Even more Pollen.",
        translate: true,
    },

    tierNectarTitle: {
        text: "Nectar — Community pillar",
        translate: true,
    },

    tierNectarDescription: {
        text: "Your work helps others. You're part of what makes this place good.",
        translate: true,
    },

    // Earn Pollen: Quests
    questsSubtitle: {
        text: "Ways to Contribute",
        translate: true,
    },

    questsDescription: {
        text: "Fix a bug, answer a question, share what you built, improve the docs. Every contribution matters and gets recognized.",
        translate: true,
    },

    questsStatus: {
        text: "New",
        translate: true,
    },

    // What You Can Build Section
    buildTitle: {
        text: "What People Build",
        translate: true,
    },

    buildIntro: {
        text: "Some things the community has made:",
        translate: true,
    },

    buildFeature1: {
        text: "Chatbots and agents with memory",
        translate: true,
    },

    buildFeature2: {
        text: "Visual worlds with consistent characters and assets",
        translate: true,
    },

    buildFeature3: {
        text: "Multi-step workflows for research, summarization, and creation",
        translate: true,
    },

    buildFeature4: {
        text: "Interactive media and new modalities (video, audio, and more — coming soon)",
        translate: true,
    },

    // Why Choose Section
    whyChooseTitle: {
        text: "Why Developers Choose pollinations.ai",
        transform: true,
    },

    whyChooseIntro: {
        text: "You get a platform that cares about aesthetics and ergonomics as much as raw capability.",
        transform: true,
    },

    whyChooseFeature1: {
        text: "Beautiful Dev Experience — clean APIs, thoughtful defaults, and friendly docs.",
        transform: true,
    },

    whyChooseFeature2: {
        text: "Unified API — work with multiple models and modalities through one place.",
        transform: true,
    },

    whyChooseFeature3: {
        text: "No BS Pricing — Pollen is simple and transparent. You always know what you're paying.",
        transform: true,
    },

    whyChooseFeature4: {
        text: "Community-Driven — the roadmap comes from Discord conversations, not boardrooms.",
        transform: true,
    },

    whyChooseFeature5: {
        text: "Fully Open Source — every line of code is on GitHub. Fork it, read it, improve it.",
        transform: true,
    },

    // Community Section
    communityTitle: {
        text: "Built Together",
        transform: true,
    },

    communityDescription: {
        text: "Students, indie devs, artists, researchers, hobbyists — all building weird and wonderful things together. Jump into Discord, browse the GitHub, see what people are making. This is a place where people actually help each other.",
        transform: true,
    },

    // Roadmap Section
    roadmapTitle: {
        text: "What's Next",
        translate: true,
    },

    roadmapIntro: {
        text: "Here's what we're working on:",
        translate: true,
    },

    roadmapItem1Title: {
        text: "Frontend Auth",
        translate: true,
    },

    roadmapItem1Description: {
        text: "Call the API from the browser — no backend needed.",
        transform: true,
    },

    roadmapItem2Title: {
        text: "User Pollen",
        translate: true,
    },

    roadmapItem2Description: {
        text: "Your users bring their own Pollen. You build, they pay for what they use.",
        transform: true,
    },

    roadmapItem3Title: {
        text: "App Hosting & Discovery",
        translate: true,
    },

    roadmapItem3Description: {
        text: "Ship your app and make it easy to find.",
        transform: true,
    },

    roadmapItem4Title: {
        text: "More Models, More Modalities",
        translate: true,
    },

    roadmapItem4Description: {
        text: "Video, real-time, and whatever comes next.",
        transform: true,
    },

    // CTA Section
    ctaTitle: {
        text: "Ready to Create?",
        transform: true,
    },

    ctaDescription: {
        text: "Start building with tools that feel good to use.",
        transform: true,
    },

    // CTA Buttons
    getApiKeyButton: {
        text: "Get Your API Key",
        translate: true,
    },

    startCreatingButton: {
        text: "Start Creating",
        translate: true,
    },

    joinCommunityButton: {
        text: "Join the Community",
        translate: true,
    },

    viewPricingButton: {
        text: "View Pricing",
        translate: true,
    },

    exploreTiersButton: {
        text: "Learn More About Tiers",
        translate: true,
    },

    seeAppsButton: {
        text: "See Featured Apps",
        translate: true,
    },
};
