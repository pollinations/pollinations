...
function Tip({
    ...
    return (
        <div className="group/tip relative">
            {children}
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 bg-white text-dark text-[10px] rounded-tag shadow-lg border border-border opacity-0 group-hover/tip:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                {label}
            </div>
        </div>
    );
    ...
    const TextPanel = (
        ...
        >
            ...
            <div className="self-start inline-flex items-center gap-2">
                ...
                <span
                    className={`font-headline inline-flex items-center justify-center py-2 text-sm font-black uppercase tracking-wider rounded-tag cursor-pointer transition-colors min-w-[220px] ${!onPR ? "bg-dark text-white" : "bg-white text-muted hover:text-dark"}`}
                    onClick={() => setY(0)}
                >
                    {dateLabel}
                </span>
