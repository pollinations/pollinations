    ...
    ({ className, children, isTranslating, ...props }, ref) => {
        return (
            <div
                ref={ref}
                className={cn(
                    "relative bg-cream border-r-4 border-b-4 border-dark shadow-dark-lg p-6 md:p-8 rounded-card",
                    className,
                )}
                {...props}
            >
                ...
                {isTranslating && (
                    <div className="absolute top-2 right-3 md:top-3 md:right-4 z-10">
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-black/70 border border-white/20 text-[10px] text-white/70">
