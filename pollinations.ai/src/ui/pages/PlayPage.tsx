...
function PlayPage() {
  ...
  const { imageModels, textModels, allowedImageModelIds, allowedTextModelIds } =
    useModelList(apiKey);
    ...
    <PageContainer>
      <PageCard isTranslating={isTranslating}>
        ...
        <div className="flex items-center gap-4 mb-8">
          <Title spacing="none">
            {view === "play" ? pageCopy.createTitle : pageCopy.watchTitle}
          </Title>
          <Button
            variant="secondary"
            size="sm"
            onClick={() => setView(view === "play" ? "feed" : "play")}
          >
            {view === "play"
        ...
        {view === "play" &&
          (!isLoggedIn ? (
            <div className="flex items-center gap-4 p-4 mb-8 bg-surface-card rounded-sub-card border-l-4 border-border-highlight">
              ...
              <Button type="button" onClick={login} variant="primary" size="sm">
