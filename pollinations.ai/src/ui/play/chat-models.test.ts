...
describe("chat attachments", () => {
    ...
    it("rejects unsupported audio formats", () => {
        expect(audioFormat({ name: "recording.aac", type: "audio/aac" })).toBeNull();
    });

    it("encodes audio bytes without a data URL prefix", () => {
        expect(arrayBufferToBase64(Uint8Array.from([0, 1, 2, 255]).buffer)).toBe(
            "AAEC/w==",
        );
    });
...
describe("streamed media rendering", () => {
    ...
    it("does not embed unsafe or unfinished links", () => {
        const markdown = "[audio](javascript:alert(1)) and ![half](https://media.test/pic";
