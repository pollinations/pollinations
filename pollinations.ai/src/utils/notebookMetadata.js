...
function readMetadata(notebookJSON) {
	...
	if (!allParameters[0]) return null;
...
const extractParametersWithComment = (text, i, codeRows) => {
	...
	if (
		params &&
		previousRow &&
		previousRow.trim().startsWith("#") &&
		!previousRow.includes("#@param")
	) {
...
const isParameterCell = (cell) =>
	cell["cell_type"] === "code" && cell["source"].join("\n").includes("#@param");

// finds the first cell of type markdown
const isMarkdownCell = (cell) => cell["cell_type"] === "markdown";

const isDescriptionCell = (cell) =>
	isMarkdownCell(cell) && cell.metadata?.id !== "view-in-github";

const isColabLinkCell = (cell) =>
	isMarkdownCell(cell) && cell.metadata?.id === "view-in-github";
...
export const getNotebookMetadata = (ipfs) =>
	readMetadata(
		(ipfs?.input && ipfs.input["notebook.ipynb"]) ||
			(ipfs && ipfs["notebook.ipynb"]),
	);
