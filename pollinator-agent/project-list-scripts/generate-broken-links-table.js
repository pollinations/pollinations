...
`;
    ...
    const missing404Repos = sortedLinks.filter(link => 
...
`;
    ...
    const brokenMainUrls = sortedLinks.filter(link => 
      link.type === 'Main URL' && (link.status === 404 || link.error)
    );

    if (brokenMainUrls.length > 0) {
      for (const url of brokenMainUrls.slice(0, 10)) { // Show first 10
...
generateBrokenLinksTable().then(markdown => {
  if (markdown) {
    // Save to the main pollinations repo for visibility
    const outputPath = path.join(process.cwd(), '..', 'BROKEN_LINKS.md');
