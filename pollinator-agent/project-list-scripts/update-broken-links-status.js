...
async function updateBrokenLinksTable() {
  try {
    const brokenLinksPath = path.join(process.cwd(), '..', 'BROKEN_LINKS.md');
...
`;
    ...
    const fullSearchSection = searchResultsSection + tableRows + `
...
`;
    ...
    if (insertPoint !== -1) {
      content = content.slice(0, insertPoint) + fullSearchSection + content.slice(insertPoint);
    } else {
    ...
    console.log(`📊 Investigated ${Object.keys(searchResults).length} projects`);
    console.log(`❌ Found ${Object.values(searchResults).filter(r => r.status.includes('REMOVED')).length} confirmed dead projects`);
...
`;
  ...
  await fs.writeFile(path.join(process.cwd(), 'cleanup-dead-projects.js'), cleanupScript);
