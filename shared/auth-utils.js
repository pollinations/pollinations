...
async function _checkReferrerInDb(referrer) {
    ...
    try {
        ...
        if (data && data.valid && data.user_id) {
            ...
            referrerLog(
                `❌ Domain ${domain} is not registered in the database`,
            );
            return null;
        }
    } catch (error) {
...
async function _validateApiTokenDb(token) {
    ...
    try {
        ...
        tokenLog(
            "validateApiTokenDb: Error during API call: %s",
            error.message,
        );
...
export async function shouldBypassQueue(req) {
    ...
    if (token) {
        ...
        tokenLog(
            "❌ Invalid or unrecognized token provided: %s. Will try other auth methods.",
            debugInfo.token,
        );
        errorLog(
            "Invalid or unrecognized token provided (source: %s, token: %s)",
            debugInfo.tokenSource || "unknown",
            debugInfo.token,
        );
        throw new Error("Invalid or unrecognized token provided");
    }

    // 2 Referrer-based authentication
    if (ref) {
