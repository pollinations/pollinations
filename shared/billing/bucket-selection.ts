 ...
 * the flag means "must hold paid balance", a positive-balance requirement
 * rather than a cost-coverage one, so a zero-balance caller never gets a
...
export function selectWalletBucket(
    ...
    if (isPaidOnly) return balances.packBalance > threshold ? "pack" : null;
