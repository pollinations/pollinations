...
// Per-second video ceiling. First-party video models run up to ~$0.25/s;
// allow the same headroom community providers may need for GPU cost + margin.
...
// Video endpoints bill output duration in seconds. The probe normalises the
// upstream response to `duration_seconds` (in the data[0] item), so that is
// the only path declared here.
...
/**
 * Pull the first usable video out of a community video-generations response.
 * Expects the same `data[0].url` / `data[0].b64_json` envelope as the image
 * endpoint, plus a `duration_seconds` number for billing. Returns null when
 * the body carries no usable video; throws HttpError(502) for unsafe or
 * oversized URLs so the gen funnel surfaces the status directly.
 */
export async function firstCommunityVideoData(
    ...
    for (const item of body.data) {
        ...
        if (
            ...
            bytes = decodeCommunityBase64(item.b64_json);
        } else if (
            "url" in item &&
            typeof item.url === "string" &&
            item.url.length > 0
        ) {
            bytes = await fetchCommunityVideoBytes(item.url, endpointBaseUrl);
        }
