import type { ModelDefinition } from "./registry";

// Voices available for openai-audio model - exported for schema validation
export const AUDIO_VOICES = [
    "alloy",
    "echo",
    "fable",
    "onyx",
    "nova",
    "shimmer",
    "coral",
    "verse",
    "ballad",
    "ash",
    "sage",
    "amuch",
    "dan",
] as const;

export type AudioVoice = (typeof AUDIO_VOICES)[number];

export const DEFAULT_TEXT_MODEL = "gpt-5.4-nano" as const;
export type TextModelName = keyof typeof TEXT_SERVICES;

export const TEXT_SERVICES = {
    "gpt-5.4-nano": {
        brand: "OpenAI",
        provider: "azure",
        aliases: ["openai", "gpt-5-mini"],
        description: "Text generation with image input.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 400000,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000002,
            promptCachedTokens: 0.00000002,
            completionTextTokens: 0.00000125,
        },
    },
    "gpt-5-nano-2025-08-07": {
        brand: "OpenAI",
        provider: "azure",
        version: "2025-08-07",
        aliases: ["gpt-5-nano", "openai-fast"],
        description: "Text generation with image input.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 400000,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.00000005,
            promptCachedTokens: 0.000000005,
            completionTextTokens: 0.0000004,
        },
    },
    "gpt-5.4": {
        brand: "OpenAI",
        provider: "azure",
        aliases: [
            "openai-large",
            "gpt-5.4",
            "gpt-5.4-reasoning",
            "gpt-5.2",
            "openai-reasoning",
            "gpt-5.2-reasoning",
        ],
        description: "Multimodal text generation with reasoning.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 400000,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000025,
            promptCachedTokens: 0.00000025,
            completionTextTokens: 0.000015,
        },
    },
    "qwen3-coder-30b-a3b-instruct": {
        brand: "Qwen",
        provider: "ovhcloud",
        aliases: ["qwen3-coder-30b", "qwen-coder", "qwen3-coder"],
        description: "Code generation and editing.",
        category: "text",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 262144,
        introducedAt: 1767571200000,
        cost: {
            promptTextTokens: 0.00000006,
            completionTextTokens: 0.00000022,
        },
    },
    "mistral-small-3.2-24b-instruct-2506": {
        brand: "Mistral",
        provider: "ovhcloud",
        version: "2506",
        aliases: ["mistral-small-3.2", "mistral", "mistral-small"],
        description: "Text generation with image input.",
        category: "text",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 131072,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000001,
            completionTextTokens: 0.0000003,
        },
    },
    "gpt-audio-mini-2025-12-15": {
        brand: "OpenAI",
        provider: "azure",
        version: "2025-12-15",
        aliases: [
            "gpt-audio-mini",
            "openai-audio",
            "gpt-4o-mini-audio-preview",
            "gpt-4o-mini-audio-preview-2024-12-17",
        ],
        description: "Multimodal generation with text and audio output.",
        category: "audio",
        profile: "fast",
        inputModalities: ["text", "image", "audio"],
        outputModalities: ["audio", "text"],
        capabilities: ["tools"],
        contextLength: 128000,
        voices: [...AUDIO_VOICES],
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000006,
            promptAudioTokens: 0.00001,
            completionTextTokens: 0.0000024,
            completionAudioTokens: 0.00002,
        },
    },
    "gpt-audio-1.5": {
        brand: "OpenAI",
        provider: "azure",
        aliases: ["openai-audio-large", "gpt-audio", "gpt-audio-2025-12-15"],
        description: "Multimodal generation with text and audio output.",
        category: "audio",
        profile: "pro",
        inputModalities: ["text", "image", "audio"],
        outputModalities: ["audio", "text"],
        capabilities: ["tools"],
        contextLength: 128000,
        voices: [...AUDIO_VOICES],
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000025,
            promptAudioTokens: 0.00004,
            completionTextTokens: 0.00001,
            completionAudioTokens: 0.00008,
        },
    },
    "gemini-3-flash-preview": {
        brand: "Google",
        provider: "google",
        version: "preview",
        aliases: ["gemini-3-flash", "gemini"],
        description: "Multimodal text generation with web search.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image", "audio", "video"],
        outputModalities: ["text"],
        capabilities: ["tools", "search", "codeExecution"],
        contextLength: 1048576,
        paidOnly: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000005,
            promptCachedTokens: 0.00000005,
            promptAudioTokens: 0.0000005,
            completionTextTokens: 0.000003,
        },
        price: {
            promptTextTokens: 0.00000075,
            promptCachedTokens: 0.000000075,
            promptAudioTokens: 0.00000075,
            completionTextTokens: 0.0000045,
        },
    },
    "gemini-3.1-flash-lite-preview": {
        brand: "Google",
        provider: "google",
        version: "preview",
        aliases: [
            "gemini-flash-lite-3.1",
            "gemini-3.1-flash-lite",
            "gemini-flash-lite",
        ],
        description: "Multimodal text generation with web search.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image", "audio", "video"],
        outputModalities: ["text"],
        capabilities: ["tools", "search", "codeExecution"],
        contextLength: 1048576,
        paidOnly: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.00000025,
            promptCachedTokens: 0.000000025,
            promptAudioTokens: 0.0000005,
            completionTextTokens: 0.0000015,
        },
        price: {
            promptTextTokens: 0.000000375,
            promptCachedTokens: 0.0000000375,
            promptAudioTokens: 0.00000075,
            completionTextTokens: 0.00000225,
        },
    },
    "gemini-2.5-flash-lite": {
        brand: "Google",
        provider: "google",
        aliases: ["gemini-fast"],
        description: "Multimodal text generation with web search.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image", "audio", "video"],
        outputModalities: ["text"],
        capabilities: ["tools", "search", "codeExecution"],
        contextLength: 1048576,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000001,
            promptCachedTokens: 0.00000001,
            promptAudioTokens: 0.0000003,
            completionTextTokens: 0.0000004,
        },
        price: {
            promptTextTokens: 0.0000003,
            promptCachedTokens: 0.00000003,
            promptAudioTokens: 0.0000003,
            completionTextTokens: 0.0000012,
        },
    },
    "deepseek-v3.2": {
        brand: "DeepSeek",
        provider: "fireworks",
        aliases: ["deepseek", "deepseek-v3", "deepseek-reasoning"],
        description: "Text generation with reasoning.",
        category: "text",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 163840,
        introducedAt: 1775952000000,
        cost: {
            promptTextTokens: 0.00000056,
            promptCachedTokens: 0.00000028,
            completionTextTokens: 0.00000168,
        },
    },
    "grok-4-1-fast-non-reasoning": {
        brand: "xAI",
        provider: "azure",
        aliases: [
            "grok-4.1-fast",
            "grok",
            "grok-fast",
            "grok-4-1-fast",
            "grok-legacy",
            "grok-4",
            "grok-4-fast",
        ],
        description: "Text generation.",
        category: "text",
        profile: "fast",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 2000000,
        introducedAt: 1774137600000,
        cost: {
            promptTextTokens: 0.0000002,
            promptCachedTokens: 0.00000005,
            completionTextTokens: 0.0000005,
        },
    },
    "grok-4.20-0309-reasoning": {
        brand: "xAI",
        provider: "azure",
        aliases: [
            "grok-4.20-reasoning",
            "grok-large",
            "grok-4-20",
            "grok-4-20-reasoning",
            "grok-reasoning",
            "grok-4-1-fast-reasoning",
        ],
        description: "Text generation with reasoning.",
        category: "text",
        profile: "pro",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 2000000,
        introducedAt: 1775606400000,
        cost: {
            promptTextTokens: 0.000002,
            promptCachedTokens: 0.0000002,
            completionTextTokens: 0.000006,
        },
    },
    "gemini-search": {
        brand: "Google",
        provider: "google",
        aliases: ["gemini-2.5-flash-search", "gemini-2.5-flash-lite-search"],
        description: "Multimodal text generation with web search.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image", "audio", "video"],
        outputModalities: ["text"],
        capabilities: ["search", "codeExecution"],
        contextLength: 1048576,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000002,
            promptCachedTokens: 0.00000002,
            promptAudioTokens: 0.0000002,
            completionTextTokens: 0.0000008,
        },
        price: {
            promptTextTokens: 0.0000003,
            promptCachedTokens: 0.00000003,
            promptAudioTokens: 0.0000003,
            completionTextTokens: 0.0000012,
        },
    },
    "midijourney": {
        brand: "Pollinations",
        provider: "bedrock",
        aliases: [],
        description: "Music composition and MIDI assistance.",
        category: "text",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        isSpecialized: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000011,
            promptCachedTokens: 0.00000011,
            completionTextTokens: 0.0000055,
        },
    },
    "midijourney-pro": {
        brand: "Pollinations",
        provider: "bedrock",
        aliases: ["midijourney-large"],
        description: "Music composition and MIDI assistance.",
        category: "text",
        profile: "pro",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        paidOnly: true,
        isSpecialized: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000055,
            promptCachedTokens: 0.00000055,
            completionTextTokens: 0.0000275,
        },
    },
    "claude-haiku-4-5": {
        brand: "Anthropic",
        provider: "bedrock",
        version: "2025-10-01",
        aliases: ["claude-haiku-4.5", "claude-fast", "claude-haiku"],
        description: "Text generation with image input.",
        category: "text",
        profile: "fast",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 200000,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000011,
            promptCachedTokens: 0.00000011,
            completionTextTokens: 0.0000055,
        },
    },
    "claude-sonnet-4-6": {
        brand: "Anthropic",
        provider: "bedrock",
        aliases: ["claude-sonnet-4.6", "claude", "claude-sonnet"],
        description: "Text generation with image input.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 200000,
        paidOnly: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000033,
            promptCachedTokens: 0.00000033,
            completionTextTokens: 0.0000165,
        },
    },
    "claude-opus-4-6": {
        brand: "Anthropic",
        provider: "bedrock",
        aliases: [
            "claude-opus-4.6",
            "claude-large",
            "claude-opus",
            "claude-opus-4.5",
        ],
        description: "Text generation with image input.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 200000,
        paidOnly: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.0000055,
            promptCachedTokens: 0.00000055,
            completionTextTokens: 0.0000275,
        },
    },
    "claude-opus-4-7": {
        brand: "Anthropic",
        provider: "bedrock",
        aliases: ["claude-opus-4.7"],
        description: "Text generation with image input.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 200000,
        paidOnly: true,
        introducedAt: new Date("2026-04-19").getTime(),
        cost: {
            promptTextTokens: 0.0000055,
            promptCachedTokens: 0.00000055,
            completionTextTokens: 0.0000275,
        },
    },
    "sonar": {
        brand: "Perplexity",
        provider: "perplexity",
        aliases: ["perplexity-sonar", "perplexity-fast"],
        description: "Text generation with web search.",
        category: "text",
        profile: "fast",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["search"],
        contextLength: 127072,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.000001,
            completionTextTokens: 0.000001,
        },
    },
    "sonar-reasoning-pro": {
        brand: "Perplexity",
        provider: "perplexity",
        aliases: [
            "perplexity-sonar-reasoning-pro",
            "perplexity-reasoning",
            "sonar-reasoning",
        ],
        description: "Text generation with web search.",
        category: "text",
        profile: "pro",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["reasoning", "search"],
        contextLength: 128000,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.000002,
            completionTextTokens: 0.000008,
        },
    },
    "kimi-k2.5": {
        brand: "Moonshot AI",
        provider: "fireworks",
        aliases: [
            "kimi",
            "kimi-k2p5",
            "kimi-reasoning",
            "kimi-large",
            "kimi-k2-thinking",
            "kimi-thinking",
        ],
        description: "Multimodal text generation with reasoning.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 262000,
        introducedAt: 1775952000000,
        cost: {
            promptTextTokens: 0.0000006,
            promptCachedTokens: 0.0000001,
            completionTextTokens: 0.000003,
        },
    },
    "kimi-k2.6": {
        brand: "Moonshot AI",
        provider: "fireworks",
        aliases: ["kimi-k2p6"],
        description: "Multimodal text generation with reasoning.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 262000,
        introducedAt: new Date("2026-04-21").getTime(),
        cost: {
            promptTextTokens: 0.00000095,
            promptCachedTokens: 0.00000016,
            completionTextTokens: 0.000004,
        },
    },
    "gemini-3.1-pro-preview": {
        brand: "Google",
        provider: "google",
        version: "preview",
        aliases: ["gemini-3.1-pro", "gemini-large", "gemini-2.5-pro"],
        description: "Multimodal text generation with web search.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image", "audio", "video"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning", "search"],
        contextLength: 1048576,
        paidOnly: true,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.000002,
            promptCachedTokens: 0.0000002,
            completionTextTokens: 0.000012,
        },
        price: {
            promptTextTokens: 0.000003,
            promptCachedTokens: 0.0000003,
            completionTextTokens: 0.000018,
        },
    },
    "nova-micro-v1": {
        brand: "Amazon",
        provider: "bedrock",
        aliases: [
            "nova-micro-1",
            "nova-fast",
            "amazon-nova-micro",
            "nova-micro",
        ],
        description: "Text generation.",
        category: "text",
        profile: "fast",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 128000,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.000000035,
            completionTextTokens: 0.00000014,
        },
    },
    "nova-2-lite": {
        brand: "Amazon",
        provider: "bedrock",
        aliases: ["nova", "amazon-nova-2-lite", "nova-2"],
        description: "Text generation with reasoning.",
        category: "text",
        inputModalities: ["text", "image", "video"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 1048576,
        introducedAt: 1753999200000,
        cost: {
            promptTextTokens: 0.00000033,
            completionTextTokens: 0.00000275,
        },
    },
    "glm-5.1": {
        brand: "Z.ai",
        provider: "fireworks",
        aliases: ["glm", "glm-5", "glm-5p1", "glm-4.7", "glm-4p7"],
        description: "Text generation with reasoning.",
        category: "text",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 198000,
        introducedAt: 1775952000000,
        cost: {
            promptTextTokens: 0.000001,
            promptCachedTokens: 0.0000002,
            completionTextTokens: 0.0000032,
        },
    },
    "minimax-m2.7": {
        brand: "MiniMax",
        provider: "fireworks",
        aliases: ["minimax", "minimax-m2p7", "minimax-m2.5", "minimax-m2p5"],
        description: "Text generation with reasoning.",
        category: "text",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 200000,
        introducedAt: new Date("2026-04-19").getTime(),
        cost: {
            promptTextTokens: 0.0000003,
            promptCachedTokens: 0.00000006,
            completionTextTokens: 0.0000012,
        },
    },
    "mistral-large-3": {
        brand: "Mistral",
        provider: "azure",
        aliases: ["mistral-large"],
        description: "Multimodal text generation with reasoning.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 256000,
        introducedAt: 1775606400000,
        cost: {
            promptTextTokens: 0.0000005,
            promptCachedTokens: 0.00000005,
            completionTextTokens: 0.0000015,
        },
    },
    "polly": {
        brand: "Pollinations",
        provider: "community",
        aliases: ["pollinations-ai", "polly-ai"],
        description:
            "Assistant-style text generation with tools, search, and code execution.",
        category: "text",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning", "search", "codeExecution"],
        alpha: true,
        introducedAt: 1771804800000,
        cost: {},
    },
    "qwen3-coder-next": {
        brand: "Qwen",
        provider: "alibaba",
        aliases: ["qwen-coder-large"],
        description: "Code generation and editing.",
        category: "text",
        profile: "pro",
        inputModalities: ["text"],
        outputModalities: ["text"],
        capabilities: ["tools"],
        contextLength: 262144,
        introducedAt: 1774137600000,
        cost: {
            promptTextTokens: 0.0000003,
            completionTextTokens: 0.0000015,
        },
        price: {
            promptTextTokens: 0.00000045,
            completionTextTokens: 0.00000225,
        },
    },
    "qwen3.6-plus": {
        brand: "Qwen",
        provider: "fireworks",
        aliases: ["qwen-large", "qwen3.6", "qwen3p6-plus"],
        description: "Multimodal text generation with reasoning.",
        category: "text",
        profile: "pro",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 1048576,
        introducedAt: 1775952000000,
        cost: {
            promptTextTokens: 0.0000005,
            promptCachedTokens: 0.0000001,
            completionTextTokens: 0.000003,
        },
    },
    "qwen3-vl-30b-a3b-thinking": {
        brand: "Qwen",
        provider: "fireworks",
        aliases: [
            "qwen3-vl-thinking",
            "qwen-vision",
            "qwen3-vl",
            "qwen3-vl-plus",
            "qwen-vl",
        ],
        description: "Multimodal text generation with reasoning.",
        category: "text",
        inputModalities: ["text", "image"],
        outputModalities: ["text"],
        capabilities: ["tools", "reasoning"],
        contextLength: 262144,
        introducedAt: new Date("2026-04-19").getTime(),
        cost: {
            promptTextTokens: 0.00000015,
            promptCachedTokens: 0.00000008,
            completionTextTokens: 0.0000006,
        },
    },
    "qwen3guard-gen-8b": {
        brand: "Qwen",
        provider: "ovhcloud",
        aliases: ["qwen3-guard-8b", "qwen-safety"],
        description: "Content safety and moderation.",
        category: "text",
        inputModalities: ["text"],
        outputModalities: ["text"],
        isSpecialized: true,
        introducedAt: 1771113600000,
        cost: {
            promptTextTokens: 0.00000001,
            completionTextTokens: 0.00000001,
        },
    },
} as const satisfies Record<string, ModelDefinition>;
