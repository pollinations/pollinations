...
export async function verifyTurnstile(token, ip, hostname, env) {
    ...
    console.log(
        "[turnstile] Using secret type:",
        env.TURNSTILE_TEST_SECRET ? "TEST" : "PRODUCTION",
    );
    ...
    try {
        ...
        console.log(
            `[turnstile] Verification result for ${hostname}: success=${result.success}, error-codes=${result["error-codes"]?.join(",") || "none"}`,
        );
        ...
        if (isTestKey) {
            console.log(
                "[turnstile] ℹ️ Using test key, skipping hostname validation",
            );
        }
...
export function needsTurnstileVerification(origin, method) {
    ...
    if (origin.endsWith(".pollinations.ai")) {
        ...
        const mainDomains = ["text.pollinations.ai", "image.pollinations.ai"];
