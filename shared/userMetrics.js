...
export async function incrementUserMetric(userId, metricKey, incrementBy = 1) {
  ...
  console.log(`User ID found: ${userId}. Attempting to increment ${metricKey}.`);
  
  const adminApiKey = process.env.ADMIN_API_KEY;
  if (!adminApiKey) {
    console.log(`ADMIN_API_KEY not found in environment. Cannot increment ${metricKey}.`);
    return;
  }
  ...
  .then(async response => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`Failed to increment ${metricKey} for user ${userId}. Status: ${response.status}`, responseBody);
      } else {
        console.log(`Successfully triggered ${metricKey} increment for user ${userId}. Response:`, responseBody);
      }
    } catch (e) {
  ...
  .catch(error => {
    console.error(`Network error or other issue incrementing ${metricKey} for user ${userId}:`, error);
  });
}
