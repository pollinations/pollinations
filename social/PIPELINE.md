...
═══════════════════════════════════════════════════════════════════════
             ...
             Sunday 00:00 UTC ──→ generate_weekly.py
...
| `NEWS_weekly_publish.yml` | `cron: 0 8 * * 1` (Monday 08:00 UTC) — checks if weekly PR merged, publishes all 5 platforms | `NEWS_Discord_post_weekly_news.yml` + `NEWS_Buffer_stage_posts.yml` (weekly) |
...
| `publish_daily.py` | On daily PR merge: Buffer stage (X, LI, IG) + Reddit + highlights + README | (new — consolidates publish steps) |
...
| **Daily platform images** | `generate_daily.py` | Tier 2 (00:00 UTC) | Brand pixel art (from `brand_visual.md`) | 1 Twitter + 1 LinkedIn + 3 Instagram + 1 Reddit = **6 per day** | Twitter, LinkedIn, Instagram, Reddit daily posts |
| **Weekly platform images** | `generate_weekly.py` | Tier 3 (Sunday 00:00 UTC) | Brand pixel art (from `brand_visual.md`) | 1 Twitter + 1 LinkedIn + 3 Instagram + 1 Reddit + 1 Discord = **7 per week** | Twitter, LinkedIn, Instagram, Reddit, Discord weekly posts |
...
- Create `generate_weekly.py` + `NEWS_weekly_summary.yml` (Sunday 00:00 UTC)
