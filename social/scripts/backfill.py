...
def fetch_merged_prs_for_date(date_str: str, github_token: str) -> list[dict]:
    ...
    for item in results:
        ...
        is_social_post = any(
            title.startswith(f"{p} Post - ")
            for p in ["Twitter", "Instagram", "LinkedIn", "Reddit", "Discord"]
        )
...
def backfill_gists(
    ...
    for i, pr_item in enumerate(prs, 1):
        ...
        gist = generate_gist_for_pr(pr_num, date_str, github_token, pollinations_token)
        if gist:
            gists.append(gist)
