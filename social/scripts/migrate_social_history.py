...
def resolve_repo_root(repo_root_arg: str) -> Path:
    ...
    if not news_root.exists():
        ...
        print(
            "Hint: run this against a checkout or worktree that contains the news branch."
        )
        sys.exit(1)
    return repo_root
...
def build_daily_summary_from_legacy(
    ...
    if not source_text and reddit_post:
        source_text = str(
            reddit_post.get("body") or reddit_post.get("title") or ""
        ).strip()
...
def build_weekly_summary_from_legacy(
    ...
    if linkedin_post:
        ...
        if body:
            ...
            title, summary = split_title_and_summary(
                full_post, f"Week ending {week_end}"
            )
        elif hook:
...
def migrate_daily_folder(
    repo_root: Path, folder: Path, apply: bool
) -> tuple[bool, str]:
...
def migrate_weekly_folder(
    ...
    if not any(
        [twitter_post, linkedin_post, instagram_post, reddit_post, discord_post]
    ):
        return False, "no legacy platform JSON found"
    ...
    if source_dir != target_dir:
        print(f"[weekly:{folder.name}] rename to Sunday publish date {publish_date}")
        print(
            f"  MOVE  {source_dir.relative_to(repo_root)} -> {target_dir.relative_to(repo_root)}"
        )
    else:
