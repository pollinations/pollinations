      ...
      try {
        ...
        if (response && response.length > 0) {
          ...
          if (response.toUpperCase().startsWith("ENGLISH:")) {
            ...
            log(
              `Content detected as English, keeping original ad text for ${affiliate.name} (${affiliateId})`,
            );
          } else {
            ...
            if (colonIndex > 0) {
              ...
              log(
                `Translated ad for ${affiliate.name} (${affiliateId}) to ${detectedLanguage}`,
              );
            } else {
              // If format is unexpected, use the response as is
              adTextSource = response;
              log(
                `Received unformatted translation for ${affiliate.name} (${affiliateId})`,
              );
            }
          }
        }
