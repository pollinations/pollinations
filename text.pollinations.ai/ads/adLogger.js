...
export async function logAdInteraction(data) {
    ...
    const requestId = explicitRequestId || 
                     (req?.id ? req.id : `req-${Date.now()}${isStreaming ? '-stream' : ''}${reason === 'error' ? '-error' : ''}`);
    ...
    try {
        ...
        if (affiliateData) {
            ...
            if (affiliateData.product) logEntry += `- **Product:** ${affiliateData.product}\n`;
            if (affiliateData.description) logEntry += `- **Description:** ${affiliateData.description}\n`;
            logEntry += '\n';
        }
