...
async function refreshGcloudAccessToken() {
    try {
        ...
        if (!credentialsPath) {
            errorLog('GOOGLE_APPLICATION_CREDENTIALS environment variable not set');
            return null;
        }
...
async function generateJwtToken(keyData) {
    try {
        ...
        if (!keyData.private_key || !keyData.private_key_id || !keyData.client_email) {
...
async function exchangeJwtForAccessToken(jwt) {
    try {
        ...
        try {
            ...
            errorLog('Network error when contacting Google OAuth endpoint:', fetchError);
            return null;
        }
        ...
        if (!response.ok) {
            ...
            errorLog(`Failed to exchange JWT for access token: ${response.status} ${response.statusText} - ${errorText}`);
            return null;
        }
