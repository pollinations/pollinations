...
export function generateCacheKey(url, requestBody = null) {
  ...
  if (requestBody) {
    try {
      ...
      // Log if no seed is provided, as this might be useful for debugging cache misses
      if (requestBody.seed === undefined) {
        console.log('Note: No seed provided in request body. Origin will handle seed generation.');
      }
    } catch (error) {
