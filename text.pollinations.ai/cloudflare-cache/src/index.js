...
const isCacheable = (u) => {
  ...
  logMain(
    "Path: %s, hasNoCache: %s, isNonCachePath: %s, isCacheable: %s",
    u.pathname,
    hasNoCache,
    isNonCachePath,
    result,
...
const getCached = async (env, key) => {
  ...
  try {
    const result = await env.TEXT_BUCKET.get(key, {
      httpMetadata: true,
      customMetadata: true,
    });
    ...
    if (result) {
      ...
      logCache(
        "Cached response: %s, size: %s bytes",
      ...
      if (
        ...
        logCache(
          "Warning: Streaming response cache hit but size is suspiciously small",
        );
      }
    } else {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (!originResp.ok) {
        ...
        logMain(
          "Origin response not OK (%d), skipping cache",
          originResp.status,
        );
