...
export async function findSimilarText(cache, text, modelName = 'unknown', userPrefix = 'anon') {
  console.log(`[SEMANTIC_CACHE] findSimilarText called with model: ${modelName}, userPrefix: ${userPrefix}, text length: ${text.length}`);
  ...
  console.log('[SEMANTIC_CACHE] Vectorize is available, proceeding with semantic search');
  ...
  try {
    ...
    if (searchResults.matches.length === 0) {
      console.log(`[SEMANTIC_CACHE] No matches in Vectorize. Returning similarity 0`);
      return {
        cacheKey: null,
        similarity: 0,
        model: modelName,
    ...
    if (bestMatch.score >= cache.similarityThreshold) {
      console.log(`[SEMANTIC_CACHE] Found similar text for model ${modelName}, user ${userPrefix}: score=${bestMatch.score}`);
      return {
    ...
    console.log(`[SEMANTIC_CACHE] Best match similarity below threshold: ${bestMatch.score} < ${cache.similarityThreshold}`);
    return {
      cacheKey: null,
      similarity: bestMatch.score,
      model: bestMatch.metadata?.model || 'unknown',
...
export async function cacheTextEmbedding(cache, cacheKey, text, modelName = 'unknown', userPrefix = 'anon') {
  ...
  try {
    ...
    if (Array.isArray(embedding)) {
      ...
      console.log(`[SEMANTIC_CACHE] Embedding preview [${preview}] (dims: ${embedding.length})`);
    }
    ...
    console.log(`[SEMANTIC_CACHE] Cached embedding for key: ${cacheKey}, model: ${modelName}, user: ${userPrefix}`);
  } catch (error) {
