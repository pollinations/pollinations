...
async function testCacheFlow() {
  ...
  await makeRequest("Hi, can you help me understand machine learning?", 'Step 3');
}
...
async function makeRequest(content, stepName) {
  ...
  try {
    ...
    if (stepName === 'Step 1') {
      ...
      console.log(`  🔍 Expected: Direct HIT (no semantic similarity), Got: ${cacheType} with ${semanticSimilarity} similarity`);
      if (semanticSimilarity !== 'N/A' && cacheType === 'hit') {
        console.log(`  ❌ BUG: Direct cache hit should NOT have semantic similarity!`);
      }
    } else if (stepName === 'Step 3') {
      console.log(`  🔍 Expected: Semantic match or MISS, Got: ${cacheType} with ${semanticSimilarity} similarity`);
    }
