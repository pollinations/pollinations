...
const FRESH_REQUEST = {
  model: 'gpt-4',
  messages: [{"role": "user", "content": "Debug test prompt for cache investigation 2024"}],
  temperature: 0.1,
  stream: false
};
...
async function debugDetailedCache() {
  ...
  const uniqueRequest = {
    ...FRESH_REQUEST,
    messages: [{"role": "user", "content": `Debug test prompt for cache investigation ${timestamp}`}]
  };
  ...
  // Analysis
  console.log('\n📊 **DETAILED ANALYSIS:**');
  console.log(`Fresh Request:   ${result1.responseTime}ms | ${result1.cacheType} | ${result1.similarity} | X-Cache: ${result1.xCache}`);
  console.log(`Second Request:  ${result2.responseTime}ms | ${result2.cacheType} | ${result2.similarity} | X-Cache: ${result2.xCache}`);
  console.log(`Third Request:   ${result3.responseTime}ms | ${result3.cacheType} | ${result3.similarity} | X-Cache: ${result3.xCache}`);
  
  console.log(`\nCache Key Consistency:`);
  console.log(`Fresh:  ${result1.cacheKey}`);
  console.log(`Second: ${result2.cacheKey}`);
  console.log(`Third:  ${result3.cacheKey}`);
  console.log(`Keys Match: ${result1.cacheKey === result2.cacheKey && result2.cacheKey === result3.cacheKey}`);
  
  // Expected behavior analysis
  console.log(`\n🎯 **EXPECTED vs ACTUAL:**`);
  console.log(`Expected: FRESH=miss → SECOND=hit → THIRD=hit`);
  console.log(`Actual:   FRESH=${result1.cacheType} → SECOND=${result2.cacheType} → THIRD=${result3.cacheType}`);
  
  if (result1.cacheType === 'miss' && result2.cacheType !== 'hit') {
    console.log(`\n🚨 **DIRECT CACHE FAILURE CONFIRMED**`);
    console.log(`The direct cache lookup is failing even after successful cache write.`);
  }
}
