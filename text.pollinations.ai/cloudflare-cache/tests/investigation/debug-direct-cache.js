...
async function debugDirectCache() {
  ...
  // Analysis
  console.log('\n📊 **ANALYSIS:**');
  console.log(`First Request:  ${result1.responseTime}ms | ${result1.cacheType} | ${result1.similarity}`);
  console.log(`Second Request: ${result2.responseTime}ms | ${result2.cacheType} | ${result2.similarity}`);
  ...
  if (result1.cacheKey === result2.cacheKey && result2.cacheType !== 'hit') {
    console.log('\n🚨 **PROBLEM IDENTIFIED:**');
    console.log('Same cache key but second request is not getting direct cache hit!');
  ...
  console.log(`Third Request:  ${result3.responseTime}ms | ${result3.cacheType} | ${result3.similarity}`);
}
