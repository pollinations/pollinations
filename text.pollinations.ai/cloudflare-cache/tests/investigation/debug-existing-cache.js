...
const AI_REQUEST = {
  model: 'gpt-4',
  messages: [{"role": "user", "content": "What is artificial intelligence and how does it work?"}],
  temperature: 0.1,
  stream: false
};
...
async function debugExistingCache() {
  ...
  console.log('Testing the problematic AI prompt that was getting semantic hits...\n');
  ...
  // Analysis
  console.log('\n📊 **ANALYSIS:**');
  console.log(`First AI Request:  ${result1.responseTime}ms | ${result1.cacheType} | ${result1.similarity} | X-Cache: ${result1.xCache}`);
  console.log(`Second AI Request: ${result2.responseTime}ms | ${result2.cacheType} | ${result2.similarity} | X-Cache: ${result2.xCache}`);
  console.log(`Third AI Request:  ${result3.responseTime}ms | ${result3.cacheType} | ${result3.similarity} | X-Cache: ${result3.xCache}`);
  ...
  if (result1.cacheType === 'semantic') {
    console.log(`\n🔍 **DIAGNOSTIC:**`);
    console.log(`The AI question is finding a semantic match with similarity ${result1.similarity}`);
    console.log(`This means there's already similar content in the vector cache.`);
  }
  
  if (result1.cacheType === 'semantic' && result2.cacheType === 'semantic') {
    console.log(`\n❓ **THE MYSTERY:**`);
    console.log(`Even identical requests are going through semantic cache instead of direct cache.`);
    console.log(`This suggests the semantic cache might be finding a DIFFERENT cached response`);
...
async function makeDetailedRequest(requestBody, label) {
  ...
  try {
    ...
    console.log(`${label}: ${responseTime}ms | ${cacheType} | Sim: ${semanticSimilarity} | Model: ${cacheModel}`);
