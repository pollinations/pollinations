...
async function debugSemanticHits() {
	console.log(
		"🔍 **DEBUG: Why are TEST-6 and TEST-9 getting semantic hits?**\n",
	);

	for (const testCase of TEST_CASES) {
		console.log(
			`📡 Testing ${testCase.name}: "${testCase.request.messages[0].content}"`,
		);
		...
		results.forEach((result, i) => {
			console.log(
				`  Run ${i + 1}: ${result.responseTime}ms | ${result.cacheType} | ${result.similarity} | ${result.cacheKey?.substring(0, 20)}...`,
			);
		});

		// Check for consistency
		const allSameType = results.every(
			(r) => r.cacheType === results[0].cacheType,
		);
		const allSameKey = results.every((r) => r.cacheKey === results[0].cacheKey);
		const allSameSimilarity = results.every(
			(r) => r.similarity === results[0].similarity,
		);
		...
		if (results[0].cacheType === "semantic") {
			console.log(
				`  🔍 This prompt is finding a DIFFERENT cached entry with similarity ${results[0].similarity}`,
			);
