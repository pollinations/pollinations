...
console.log(
	"🔍 Debug Threshold Test - Testing Completely Different Conversations",
);
		...
		messages: [
			{
				role: "user",
				content:
					"test123... this one is about being weird and old.. nothing to do with the previous one",
			},
			...messages,
		],
		temperature: 0.3,
	...
	try {
		...
		try {
			...
			if (
				...
				console.log(
					`Response: "${content.substring(0, 150)}${content.length > 150 ? "..." : ""}"`,
				);
			}
		} catch (e) {
...
async function runThresholdTest() {
	console.log(
		"Testing completely different topics that should NOT be semantically similar\n",
	);
	...
	await testRequest("Test 3: Space Facts", [
		...
		{
			role: "user",
			content: "Tell me about black holes and their properties.",
		},
	]);
	...
	await testRequest("Test 4: Programming Help", [
		...
		{
			role: "user",
			content: "Explain how to implement a binary search tree in JavaScript.",
		},
	]);
