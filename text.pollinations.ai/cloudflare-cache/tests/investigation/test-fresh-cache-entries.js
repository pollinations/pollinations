...
async function testFreshCacheEntries() {
  console.log('🔍 **TEST: Fresh Cache Entry Creation and Direct Hit Verification**\n');
  ...
  for (let i = 0; i < uniquePrompts.length; i++) {
    ...
    console.log(`  First:  ${result1.responseTime}ms | ${result1.cacheType} | ${result1.similarity}`);
    console.log(`  Second: ${result2.responseTime}ms | ${result2.cacheType} | ${result2.similarity}`);
    console.log(`  Third:  ${result3.responseTime}ms | ${result3.cacheType} | ${result3.similarity}`);
    
    const expectedPattern = result1.cacheType === 'miss' && result2.cacheType === 'hit' && result3.cacheType === 'hit';
    console.log(`  Expected Pattern (miss → hit → hit): ${expectedPattern ? '✅' : '❌'}`);
    
    if (!expectedPattern) {
      console.log(`  🚨 ISSUE: Expected miss → hit → hit, got ${result1.cacheType} → ${result2.cacheType} → ${result3.cacheType}`);
    }
