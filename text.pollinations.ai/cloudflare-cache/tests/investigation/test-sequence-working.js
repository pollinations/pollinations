...
async function makeRequest(content, model, testName) {
  ...
  try {
    ...
    if (testName === 'Test 1') {
      ...
      if (cacheType === 'hit' && semanticSimilarity === 'N/A' && responseTime < 500) {
        console.log(`  ✅ PERFECT: Direct cache hit, no semantic search performed!`);
      } else {
        console.log(`  ❌ Issue: Expected fast direct hit with no semantic similarity`);
      }
    } else if (testName === 'Test 3') { 
      if (cacheType === 'semantic' && semanticSimilarity !== 'N/A') {
        console.log(`  ✅ PERFECT: Semantic cache hit with similarity ${semanticSimilarity}!`);
      } else if (cacheType === 'miss') {
        console.log(`  📝 Note: Semantic similarity too low or miss (expected behavior)`);
      } else {
