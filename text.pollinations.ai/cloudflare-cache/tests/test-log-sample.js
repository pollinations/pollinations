...
console.log('Testing random subset of 10 conversations to see semantic matches in detail\n');
...
async function makeRequest(conversation, baseUrl = CACHE_URL) {
  ...
  try {
    ...
    return {
      ...
      responseTime,
      cacheType,
      semanticSimilarity: semanticSimilarity ? parseFloat(semanticSimilarity) : null,
      cacheModel,
      fullResponse: data,
      mappedModel
    };
...
async function runSampleTest() {
  ...
  console.log(`🎲 Selected random subset of ${conversations.length} conversations for testing`);
  ...
  for (let i = 0; i < conversations.length; i++) {
    ...
    console.log(`Status: ${result.status} | Time: ${result.responseTime}ms | Cache: ${result.cacheType}`);
