...
console.log(
	"Testing 30 prompts with varying similarity to find the caching threshold",
);
console.log("Current threshold: 0.92 (92%)\n");

// Base prompt for comparison
const BASE_PROMPT =
	"What is artificial intelligence and how does it work in 2025?";
...
async function testSemanticBoundary() {
	...
	const validResults = results.filter(
		(r) => r.similarity !== "N/A" && r.similarity !== null,
	);
	validResults.sort(
		(a, b) => parseFloat(b.similarity || 0) - parseFloat(a.similarity || 0),
	);

	console.log("🟢 **SEMANTIC CACHE HITS (Above 0.92 threshold):**");
	validResults
		.filter((r) => parseFloat(r.similarity) >= 0.92)
		.forEach((r) => {
	...
	validResults
		.filter(
			(r) =>
				parseFloat(r.similarity) >= 0.85 && parseFloat(r.similarity) < 0.92,
		)
		.forEach((r) => {
			console.log(`  ${r.similarity} - "${r.prompt.substring(0, 50)}..."`);
		});
	...
	validResults
		.filter(
			(r) => parseFloat(r.similarity) >= 0.7 && parseFloat(r.similarity) < 0.85,
		)
		.forEach((r) => {
			console.log(`  ${r.similarity} - "${r.prompt.substring(0, 50)}..."`);
		});
	...
	validResults
		.filter((r) => parseFloat(r.similarity) < 0.7)
		.forEach((r) => {
			console.log(`  ${r.similarity} - "${r.prompt.substring(0, 50)}..."`);
		});
	...
	const misses = results.filter(
		(r) => r.cacheType === "miss" || r.cacheType === "none",
	).length;
...
async function makeRequest(content, testName) {
	...
	try {
		...
		console.log(
			`${testName}: ${responseTime}ms | ${cacheType} | ${semanticSimilarity} | "${content.substring(0, 40)}..."`,
		);
