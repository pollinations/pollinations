...
function extractRequestInfo(req) {
	try {
		return {
			...
			user_agent: req.headers?.["user-agent"] || req.headers?.get?.("user-agent") || "unknown",
			referrer: req.headers?.["referer"] || req.headers?.get?.("referer") || "unknown",
			content_type: req.headers?.["content-type"] || req.headers?.get?.("content-type") || "unknown"
		};
	} catch (error) {
		return {
			method: "unknown",
...
export function logRateLimitError(error, authResult, req, options = {}) {
	try {
		...
		const logEntry = {
			...
			context: {
				queue_utilization_percent: error.queueInfo?.maxAllowed ? 
					Math.round((error.queueInfo.total / error.queueInfo.maxAllowed) * 100) : 0,
		...
		log(`Rate limit error logged for ${authResult.username || ip}: ${error.queueInfo?.total}/${error.queueInfo?.maxAllowed} queue usage`);
...
export function getRateLimitStats(hours = 1) {
	try {
		...
		if (recentErrors.length > 0) {
			stats.avg_queue_utilization = Math.round(totalUtilization / recentErrors.length);
		}
