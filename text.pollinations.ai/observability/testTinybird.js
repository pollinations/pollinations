...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('  send-batch [count] - Send multiple test events (default: 5)');
      process.exit(1);
  }
...
async function sendSuccessEvent() {
  ...
  await sendTinybirdEvent({
    ...
    content: 'This is a detailed response to the user query about machine learning techniques for natural language processing.',
    messages: [
      { role: 'system', content: 'You are an AI expert specialized in machine learning.' },
      { role: 'user', content: 'Can you explain the most effective ML techniques for NLP tasks?' }
    ],
    organization: 'pollinations',
    project: 'text.pollinations.ai',
    environment: 'testing',
...
async function sendErrorEvent() {
  ...
  await sendTinybirdEvent({
    ...
    error,
    messages: [
      { role: 'user', content: 'Generate a very long response about quantum computing' }
    ],
    organization: 'pollinations',
    project: 'text.pollinations.ai',
    environment: 'testing',
...
async function sendBatchEvents(count) {
  ...
  const providers = ['openai', 'azure-openai', 'anthropic', 'mistral', 'google'];
  const models = ['gpt-4-turbo', 'claude-3-opus', 'gpt-3.5-turbo', 'mistral-large', 'gemini-pro'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const completionTokens = status === 'success' ? Math.floor(Math.random() * 300) + 20 : 0;
    ...
    await sendTinybirdEvent({
      ...
      promptTokens,
      completionTokens,
      content: status === 'success' ? `This is test response ${i+1}` : undefined,
      messages: [
        { role: 'user', content: `This is test prompt ${i+1}` }
      ],
      organization: 'pollinations',
      project: 'text.pollinations.ai',
      environment: 'testing',
