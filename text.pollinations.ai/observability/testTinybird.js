        ...
        messages: [
            {
                role: "system",
                content:
                    "You are an AI expert specialized in machine learning.",
            },
            {
                role: "user",
                content:
        ...
        messages: [
            {
                role: "user",
                content:
                    "Generate a very long response about quantum computing",
            },
        ],
        organization: "pollinations",
        project: "text.pollinations.ai",
...
async function sendBatchEvents(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        console.log(
            `Sent event ${i + 1}/${count}: ${status} ${provider} ${model}`,
        );
