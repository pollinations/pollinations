...
async function loadLocalModels() {
    try {
        ...
        console.log(`✅ Loaded ${availableModels.length} models from local file`);
        ...
        console.error('Make sure availableModels.js exists and is properly formatted');
        return [];
    }
}
...
async function getAvailableModels() {
    ...
    if (!serverRunning) {
        ...
        console.log('💡 Loading models from local file only (no API testing will be performed)');
    }
...
async function testModelForOriginalName(modelName, modelMetadata = null) {
    try {
        ...
        if (modelMetadata && modelMetadata.output_modalities) {
            ...
            isAudioModel = modelName.includes('audio') || modelName.includes('hypnosis');
        }
        ...
        if (stderr) {
            ...
            console.log(`  ❌ Error (${duration}ms): ${errorMsg.substring(0, 100)}`);
        ...
        try {
            ...
            if (response.error) {
                console.log(`  ❌ API Error (${duration}ms): ${JSON.stringify(response.error)}`);
                return { 
                    modelName, 
                    originalName: null, 
...
async function updateAvailableModelsFile(results) {
    try {
        ...
        for (const result of results) {
            if (result.success && result.originalName && result.originalName !== result.modelName) {
                ...
                if (newContent !== content) {
                    content = newContent;
                    updateCount++;
                    console.log(`  ✅ Updated ${result.modelName} → ${result.originalName}`);
                } else {
                    console.log(`  ⚠️  Could not update ${result.modelName} (pattern not found)`);
                }
            }
        }
...
async function discoverAllOriginalNames() {
    ...
    if (!canTestAPI) {
        ...
        console.log('   5. Then run this script again: node scripts/getOriginalModelNamesFromAPI.js');
    ...
    for (let i = 0; i < modelNames.length; i += batchSize) {
        ...
        for (const modelName of batch) {
            ...
            const result = await testModelForOriginalName(modelName, modelMetadata);
            results.push(result);
    ...
    const different = successful.filter(r => r.originalName && r.originalName !== r.modelName);
    const same = successful.filter(r => !r.originalName || r.originalName === r.modelName);
        ...
        Object.entries(errorsByType).forEach(([errorType, failures]) => {
            ...
            failures.forEach(result => {
                ...
                if (result.rawResponse) {
                    console.log(`     📝 Raw Response: ${result.rawResponse.substring(0, 150)}...`);
                }
                console.log('');
            });
        });
    ...
    for (const apiModel of apiModels) {
        ...
        console.log(`  ${status} ${apiModel.name.padEnd(20)} | API alias: ${aliases.padEnd(30)} | Actual: ${originalName}`);
    }
    ...
    console.log('='.repeat(80));
    console.log(`📈 Successfully discovered ${different.length} different model names`);
    console.log(`📝 Updated ${updateCount} models in availableModels.js`);
    console.log(`🎯 API provided ${apiModels.length} models, tested ${results.length}`);
...
function parseArguments() {
    ...
    if (args.includes('--help') || args.includes('-h')) {
        ...
        console.log('=====================================\n');
        console.log('Usage:');
        console.log('  node scripts/getOriginalModelNamesFromAPI.js [model_name]\n');
        console.log('Examples:');
        console.log('  node scripts/getOriginalModelNamesFromAPI.js              # Test all models');
        console.log('  node scripts/getOriginalModelNamesFromAPI.js openai       # Test only "openai" model');
        console.log('  node scripts/getOriginalModelNamesFromAPI.js qwen         # Test only "qwen" model\n');
        console.log('Options:');
