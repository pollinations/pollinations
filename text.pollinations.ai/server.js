...
import {
  ...
  createStreamingAdWrapper,
} from "./ads/initRequestFilter.js";
import {
  getRequestData,
  getReferrer,
  WHITELISTED_DOMAINS,
...
export function getQueue(ip) {
  if (!queues.has(ip)) {
    queues.set(
      ip,
      new PQueue({ concurrency: 1, interval: 6000, intervalCap: 1 }),
    );
  }
    ...
    if (completion.choices?.[0]?.message?.content) {
      ...
      if (!isAudioResponse) {
        try {
          ...
          processedContent = await processRequestForAds(
            processedContent,
            req,
            requestData.messages,
          );
...
export async function processRequest(req, res, requestData) {
  ...
  const bypassQueue =
    requestData.isImagePollinationsReferrer ||
    requestData.isRobloxReferrer ||
    shouldBypassDelay(req);
