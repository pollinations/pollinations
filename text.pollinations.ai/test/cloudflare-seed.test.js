...
process.env.CLOUDFLARE_AUTH_TOKEN =
	process.env.CLOUDFLARE_AUTH_TOKEN || "test-token";
...
test("transformCloudflareRequest should remove seed parameter", (t) => {
	...
	t.false(
		"seed" in transformedBody,
		"Seed parameter should be removed from the request body",
	);

	// Verify other parameters are preserved
	t.is(
		transformedBody.model,
		testRequestBody.model,
		"Model parameter should be preserved",
	);
	t.is(
		transformedBody.temperature,
		testRequestBody.temperature,
		"Temperature parameter should be preserved",
	);
	t.deepEqual(
		transformedBody.messages,
		testRequestBody.messages,
...
test("transformCloudflareRequest should remove seed parameter in streaming mode", (t) => {
	...
	t.false(
		"seed" in transformedBody,
		"Seed parameter should be removed from the streaming request body",
	);
	...
	t.is(
		transformedBody.stream,
		testRequestBody.stream,
		"Stream parameter should be preserved",
	);
	t.deepEqual(
		transformedBody.messages,
		testRequestBody.messages,
