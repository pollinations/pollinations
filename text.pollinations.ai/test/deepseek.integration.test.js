...
test.serial("should respect seed parameter", async (t) => {
  ...
  try {
    ...
    const response3 = await generateDeepseek(messages, {
      model: "deepseek-chat",
      seed: seed + 1,
      temperature: 0.7,
    });
    ...
    if (
      ...
      t.is(
        content1,
        content2,
        "Responses with the same seed should be identical",
      );
      ...
      if (content1 !== content3) {
        ...
        t.pass(
          "Responses with different seeds happened to be the same (this is possible but unlikely)",
        );
      }
