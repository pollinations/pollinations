 ...
 */
test.serial(
	"Streaming response should have exactly one [DONE] event",
	async (t) => {
		const response = await axiosInstance.post(
			"/openai/chat/completions",
			{
				messages: [
					{ role: "user", content: "Write a short sentence about testing" },
				],
				model: "openai",
				stream: true,
				cache: false,
 ...
 */
test.serial(
	"Azure OpenAI via Portkey streaming should have exactly one [DONE] event",
	async (t) => {
		...
		const response = await axiosInstance.post(
			"/openai/chat/completions",
			{
				messages: [
					{
						role: "user",
						content: "Write a short sentence about cloud computing",
					},
				],
		...
		log(
			`Found ${doneCount} [DONE] events in the Azure OpenAI via Portkey response`,
		);
