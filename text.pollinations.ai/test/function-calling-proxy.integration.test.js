...
test("Function calling - tools parameter is correctly passed through the proxy", async (t) => {
    ...
    mockApp.post("/v1/chat/completions", (req, res) => {
        ...
        t.truthy(
            req.body.tools,
            "Tools parameter should be present in the request",
        );
        t.truthy(
            req.body.tool_choice,
            "Tool choice parameter should be present in the request",
        );

        // Check if the tools parameter has the correct structure
        t.is(req.body.tools.length, 1, "Should have exactly one tool");
        t.is(
            req.body.tools[0].type,
            "function",
            "Tool type should be function",
        );
        t.is(
            req.body.tools[0].function.name,
            "get_weather",
        ...
        t.is(
            req.body.tool_choice.type,
            "function",
            "Tool choice type should be function",
        );
        t.is(
            req.body.tool_choice.function.name,
            "get_weather",
            "Tool choice function name should be get_weather",
        );
    ...
    try {
        ...
        t.is(
            response.data.choices[0].message.tool_calls[0].function.name,
            "get_weather",
            "Function name in response should be get_weather",
        );
...
test("Function calling - streaming with tools parameter", async (t) => {
    ...
    try {
        ...
        t.is(
            ...
            "Response content type should be SSE",
        );
        ...
        t.true(
            streamData.includes("data:"),
            "Stream should include data events",
        );
        t.true(
            streamData.includes("[DONE]"),
            "Stream should include completion marker",
        );
