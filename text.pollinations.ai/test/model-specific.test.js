...
availableModels.forEach((model) => {
    test(`should return correct response for ${model.name}`, async (t) => {
        try {
            ...
            const potentiallyUnavailableModels = [
                "gemini",
                "gemini-thinking",
                "claude-hybridspace",
                "deepseek",
            ...
            if (
                potentiallyUnavailableModels.includes(model.name) &&
                response.status === 500
            ) {
                // Skip the test for models that are known to be potentially unavailable
                t.pass(
                    `Skipping test for ${model.name} as it appears to be unavailable (status: ${response.status})`,
                );
            } else {
                ...
                t.is(
                    response.status,
                    200,
                    `Response status for ${model.name} should be 200`,
                );
                t.truthy(
                    response.data,
                    `Response for ${model.name} should contain data`,
                );
            }
        } catch (error) {
            ...
            if (potentiallyUnavailableModels.includes(model.name)) {
                // Skip the test for models that are known to be potentially unavailable
                t.pass(
                    `Skipping test for ${model.name} as it appears to be unavailable`,
                );
            } else {
...
const chatModels = availableModels.filter(
    (model) => model.type === "chat" && model.baseModel === true,
);
...
for (const modelConfig of chatModels) {
    test(`Seed behavior for ${modelConfig.name} model`, async (t) => {
        ...
        if (potentiallyUnavailableModels.includes(modelConfig.name)) {
            ...
            try {
                ...
                if (testResponse.status !== 200) {
                    ...
                    t.pass(
                        `Skipping seed test for ${modelConfig.name} as it appears to be unavailable (status: ${testResponse.status})`,
                    );
                    return;
                }
            } catch (error) {
                ...
                t.pass(
                    `Skipping seed test for ${modelConfig.name} as it appears to be unavailable (error: ${error.message})`,
                );
                return;
            }
        }
        ...
        const prompt =
            "Tell me a random number between 1 and 100. Also list 5 random colors.";
        ...
        try {
            ...
            if (potentiallyUnavailableModels.includes(modelConfig.name)) {
                t.pass(
                    `Skipping seed test for ${modelConfig.name} due to error: ${error.message}`,
                );
            } else {
                // For other models, fail the test
                t.fail(
                    `Seed test for ${modelConfig.name} failed: ${error.message}`,
                );
            }
        }
    });
