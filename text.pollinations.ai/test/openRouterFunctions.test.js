...
test("OpenRouter function calling - calculator", async (t) => {
    ...
    const messages = [{ role: "user", content: "Calculate 15 * 7 + 22" }];
    ...
    console.log(
        "OpenRouter calculator response:",
        JSON.stringify(response, null, 2),
    );
...
test.skip("OpenRouter function calling - complete flow", async (t) => {
    ...
    const response1 = await generateTextOpenRouter(messages, {
        tools: [weatherTool],
        tool_choice: "auto",
    });
    // Check if the model called the function
    if (!response1.choices[0].message.tool_calls) {
        t.pass(
            "Model did not call the function, skipping function execution test",
        );
        return;
    }
    ...
    const response2 = await generateTextOpenRouter(updatedMessages, {
        tools: [weatherTool],
        tool_choice: "auto",
    });
