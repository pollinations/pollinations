...
test("Azure OpenAI configuration should be set up correctly", (t) => {
  ...
  t.truthy(
    portkeyConfig["gpt-4o-mini"],
    "Should have configuration for gpt-4o-mini",
  );
  ...
  const azureModels = Object.entries(portkeyConfig).filter(
  ...
  for (const [model, config] of azureModels) {
    t.truthy(
      ...
      `${model} should be an Azure model`,
    );
    t.truthy(
      config["azure-resource-name"],
      `${model} should have azure-resource-name`,
    );
    t.truthy(
      config["azure-deployment-id"],
      `${model} should have azure-deployment-id`,
    );
    t.truthy(config["azure-api-key"], `${model} should have azure-api-key`);
    t.truthy(
      config["azure-api-version"],
      `${model} should have azure-api-version`,
    );
  }
});
