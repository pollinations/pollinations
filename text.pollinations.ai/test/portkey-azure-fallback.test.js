...
import { extractBaseUrl, extractResourceName, extractDeploymentName, azureConfig } from '../generateTextPortkey.js';
 ...
 * 
 * Expected behavior:
 * 1. The azureConfig object should contain configurations for all supported models
...
test('Azure OpenAI configuration should be set up correctly', t => {
    // Check that azureConfig exists and has the expected properties
    t.truthy(azureConfig, 'azureConfig should exist');
    t.truthy(azureConfig['gpt-4o-mini'], 'Should have configuration for gpt-4o-mini');
    t.truthy(azureConfig['gpt-4o'], 'Should have configuration for gpt-4o');
    t.truthy(azureConfig['o1-mini'], 'Should have configuration for o1-mini');
    
    // Check that each configuration has the expected properties
    for (const [model, config] of Object.entries(azureConfig)) {
        t.truthy(config.baseUrl, `${model} should have baseUrl`);
        t.truthy(config.resourceName, `${model} should have resourceName`);
        t.truthy(config.deploymentName, `${model} should have deploymentName`);
        t.truthy(config.apiKey, `${model} should have apiKey`);
    }
});