...
The battery life is amazing and the performance is outstanding.`;
  ...
  t.true(
    result.includes("pollinations.ai/referral?topic="),
    "Should add at least one referral link",
  );
  t.true(result.includes("MacBook"), "Should preserve product mention");
  t.true(
    result.includes("# Product Review"),
...
5. Get the Apple Watch`;
  ...
  const result = await processReferralLinks(content);
  const linkCount = (result.match(/pollinations\.ai\/referral\?topic=/g) || [])
    .length;

  t.true(linkCount <= 3, "Should not add more than 3 referral links");
  t.true(
    result.includes("# Shopping Guide"),
...
This is some test content with a product mention.`;
  ...
  t.is(
    skippedResult,
    content,
    "Should return original content when probability check fails",
  );
  ...
  t.not(
    processedResult,
    content,
    "Should process content when probability check passes",
  );
});
