...
console.log(
  ...
  process.env.AZURE_OPENAI_LARGE_ENDPOINT ? "Set" : "Not set",
);
console.log(
  "AZURE_OPENAI_LARGE_API_KEY:",
  process.env.AZURE_OPENAI_LARGE_API_KEY ? "Set" : "Not set",
);
...
test("Basic function calling with OpenAI", async (t) => {
  ...
  try {
    ...
    console.log(
      "Calling OpenAI with function calling enabled and required tool choice...",
    );
    const response = await generateTextPortkey(
      ...
      false,
    ); // Don't use performSearch flag, use proxy mode
    ...
    if (response) {
      ...
      if (response.choices && response.choices.length > 0) {
        console.log(
          ...
          response.choices[0].message ? "Yes" : "No",
        );
        if (response.choices[0].message) {
          console.log(
            "Message has tool_calls:",
            response.choices[0].message.tool_calls ? "Yes" : "No",
          );
        }
      }
    }
    ...
    if (response.choices[0].message.tool_calls) {
      ...
      t.true(
        ["web_search", "web_scrape", "get_weather"].includes(
      ...
      t.fail(
        'Model did not call the function even with tool_choice set to "required"',
      );
    }
  } catch (error) {
