...
if (!token) {
  console.error("❌  MY_POLLINATIONS_TOKEN not found in .env file (text.pollinations.ai/.env)");
  process.exit(1);
}
...
try {
  ...
  console.error(`Failed to fetch model list from ${API_BASE}/models - ${err.message}`);
  process.exit(1);
}
...
for (const modelName of textModels) {
  const model = typeof modelName === "string" ? modelName : modelName.id || modelName.name || "unknown";
  ...
  try {
    ...
    if (response.ok) {
      ...
      results.push({ model: model, status: "success", statusCode: response.status, time: elapsed });
    } else {
      console.error(`❌  [${model}] ${response.status} - ${body.slice(0, 120)}...`);
      results.push({
        model: model,
        status: "http_error",
