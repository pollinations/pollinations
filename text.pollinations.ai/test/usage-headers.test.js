...
describe("Usage headers - Non-streaming (Issue #4638)", () => {
    ...
    it("should have consistent token counts", async () => {
        ...
        const promptTokens = parseInt(response.headers.get("x-usage-prompt-text-tokens") || "0", 10);
        const completionTokens = parseInt(response.headers.get("x-usage-completion-text-tokens") || "0", 10);
