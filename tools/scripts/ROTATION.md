...
| `PLN_IMAGE_BACKEND_TOKEN` | EC2 image service → GPU workers | image `env.json` | RunPod pods (Flux+Z-Image, Klein), Lambda Labs GH200 (LTX-2, ACE-Step, Sana) |
...
| `PLN_IMAGE_BACKEND_TOKEN` in SOPS but not GPU workers | EC2 sends new token → GPU workers reject → image generation fails |
| `PLN_IMAGE_BACKEND_TOKEN` on GPU workers but not SOPS | GPU workers expect new token → EC2 sends old → image generation fails |
