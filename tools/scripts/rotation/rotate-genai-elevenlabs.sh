...
open_pr_and_merge "$BRANCH" \
    ...
    "Rotates \`ELEVENLABS_API_KEY\` runtime key (new SA key). Wrangler already has the new value live. This PR syncs SOPS + triggers a worker redeploy via production. Automated by \`rotate-genai-elevenlabs.sh\`." \
    || exit 1
