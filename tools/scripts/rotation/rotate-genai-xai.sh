...
open_pr_and_merge "$BRANCH" \
    "rotate: xAI API key" \
    "Rotates \`XAI_API_KEY\` via create-new + delete-old (replaces the old in-place /rotate endpoint). Old key \`$OLD_KEY_ID\` stays valid until this PR merges, production deploys, and health check passes. Automated by \`rotate-genai-xai.sh\`." \
    || exit 1
...
HC_META=$(curl -sS --max-time 90 -o "$HC_TMP" \
    ...
    "$GEN_BASE/image/healthcheck%20cat?model=$HEALTH_MODEL&width=512&height=512&seed=$(date +%s)" \
